...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        let show_chatgpt_usage_link = config.model_provider.requires_firefamai_auth;
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from("Firefam").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{FIREFAM_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run firefam login to use ChatGPT)".to_string()
            }
        });
        ...
        let note_first_line = Line::from(vec![
            ...
            "https://chatgpt.com/firefam/settings/usage"
                .cyan()
                .underlined(),
        ...
        // The ChatGPT usage page only applies to providers backed by FirefamAI auth;
...
fn format_model_provider(config: &Config, runtime_base_url: Option<&str>) -> Option<String> {
    ...
    let is_default_firefamai = provider.is_firefamai() && base_url.is_none();
    if is_default_firefamai {
        return None;
    }
