...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestFirefamHelperPaths,
    child: Child,
...
pub(crate) struct TestFirefamHelperPaths {
    pub(crate) firefam_exe: PathBuf,
    pub(crate) firefam_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_firefam_helper_paths() -> anyhow::Result<TestFirefamHelperPaths> {
    let (helper_binary, firefam_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestFirefamHelperPaths {
        firefam_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.firefam_exe);
