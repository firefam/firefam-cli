...
use firefam_apply_patch::FIREFAM_CORE_APPLY_PATCH_ARG1;
use firefam_exec_server::FIREFAM_FS_HELPER_ARG1;
use firefam_sandboxing::landlock::FIREFAM_LINUX_SANDBOX_ARG0;
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "firefam-execve-wrapper";
...
pub struct Arg0DispatchPaths {
    ...
    /// instead of the real Firefam CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(firefam_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if exe_name == FIREFAM_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        firefam_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        firefam_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == FIREFAM_FS_HELPER_ARG1 {
        firefam_exec_server::run_fs_helper_main();
    }
    if argv1 == FIREFAM_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match firefam_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(firefam_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    firefam_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!(
                    "Error: {FIREFAM_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument."
                );
                1
            }
        };
    ...
    match prepend_path_entry_for_firefam_aliases() {
...
/// While we want to deploy the Firefam CLI as a single executable for simplicity,
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        firefam_self_exe: current_exe.clone(),
        firefam_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `firefam-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().firefam_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
fn load_dotenv() {
    if let Ok(firefam_home) = find_firefam_home()
        && let Ok(iter) = dotenvy::from_path_iter(firefam_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `FIREFAM_` keys.
...
///   with the hidden `--firefam-run-as-apply-patch` flag.
///
...
pub fn prepend_path_entry_for_firefam_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if firefam_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (firefam_home: {firefam_home:?})"
                ),
            ));
        }
    }
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {FIREFAM_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        firefam_self_exe: std::env::current_exe().ok(),
        firefam_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(FIREFAM_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_firefam_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("firefam-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                firefam_self_exe: Some(PathBuf::from("/usr/bin/firefam")),
                firefam_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/firefam"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("firefam-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/firefam")),
            |paths| async move {
