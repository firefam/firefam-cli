...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("FIREFAM_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("FIREFAM_THREAD_ID".to_string(), "thread-1".to_string()),
        (
...
fn exec_server_params_use_env_policy_overlay_contract() {
    let cwd: firefam_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        firefam_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = firefam_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = firefam_protocol::models::PermissionProfile::Disabled;
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: firefam_exec_server::ExecEnvPolicy {
                inherit: firefam_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: firefam_sandboxing::SandboxType::None,
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let firefam_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        firefam_protocol::protocol::ExecCommandStatus::Failed
    );
