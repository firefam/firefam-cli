...
use firefam_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use firefam_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use firefam_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use firefam_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Firefam-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Firefam attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Firefam attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Firefam was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `firefam`).
