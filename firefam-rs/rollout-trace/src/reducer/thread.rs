...
impl TraceReducer {
    ...
    /// Starts a Firefam turn inside an existing thread.
    pub(super) fn start_firefam_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.firefam_turns.contains_key(&firefam_turn_id) {
            bail!("duplicate firefam turn start for {firefam_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.firefam_turns.insert(
    ...
    /// Marks a Firefam turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_firefam_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.firefam_turns.get(&firefam_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "firefam turn end for {firefam_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.firefam_turns.get_mut(&firefam_turn_id) else {
            bail!("firefam turn end referenced unknown turn {firefam_turn_id}");
        };
