...
function Get-ReleaseAssetMetadata {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/firefam/firefam-cli/releases/tags/rust-v$ResolvedVersion"
    $asset = $release.assets | Where-Object { $_.name -eq $AssetName } | Select-Object -First 1
    if ($null -eq $asset) {
        throw "Could not find release asset $AssetName for Firefam $ResolvedVersion."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Firefam archive checksum did not match release metadata. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Version {
    ...
    $release = Invoke-RestMethod -Uri "https://api.github.com/repos/firefam/firefam-cli/releases/latest"
    if (-not $release.tag_name) {
        Write-Error "Failed to resolve the latest Firefam release version."
        exit 1
    }
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Firefam install at $VisibleBinDir"
    Write-WarningStep "To continue, Firefam needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Firefam setup now?")) {
...
function Get-ConflictingInstall {
    ...
    Write-Step "Detected existing $manager-managed Firefam at $existingPath"
    Write-WarningStep "Multiple managed Firefam installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "firefam/firefam-cli")
    } else {
        @("uninstall", "-g", "firefam/firefam-cli")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed Firefam now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Firefam. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Firefam installed. PATH order will determine which firefam runs."
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Firefam requires a 64-bit version of Windows."
    exit 1
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Firefam CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Firefam CLI"
} else {
    Write-Step "Installing Firefam CLI"
}
...
$packageAsset = "firefam-npm-$npmTag-$resolvedVersion.tgz"
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("firefam-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target)) {
            ...
            Write-Step "Downloading Firefam CLI"
...
Write-Step "Current PowerShell session: firefam"
