...
struct TestHandler {
    tool_name: firefam_tools::ToolName,
}

#[async_trait::async_trait]
impl ToolExecutor<ToolInvocation> for TestHandler {
    fn tool_name(&self) -> firefam_tools::ToolName {
...
enum RecordedToolLifecycle {
    ...
    Finish {
        call_id: String,
        tool_name: firefam_tools::ToolName,
        outcome: firefam_extension_api::ToolCallOutcome,
    },
}
...
impl firefam_extension_api::ToolLifecycleContributor for ToolLifecycleRecorder {
    fn on_tool_start<'a>(
        &'a self,
        input: firefam_extension_api::ToolStartInput<'a>,
    ) -> firefam_extension_api::ToolLifecycleFuture<'a> {
    ...
    fn on_tool_finish<'a>(
        &'a self,
        input: firefam_extension_api::ToolFinishInput<'a>,
    ) -> firefam_extension_api::ToolLifecycleFuture<'a> {
        let records = Arc::clone(&self.records);
...
fn handler_looks_up_namespaced_aliases_explicitly() {
    let namespace = "mcp__firefam_apps__gmail";
    let tool_name = "gmail_get_recent_emails";
    let plain_name = firefam_tools::ToolName::plain(tool_name);
    let namespaced_name = firefam_tools::ToolName::namespaced(namespace, tool_name);
    ...
    let missing_namespaced = registry.tool(&firefam_tools::ToolName::namespaced(
        "mcp__firefam_apps__calendar",
        tool_name,
    ));
...
async fn dispatch_notifies_tool_lifecycle_contributors() -> anyhow::Result<()> {
    ...
    let ok_tool = firefam_tools::ToolName::plain("ok_tool");
    let failing_tool = firefam_tools::ToolName::plain("failing_tool");
    ...
    let expected = vec![
        ...
        RecordedToolLifecycle::Finish {
            call_id: "ok-call".to_string(),
            tool_name: ok_tool,
            outcome: firefam_extension_api::ToolCallOutcome::Completed { success: false },
        },
        RecordedToolLifecycle::Start {
        ...
        RecordedToolLifecycle::Finish {
            ...
            outcome: firefam_extension_api::ToolCallOutcome::Failed {
