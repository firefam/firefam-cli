...
use clap::Parser;
use firefam_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `firefam resume` subcommand.
    // These are not exposed as user flags on the base `firefam` command.
    #[clap(skip)]
    ...
    /// top-level `firefam resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `firefam fork` subcommand.
    // These are not exposed as user flags on the base `firefam` command.
    #[clap(skip)]
    ...
    /// top-level `firefam fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
