...
`firefam-exec-server` is the library backing `firefam exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`firefam-utils-pty`.

It provides:

- a CLI entrypoint: `firefam exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `firefam` binary owns hidden helper dispatch for sandboxed
filesystem operations and `firefam-linux-sandbox`.

## Transport

The server speaks the shared `firefam-app-server-protocol` message envelope on
the wire.
...
It uses the standard Firefam ChatGPT sign-in state; run `firefam login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `FIREFAM_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Firefam then registers an Agent task and sends the
...
`firefam.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/firefam.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `firefam-utils-pty`.
...
hidden helper process launched from the top-level `firefam` executable and
...
`firefam exec-server` command builds these paths from the `firefam` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Firefam auth
