...
usage() {
  cat <<'EOF'
Usage: build-firefam-package-archive.sh \
  --target <rust-target> \
...
case "$bundle" in
  primary)
    variant="firefam"
    entrypoint="firefam"
    archive_stem="firefam-package"
    ;;
  app-server)
    variant="firefam-app-server"
    entrypoint="firefam-app-server"
    archive_stem="firefam-app-server-package"
    ;;
  *)
...
"$python_bin" "${repo_root}/scripts/build_firefam_package.py" \
  --target "$target" \
  --variant "$variant" \
