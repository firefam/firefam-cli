...
use crate::environment_toml::environment_provider_from_firefam_home;
...
pub const FIREFAM_EXEC_SERVER_URL_ENV_VAR: &str = "FIREFAM_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the Firefam runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `FIREFAM_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `FIREFAM_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    pub async fn from_firefam_home(
        firefam_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
    ) -> Result<Self, ExecServerError> {
        let provider = environment_provider_from_firefam_home(firefam_home.as_ref())?;
...
impl Environment {
    /// Builds an environment from the raw `FIREFAM_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `FIREFAM_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            firefam_protocol::models::PermissionProfile::from_runtime_permissions(
                &firefam_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                firefam_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
