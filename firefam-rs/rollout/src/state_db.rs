...
pub use firefam_state::LogEntry;
...
pub type StateDbHandle = Arc<firefam_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        firefam_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .map_err(|err| {
                anyhow::anyhow!(
    ...
    .await;
    firefam_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    runtime: &firefam_state::StateRuntime,
    firefam_home: &Path,
    ...
    loop {
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
                ...
                firefam_home.display()
            )
        })?;
        if backfill_state.status == firefam_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, firefam_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = firefam_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match firefam_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: firefam_otel::MetricsClient,
    originator: &str,
) -> firefam_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == firefam_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<firefam_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&firefam_state::StateRuntime>,
    firefam_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.firefam_home() != firefam_home {
        warn!(
            "state db firefam_home mismatch: expected {}, got {}",
            ctx.firefam_home().display(),
            firefam_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => firefam_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => firefam_state::SortKey::UpdatedAt,
            },
...
) -> Option<firefam_state::ThreadsPage> {
    let ctx = context?;
    if ctx.firefam_home() != firefam_home {
        warn!(
            "state db firefam_home mismatch: expected {}, got {}",
        ...
        .list_threads(
            ...
            firefam_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => firefam_state::SortDirection::Asc,
                    SortDirection::Desc => firefam_state::SortDirection::Desc,
                },
                search_term,
            },
        )
