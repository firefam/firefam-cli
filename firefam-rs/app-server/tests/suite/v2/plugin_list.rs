...
use firefam_app_server_protocol::PluginAuthPolicy;
use firefam_app_server_protocol::PluginInstallPolicy;
...
use firefam_core::config::set_project_trust_level;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "FIREFAM_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = firefam_home.path().to_string_lossy().into_owned();
...
{
    let firefam_home = TempDir::new()?;
    write_firefamai_curated_marketplace(
        firefam_home.path(),
...
"#,
    ...
    assert_eq!(response.marketplaces[0].name, "firefamai-curated");
    assert_eq!(
        ...
        vec![
            ...
            ("computer-use@firefamai-curated".to_string(), false, false),
        ]
    );
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_firefam_plugins_disabled() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_firefam_plugins_setting() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: firefam_app_server_protocol::PluginAvailability::Available,
                    interface: Some(firefam_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let firefam_home = TempDir::new()?;
    std::fs::create_dir_all(firefam_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(firefam_home.path())?;
    std::fs::write(
        firefam_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "firefam-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&firefam_home, "firefam-curated", "enabled-plugin")?;
    write_installed_plugin(&firefam_home, "firefam-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "firefam-curated",
...
[plugins."enabled-plugin@firefam-curated"]
enabled = true

[plugins."disabled-plugin@firefam-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "firefam-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@firefam-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@firefam-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@firefam-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let firefam_home = TempDir::new()?;
    std::fs::create_dir_all(firefam_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&firefam_home, "firefam-curated", "shared-plugin")?;
    std::fs::write(
        firefam_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "firefam-curated",
...
[plugins."shared-plugin@firefam-curated"]
enabled = true
"#,
    )?;
...
"#,
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@firefam-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    "developerName": "FirefamAI",
    "category": "Productivity",
    "capabilities": ["Interactive", "Write"],
    "websiteURL": "https://firefam.ai/",
    "privacyPolicyURL": "https://firefam.ai/policies/row-privacy-policy/",
    "termsOfServiceURL": "https://firefam.ai/policies/row-terms-of-use/",
    "defaultPrompt": [
    ...
    assert_eq!(plugin.id, "demo-plugin@firefam-curated");
    ...
    assert_eq!(
        ...
        Some("https://firefam.ai/")
    );
    assert_eq!(
        interface.privacy_policy_url.as_deref(),
        Some("https://firefam.ai/policies/row-privacy-policy/")
    );
    assert_eq!(
        interface.terms_of_service_url.as_deref(),
        Some("https://firefam.ai/policies/row-terms-of-use/")
    );
    assert_eq!(
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    ...
    write_firefamai_curated_marketplace(firefam_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":true}
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["linear@firefamai-curated"]"#))
        .mount(&server)
        .await;

    let marker_path = firefam_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(firefam_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
        ...
        let curated_marketplace = response
            ...
            .expect("expected firefamai-curated marketplace entry");
        assert_eq!(
            curated_marketplace
                .plugins
                .into_iter()
                ...
                .collect::<Vec<_>>(),
            vec![("linear@firefamai-curated".to_string(), true, true)]
        );
        wait_for_remote_plugin_request_count(&server, "/plugins/list", /*expected_count*/ 1)
            .await?;
    }
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = firefam_home
        .path()
        .join("plugins/cache/firefamai-curated-remote/linear/1.2.3");
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let old_path = firefam_home
        .path()
        .join("plugins/cache/firefamai-curated-remote/linear/1.0.0");
    let new_path = firefam_home
        .path()
        .join("plugins/cache/firefamai-curated-remote/linear/1.2.3");
    let stale_path = firefam_home
        .path()
        .join("plugins/cache/firefamai-curated-remote/stale");
    ...
    let remote_marketplace = response
        .marketplaces
        .into_iter()
        .find(|marketplace| marketplace.name == "firefamai-curated-remote")
        .expect("expected firefamai-curated-remote marketplace entry");
    assert_eq!(
        remote_marketplace
            .plugins
            .into_iter()
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    let remote_marketplace = response
        ...
        .into_iter()
        .find(|marketplace| marketplace.name == "firefamai-curated-remote")
        .expect("expected firefamai-curated remote marketplace");
    ...
    assert_eq!(
        ...
        firefam_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
#[tokio::test]
async fn plugin_list_includes_firefamai_curated_remote_collection_when_requested() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        firefam_home.path(),
  ...
  "pagination": {
    ...
    mount_firefamai_curated_remote_collection_plugin_list(&server, collection_body).await;
...
#[tokio::test]
async fn plugin_list_fail_opens_firefamai_curated_remote_collection_errors() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        firefam_home.path(),
    ...
    assert!(
        response
            .marketplaces
            .iter()
            .all(|marketplace| marketplace.name != "firefamai-curated-remote")
    );
    Ok(())
}

#[tokio::test]
async fn plugin_list_does_not_query_firefamai_curated_remote_collection_by_default() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        firefam_home.path(),
...
"#,
    ...
    assert_eq!(response.marketplaces.len(), 1);
    assert_eq!(response.marketplaces[0].name, "firefamai-curated-remote");
    assert_eq!(
        response.marketplaces[0]
            .plugins
            .iter()
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        firefam_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_firefamai_curated_marketplace(firefam_home.path(), &["linear", "gmail"])?;
    ...
    assert_eq!(
        ...
        vec!["linear@firefamai-curated".to_string()]
    );
    Ok(())
}
...
async fn mount_firefamai_curated_remote_collection_plugin_list(server: &MockServer, body: &str) {
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/list"))
        .and(query_param("scope", "GLOBAL"))
        .and(query_param("limit", "200"))
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(firefam_home, marketplace_name, plugin_name, "local")
}
...
fn write_firefamai_curated_marketplace(
    firefam_home: &std::path::Path,
    plugin_names: &[&str],
) -> std::io::Result<()> {
    let curated_root = firefam_home.join(".tmp/plugins");
...
{plugins}
    ...
    std::fs::create_dir_all(firefam_home.join(".tmp"))?;
    std::fs::write(
        firefam_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(firefam_home.join(".tmp"))?;
    std::fs::write(
        firefam_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
