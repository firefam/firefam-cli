...
use firefam_app_server_protocol::Config as ApiConfig;
use firefam_app_server_protocol::ConfigBatchWriteParams;
use firefam_app_server_protocol::ConfigLayerMetadata;
...
use firefam_app_server_protocol::OverriddenMetadata;
use firefam_app_server_protocol::WriteStatus;
...
use firefam_config::merge_toml_values;
use firefam_core::config::deserialize_config_toml_with_base;
use firefam_core::config::edit::ConfigEdit;
use firefam_core::config::edit::ConfigEditsBuilder;
use firefam_core::config::validate_feature_requirements_for_config_toml;
use firefam_core::path_utils;
use firefam_core::path_utils::SymlinkWritePaths;
use firefam_core::path_utils::resolve_symlink_write_paths;
use firefam_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.firefam_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .firefam/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_firefam_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_firefam_folder.display(),
        ),
