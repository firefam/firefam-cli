#!/usr/bin/env python3
"""
FirefamAI YAML Generator - Creates agents/firefamai.yaml for a skill folder.

Usage:
    generate_firefamai_yaml.py <skill_dir> [--name <skill_name>] [--interface key=value]
"""
...
def write_firefamai_yaml(skill_dir, skill_name, raw_overrides):
    ...
    output_path = agents_dir / "firefamai.yaml"
    output_path.write_text("\n".join(interface_lines) + "\n")
    print(f"[OK] Created agents/firefamai.yaml")
    return output_path


def main():
    parser = argparse.ArgumentParser(
        description="Create agents/firefamai.yaml for a skill directory.",
    )
    ...
    result = write_firefamai_yaml(skill_dir, skill_name, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)
