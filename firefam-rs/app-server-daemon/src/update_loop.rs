...
#[cfg(unix)]
use crate::managed_install::resolved_managed_firefam_bin;
...
async fn update_once(
    ...
    let managed_firefam_bin = resolved_managed_firefam_bin(&daemon.managed_firefam_bin).await?;
    let managed_identity = executable_identity(&managed_firefam_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_firefam_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_firefam_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_firefam_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Firefam binary {}",
            managed_firefam_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/firefam/install.sh")
        .await
        .context("failed to fetch standalone Firefam updater")?
        .error_for_status()
        .context("standalone Firefam updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone Firefam updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Firefam updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Firefam updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Firefam updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Firefam updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Firefam updater exited with status {status}")
    }
}
