...
use firefam_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-Firefam thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_firefam_thread_interactive(
    config: Config,
...
) -> Result<Firefam, FirefamErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let FirefamSpawnOk { firefam, .. } = Box::pin(Firefam::spawn(FirefamSpawnArgs {
        config,
        ...
        parent_rollout_thread_trace: firefam_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let firefam_for_events = Arc::clone(&firefam);
    ...
    let firefam_for_ops = Arc::clone(&firefam);
    tokio::spawn(async move {
        forward_ops(firefam_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Firefam {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: firefam.agent_status.clone(),
        session: Arc::clone(&firefam.session),
        session_loop_termination: firefam.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_firefam_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_firefam_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&firefam).await;
                break;
            }
            event = firefam.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&firefam, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(firefam: &Firefam) {
    let _ = firefam.submit(Op::Interrupt).await;
    let _ = firefam.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = firefam.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = firefam.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                firefam_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                firefam_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                firefam_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = firefam.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            firefam_protocol::request_user_input::RequestUserInputAnswer {
...
) -> firefam_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = firefam_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, firefam_protocol::protocol::ReviewDecision::Abort)
                .await;
            firefam_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
#[cfg(test)]
#[path = "firefam_delegate_tests.rs"]
mod tests;
