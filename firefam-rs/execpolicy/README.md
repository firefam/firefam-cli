...
- The legacy rule matcher lives in `firefam-execpolicy-legacy`.
...
- From the Firefam CLI, run `firefam execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
firefam execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
firefam execpolicy check \
...
```bash
cargo run -p firefam-execpolicy -- check --rules path/to/policy.rules git status
```
