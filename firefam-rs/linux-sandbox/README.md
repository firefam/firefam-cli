# firefam-linux-sandbox

This crate is responsible for producing:

- a `firefam-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Firefam CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `firefam-exec` CLI can check if its arg0 is `firefam-linux-sandbox` and, if so, execute as if it were `firefam-linux-sandbox`
  - this should also be true of the `firefam` multitool CLI

On Linux, Firefam prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `firefam-resources/bwrap` binary shipped
with Firefam.
Firefam also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Firefam surfaces the same startup warning
...
it cannot create the required user namespaces, so Firefam rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `firefam-resources/bwrap` path.
- If `bwrap` is missing, Firefam also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Firefam surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Firefam rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.firefam`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `firefam debug landlock`.
