...
`codex-network-proxy` is Firefam's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from Firefam's merged `config.toml` (via `codex-core` config loading).
...
When `codex-network-proxy` is embedded in managed Firefam runtime, policy decisions emit structured
