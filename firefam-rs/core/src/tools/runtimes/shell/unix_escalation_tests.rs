...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.firefam_home)
        .context("recreate firefam home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .firefam_home
        .join(firefam_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command = firefam_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(firefam_execpolicy::Policy::empty())),
        ...
        stopwatch: firefam_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        firefam_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        firefam_shell_escalation::EscalationDecision::Escalate(
            firefam_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
