...
#[path = "../src/proto/firefam.exec_server.relay.v1.rs"]
mod relay_proto;
...
use firefam_exec_server::InitializeParams;
use firefam_exec_server::InitializeResponse;
use firefam_exec_server::RemoteEnvironmentConfig;
use futures::SinkExt;
...
fn static_registry_auth_provider() -> firefam_api::SharedAuthProvider {
...
async fn multiplexed_remote_environment_routes_independent_virtual_streams() -> Result<()> {
    ...
    let (firefam_exe, firefam_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(firefam_exe, firefam_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(firefam_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
