...
use bytes::Bytes;
use firefam_client::HttpTransport;
use firefam_client::RequestBody;
use firefam_client::RequestTelemetry;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "firefam-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=firefam-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "firefam_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
