...
use core_test_support::firefam_linux_sandbox_exe_or_skip;
...
use firefam_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                firefam_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    let (mut session, mut turn_context_raw) = make_session_and_context().await;
    turn_context_raw.firefam_linux_sandbox_exe = firefam_linux_sandbox_exe_or_skip!();
    turn_context_raw
        .approval_policy
        .set(AskForApproval::OnRequest)
        .expect("test setup should allow updating approval policy");
    ...
    turn_context_raw.permission_profile = firefam_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        firefam_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: firefam_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let firefam_home = tempdir().expect("create firefam home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_firefam_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(firefam_thread_store::LocalThreadStore::new(
        firefam_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));

    let FirefamSpawnOk { firefam, .. } = Firefam::spawn(FirefamSpawnArgs {
        config,
        ...
        plugins_manager,
        mcp_manager,
        extensions: firefam_extension_api::empty_extension_registry(),
