...
use firefam_app_server_protocol::HookEventName;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                firefam_home.path().join("marketplace.json"),
            )?),
...
"#,
  ...
  "pagination": {
    ...
    assert_eq!(response.plugin.marketplace_name, "firefamai-curated-remote");
    assert_eq!(
...
#[tokio::test]
async fn plugin_read_returns_canonical_firefamai_curated_marketplace_name() -> Result<()> {
    let firefam_home = TempDir::new()?;
...
[plugins."demo-plugin@firefamai-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&firefam_home, "firefamai-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "firefamai-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@firefamai-curated");
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(firefam_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&firefam_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "firefam-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(firefam_home.path().join(".tmp"))?;
    std::fs::write(
        firefam_home
            .path()
...
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/thread-summarizer/agents/firefamai.yaml"),
        r#"policy:
  products:
    - FIREFAM
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/chatgpt-only/agents/firefamai.yaml"),
        r#"policy:
  products:
    - CHATGPT
"#,
...
[plugins."demo-plugin@firefam-curated"]
enabled = true

[hooks.state."demo-plugin@firefam-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&firefam_home, "firefam-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "firefam-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@firefam-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            firefam_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@firefam-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            firefam_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@firefam-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            firefam_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@firefam-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    write_connectors_config(firefam_home.path(), &server_url)?;
    write_chatgpt_auth(
        firefam_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(firefam_home: &TempDir) -> Result<()> {
    std::fs::write(
        firefam_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/firefam/apps", mcp_service);
