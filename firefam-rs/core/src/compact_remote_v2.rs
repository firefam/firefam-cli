...
use firefam_rollout_trace::CompactionCheckpointTracePayload;
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped { reason } => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    firefam_analytics::CompactionStatus::Interrupted,
                    Some(error),
                )
                .await;
...
) -> FirefamResult<(ResponseItem, String)> {
...
mod tests {
    ...
    fn response_stream(events: Vec<FirefamResult<ResponseEvent>>) -> ResponseStream {
