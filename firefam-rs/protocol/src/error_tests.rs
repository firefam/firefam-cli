...
fn usage_limit_reached_error_formats_plus_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/firefam/settings/usage to purchase more credits or try again later."
    );
}

#[test]
fn server_overloaded_maps_to_protocol() {
    let err = FirefamErr::ServerOverloaded;
    assert_eq!(
        err.to_firefam_protocol_error(),
        FirefamErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = FirefamErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = FirefamErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.firefam_error_info,
        Some(FirefamErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Firefam (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Visit https://chatgpt.com/firefam/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_firefam_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        let err = UsageLimitReachedError {
            ...
            promo_message: Some(
                "Visit https://chatgpt.com/firefam/settings/usage to purchase more credits"
                    .to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit for firefam_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_includes_hours_and_minutes() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/firefam/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Firefam, start a free trial of <PLAN> today".to_string(),
            ),
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Firefam, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
