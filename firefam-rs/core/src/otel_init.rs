...
use firefam_config::types::OtelExporterKind as Kind;
use firefam_config::types::OtelHttpProtocol as Protocol;
use firefam_features::Feature;
use firefam_login::default_client::originator;
use firefam_otel::OtelExporter;
use firefam_otel::OtelHttpProtocol;
use firefam_otel::OtelProvider;
use firefam_otel::OtelSettings;
use firefam_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Firefam-owned events via OTEL.
/// Keeps events that originated from firefam_otel module
pub fn firefam_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("firefam_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = firefam_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = firefam_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
    let _ = firefam_state::install_process_db_telemetry(telemetry);
}
