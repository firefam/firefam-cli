...
def _create_client():
    try:
        from firefamai import FirefamAI
    except ImportError:
        _die(f"firefamai SDK not installed in the active environment. {_dependency_hint('openai')}")
    return FirefamAI()


def _create_async_client():
    try:
        from firefamai import AsyncFirefamAI
    except ImportError:
        try:
            import firefamai as _firefamai  # noqa: F401
        except ImportError:
            _die(
                f"firefamai SDK not installed in the active environment. {_dependency_hint('openai')}"
            )
        _die(
            "AsyncFirefamAI not available in this firefamai SDK version. "
            f"{_dependency_hint('openai', upgrade=True)}"
        )
    return AsyncFirefamAI()
...
def _extract_retry_after_seconds(exc: Exception) -> Optional[float]:
    # Best-effort: firefamai SDK errors vary by version. Prefer a conservative fallback.
