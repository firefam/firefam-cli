...
use firefam_analytics::SubAgentThreadStartedInput;
use firefam_app_server_protocol::McpServerElicitationRequest;
use firefam_app_server_protocol::McpServerElicitationRequestParams;
use firefam_config::types::OAuthCredentialsStoreMode;
use firefam_exec_server::Environment;
use firefam_exec_server::EnvironmentManager;
use firefam_exec_server::FileSystemSandboxContext;
use firefam_extension_api::PromptSlot;
use firefam_features::FEATURES;
use firefam_features::Feature;
use firefam_features::unstable_features_warning_event;
use firefam_hooks::Hooks;
use firefam_hooks::HooksConfig;
use firefam_login::AuthManager;
use firefam_login::FirefamAuth;
use firefam_login::auth_env_telemetry::collect_auth_env_telemetry;
use firefam_login::default_client::originator;
use firefam_mcp::McpConnectionManager;
use firefam_mcp::McpRuntimeEnvironment;
use firefam_mcp::firefam_apps_tools_cache_key;
use firefam_models_manager::manager::RefreshStrategy;
use firefam_models_manager::manager::SharedModelsManager;
use firefam_network_proxy::NetworkProxy;
use firefam_network_proxy::NetworkProxyAuditMetadata;
use firefam_network_proxy::normalize_host;
use firefam_otel::current_span_trace_id;
use firefam_otel::current_span_w3c_trace_context;
use firefam_otel::set_parent_from_w3c_trace_context;
use firefam_protocol::ThreadId;
use firefam_protocol::account::PlanType as AccountPlanType;
use firefam_protocol::approvals::ElicitationRequestEvent;
use firefam_protocol::approvals::ExecPolicyAmendment;
use firefam_protocol::approvals::NetworkPolicyAmendment;
use firefam_protocol::approvals::NetworkPolicyRuleAction;
use firefam_protocol::config_types::ApprovalsReviewer;
use firefam_protocol::config_types::AutoCompactTokenLimitScope;
...
use firefam_protocol::dynamic_tools::DynamicToolResponse;
use firefam_protocol::dynamic_tools::DynamicToolSpec;
...
use firefam_protocol::models::AdditionalPermissionProfile;
...
use firefam_protocol::models::format_allow_prefixes;
use firefam_protocol::permissions::FileSystemSandboxPolicy;
use firefam_protocol::permissions::NetworkSandboxPolicy;
use firefam_protocol::protocol::FileChange;
use firefam_protocol::protocol::HasLegacyEvent;
use firefam_protocol::protocol::InterAgentCommunication;
use firefam_protocol::protocol::ItemCompletedEvent;
use firefam_protocol::protocol::ItemStartedEvent;
use firefam_protocol::protocol::RawResponseItemEvent;
...
use firefam_protocol::protocol::TurnContextItem;
use firefam_protocol::protocol::TurnContextNetworkItem;
...
use firefam_protocol::request_permissions::RequestPermissionsArgs;
use firefam_protocol::request_permissions::RequestPermissionsEvent;
use firefam_protocol::request_permissions::RequestPermissionsResponse;
use firefam_protocol::request_user_input::RequestUserInputArgs;
use firefam_protocol::request_user_input::RequestUserInputResponse;
use firefam_rmcp_client::ElicitationResponse;
use firefam_rollout::state_db;
use firefam_rollout_trace::AgentResultTracePayload;
use firefam_rollout_trace::ThreadStartedTraceMetadata;
use firefam_rollout_trace::ThreadTraceContext;
use firefam_sandboxing::policy_transforms::intersect_permission_profiles;
use firefam_shell_command::parse_command::parse_command;
use firefam_terminal_detection::user_agent;
use firefam_thread_store::CreateThreadParams;
use firefam_thread_store::LiveThread;
use firefam_thread_store::LiveThreadInitGuard;
use firefam_thread_store::LocalThreadStore;
use firefam_thread_store::ReadThreadParams;
use firefam_thread_store::ResumeThreadParams;
use firefam_thread_store::ThreadEventPersistenceMode;
use firefam_thread_store::ThreadPersistenceMetadata;
use firefam_thread_store::ThreadStore;
use firefam_utils_output_truncation::TruncationPolicy;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                firefam_error_info: Some(FirefamErrorInfo::BadRequest),
            },
...
use firefam_git_utils::get_git_repo_root;
use firefam_mcp::compute_auth_statuses;
use firefam_mcp::effective_mcp_servers_from_configured;
use firefam_mcp::host_owned_firefam_apps_enabled;
use firefam_otel::SessionTelemetry;
use firefam_otel::THREAD_STARTED_METRIC;
...
use firefam_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use firefam_protocol::config_types::ServiceTier;
use firefam_protocol::config_types::WindowsSandboxLevel;
use firefam_protocol::firefamai_models::ReasoningEffort as ReasoningEffortConfig;
...
use firefam_protocol::protocol::ApplyPatchApprovalRequestEvent;
use firefam_protocol::protocol::AskForApproval;
use firefam_protocol::protocol::CompactedItem;
use firefam_protocol::protocol::DeprecationNoticeEvent;
use firefam_protocol::protocol::ErrorEvent;
...
use firefam_protocol::protocol::FirefamErrorInfo;
use firefam_protocol::protocol::InitialHistory;
use firefam_protocol::protocol::McpServerRefreshConfig;
use firefam_protocol::protocol::ModelRerouteEvent;
use firefam_protocol::protocol::ModelRerouteReason;
use firefam_protocol::protocol::ModelVerification;
use firefam_protocol::protocol::ModelVerificationEvent;
use firefam_protocol::protocol::NetworkApprovalContext;
...
use firefam_protocol::protocol::RequestUserInputEvent;
...
use firefam_protocol::protocol::SessionNetworkProxyRuntime;
use firefam_protocol::protocol::StreamErrorEvent;
...
use firefam_protocol::protocol::WarningEvent;
use firefam_protocol::user_input::UserInput;
use firefam_tools::ToolEnvironmentMode;
use firefam_tools::UnifiedExecShellMode;
use firefam_tools::shell_command_backend_for_features;
use firefam_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use firefam_utils_stream_parser::ProposedPlanSegment;
...
/// Wrapper returned by [`Firefam::spawn`] containing the spawned [`Firefam`] and
/// the unique session id.
pub struct FirefamSpawnOk {
    pub firefam: Firefam,
    pub thread_id: ThreadId,
}

pub(crate) struct FirefamSpawnArgs {
    ...
    pub(crate) extensions: Arc<firefam_extension_api::ExtensionRegistry<crate::config::Config>>,
...
const CYBER_SAFETY_URL: &str = "https://support.firefam.ai/concepts/cyber-safety";

impl Firefam {
    /// Spawn a new [`Firefam`] and initialize the session.
    pub(crate) async fn spawn(args: FirefamSpawnArgs) -> FirefamResult<FirefamSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if firefam_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(FirefamSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: FirefamSpawnArgs) -> FirefamResult<FirefamSpawnOk> {
        let FirefamSpawnArgs {
            mut config,
            installation_id,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| FirefamErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            firefam_models_manager::manager::RefreshStrategy::Offline
        } else {
            firefam_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "firefam_spawn")
                        .await
                }
                None => None,
            }
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.firefam_home)
        })?;
        ...
        Ok(FirefamSpawnOk { firefam, thread_id })
    }
    ...
    pub async fn submit_with_id(&self, mut sub: Submission) -> FirefamResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| FirefamErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: firefam_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
    ...
    pub async fn shutdown_and_wait(&self) -> FirefamResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(FirefamErr::InternalAgentDied) => {}
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> FirefamResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-firefam-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &firefam_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn firefam_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn firefam_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn firefam_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.firefam_home().clone()
    }
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let firefam_home = updated.firefam_home.clone();
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_firefam_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &firefam_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| firefam_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&firefam_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                firefam_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn notify_stream_error(
        ...
        firefam_error: FirefamErr,
    ) {
        let additional_details = firefam_error.to_string();
        let firefam_error_info = FirefamErrorInfo::ResponseStreamDisconnected {
            http_status_code: firefam_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<firefam_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::FirefamHooks),
