...
fn external_migration_is_experimental_and_disabled_by_default() {
    ...
    assert_eq!(
        ...
        Some(
            "Show a startup prompt when Firefam detects migratable external agent config for this machine or project."
        )
    );
...
#[test]
fn firefam_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::FirefamHooks));
    assert_eq!(
        feature_for_key("firefam_hooks"),
        Some(Feature::FirefamHooks)
    );
}
