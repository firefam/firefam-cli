...
use firefam_agent_identity::decode_agent_identity_jwt;
use firefam_agent_identity::fetch_agent_identity_jwks;
use firefam_app_server_protocol::AuthMode;
use firefam_app_server_protocol::AuthMode as ApiAuthMode;
...
use firefam_client::FirefamHttpClient;
use firefam_config::types::AuthCredentialsStoreMode;
use firefam_protocol::account::PlanType as AccountPlanType;
use firefam_protocol::auth::PlanType as InternalPlanType;
use firefam_protocol::auth::RefreshTokenFailedError;
use firefam_protocol::auth::RefreshTokenFailedReason;
use serde_json::Value;
use thiserror::Error;

/// Authentication mechanism used by the current user.
#[derive(Debug, Clone)]
pub enum FirefamAuth {
    ApiKey(ApiKeyAuth),
    Chatgpt(ChatgptAuth),
...
impl PartialEq for FirefamAuth {
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "FIREFAM_REFRESH_TOKEN_URL_OVERRIDE";
pub const REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "FIREFAM_REVOKE_TOKEN_URL_OVERRIDE";
...
impl FirefamAuth {
    async fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::ApiKey {
            let Some(api_key) = auth_dot_json.firefamai_api_key.as_deref() else {
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(firefam_home.to_path_buf(), storage_mode);
    ...
    pub fn uses_firefam_backend(&self) -> bool {
        matches!(
            self,
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &FirefamHttpClient {
        &self.state.client
    }
}
...
pub const FIREFAM_ACCESS_TOKEN_ENV_VAR: &str = "FIREFAM_ACCESS_TOKEN";

pub fn read_firefamai_api_key_from_env() -> Option<String> {
...
pub fn read_firefam_api_key_from_env() -> Option<String> {
    read_non_empty_env_var(CODEX_API_KEY_ENV_VAR)
}

pub fn read_firefam_access_token_from_env() -> Option<String> {
    read_non_empty_env_var(FIREFAM_ACCESS_TOKEN_ENV_VAR)
}
...
/// Delete the auth.json file inside `firefam_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    firefam_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(firefam_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    let auth_dot_json = AuthDotJson {
        ...
        firefamai_api_key: Some(api_key.to_string()),
        tokens: None,
        last_refresh: None,
        agent_identity: None,
    };
    save_auth(firefam_home, &auth_dot_json, auth_credentials_store_mode)
}
...
pub async fn enforce_login_restrictions(config: &AuthConfig) -> std::io::Result<()> {
    ...
    if let Some(expected_account_ids) = config.forced_chatgpt_workspace_id.as_deref() {
        let chatgpt_account_id = match &auth {
            FirefamAuth::ApiKey(_) => return Ok(()),
            FirefamAuth::AgentIdentity(_) => auth.get_account_id(),
            FirefamAuth::Chatgpt(_) | FirefamAuth::ChatgptAuthTokens(_) => {
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(firefam_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(firefam_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(firefam_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(firefam_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

async fn load_auth(
    firefam_home: &Path,
    enable_firefam_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    chatgpt_base_url: Option<&str>,
) -> std::io::Result<Option<FirefamAuth>> {
    // API key via env var takes precedence over any other auth method.
    if enable_firefam_api_key_env && let Some(api_key) = read_firefam_api_key_from_env() {
        return Ok(Some(FirefamAuth::from_api_key(api_key.as_str())));
    }
    ...
    if let Some(auth_dot_json) = ephemeral_storage.load()? {
        let auth = FirefamAuth::from_auth_dot_json(
            firefam_home,
            auth_dot_json,
            AuthCredentialsStoreMode::Ephemeral,
            chatgpt_base_url,
    ...
    if let Some(agent_identity) = read_firefam_access_token_from_env() {
        return FirefamAuth::from_agent_identity_jwt(&agent_identity, chatgpt_base_url)
            .await
            .map(Some);
    }
...
impl AuthDotJson {
    ...
    fn resolved_mode(&self) -> ApiAuthMode {
        ...
        if self.firefamai_api_key.is_some() {
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(FirefamAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(FirefamAuth::get_account_id);
        let mode = if manager.has_external_api_key_auth()
            || cached_auth
                .as_ref()
                .is_some_and(FirefamAuth::is_external_chatgpt_tokens)
        {
    ...
    pub fn has_next(&self) -> bool {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(FirefamAuth::is_chatgpt_auth)
        {
            return false;
        }
...
/// hands out cloned `FirefamAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
/// `firefam_core::config::Config`, but this trait keeps `firefam-login` independent
/// from `firefam-core`.
...
impl Debug for AuthManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AuthManager")
            .field("firefam_home", &self.firefam_home)
            .field("inner", &self.inner)
            .field(
                "enable_firefam_api_key_env",
                &self.enable_firefam_api_key_env,
            )
            .field(
                "auth_credentials_store_mode",
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific FirefamAuth, for testing only.
    pub fn from_auth_for_testing(auth: FirefamAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            firefam_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific FirefamAuth and firefam home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: FirefamAuth, firefam_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            permanent_refresh_failure: None,
        };
    ...
    pub fn auth_cached(&self) -> Option<FirefamAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    pub fn refresh_failure_for_auth(&self, auth: &FirefamAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<FirefamAuth> {
    ...
    async fn reload_if_account_id_matches(
        ...
        let new_auth = self.load_auth_from_storage().await;
        let new_account_id = new_auth.as_ref().and_then(FirefamAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&FirefamAuth>, b: Option<&FirefamAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => match (a.api_auth_mode(), b.api_auth_mode()) {
                ...
                (ApiAuthMode::AgentIdentity, ApiAuthMode::AgentIdentity) => match (a, b) {
                    (FirefamAuth::AgentIdentity(a), FirefamAuth::AgentIdentity(b)) => {
                        a.record() == b.record()
                    }
                    _ => false,
                },
    ...
    fn auths_equal(a: Option<&FirefamAuth>, b: Option<&FirefamAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    async fn load_auth_from_storage(&self) -> Option<FirefamAuth> {
        load_auth(
            &self.firefam_home,
            self.enable_firefam_api_key_env,
            self.auth_credentials_store_mode,
    ...
    fn set_cached_auth(&self, new_auth: Option<FirefamAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn firefam_api_key_env_enabled(&self) -> bool {
        self.enable_firefam_api_key_env
    }
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<FirefamAuth> {
        ...
        match external_auth.resolve().await {
            Ok(Some(tokens)) => Some(FirefamAuth::from_api_key(&tokens.access_token)),
            Ok(None) => None,
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        if auth_before_reload
            .as_ref()
            .is_some_and(FirefamAuth::is_api_key_auth)
        {
            return Ok(());
        }
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(FirefamAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            FirefamAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            FirefamAuth::Chatgpt(chatgpt_auth) => {
            ...
            FirefamAuth::ApiKey(_) | FirefamAuth::AgentIdentity(_) => Ok(()),
        };
    ...
    pub async fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.firefam_home, self.auth_credentials_store_mode)?;
    ...
    pub async fn logout_with_revoke(&self) -> std::io::Result<bool> {
        ...
        let result = logout_all_stores(&self.firefam_home, self.auth_credentials_store_mode)?;
        // Always reload to clear any cached auth (even if file absent).
        self.reload().await;
        Ok(result)
    }
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        ...
        self.auth_cached().as_ref().map(FirefamAuth::api_auth_mode)
    }
    ...
    pub fn auth_mode(&self) -> Option<AuthMode> {
        ...
        self.auth_cached().as_ref().map(FirefamAuth::auth_mode)
    }

    pub fn current_auth_uses_firefam_backend(&self) -> bool {
        matches!(
            self.auth_mode(),
    ...
    fn is_stale_for_proactive_refresh(auth: &FirefamAuth) -> bool {
        let chatgpt_auth = match auth {
            FirefamAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
