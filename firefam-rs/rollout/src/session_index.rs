...
pub async fn append_thread_name(
    ...
    append_session_index_entry(firefam_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(firefam_home);
...
fn session_index_path(firefam_home: &Path) -> PathBuf {
    firefam_home.join(SESSION_INDEX_FILE)
}
