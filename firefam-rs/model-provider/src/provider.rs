...
use firefam_models_manager::manager::OpenAiModelsManager;
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "firefam-auto-review";
...
/// metadata used by the default FirefamAI-compatible implementation.
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    async fn api_provider(&self) -> firefam_protocol::error::Result<Provider> {
        let auth = self.auth().await;
        self.info()
            .to_api_provider(auth.as_ref().map(FirefamAuth::auth_mode))
    }

    /// Returns the provider base URL that will be used at request time.
    async fn runtime_base_url(&self) -> firefam_protocol::error::Result<Option<String>> {
        Ok(self.info().base_url.clone())
    }

    /// Returns the auth provider used to attach request credentials.
    async fn api_auth(&self) -> firefam_protocol::error::Result<SharedAuthProvider> {
        let auth = self.auth().await;
...
impl ModelProvider for ConfiguredModelProvider {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_firefamai_auth {
            self.auth_manager
                .as_ref()
                ...
                .map(|auth| match &auth {
                    FirefamAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    FirefamAuth::Chatgpt(_)
                    | FirefamAuth::ChatgptAuthTokens(_)
                    | FirefamAuth::AgentIdentity(_) => {
        ...
        Ok(ProviderAccountState {
            account,
            requires_firefamai_auth: self.info.requires_firefamai_auth,
        })
    }
...
mod tests {
    ...
    fn test_firefam_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "firefam-model-provider-test-{}",
            std::process::id()
        ))
    }
    ...
    #[test]
    fn create_model_provider_does_not_use_firefamai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("firefam-bedrock".to_string()),
                region: None,
            })),
            Some(AuthManager::from_auth_for_testing(
                FirefamAuth::from_api_key("openai-api-key"),
            )),
        );
    ...
    #[test]
    fn firefamai_provider_returns_unauthenticated_firefamai_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_firefamai_provider(/*base_url*/ None),
            /*auth_manager*/ None,
        );
    ...
    #[test]
    fn firefamai_provider_returns_api_key_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_firefamai_provider(/*base_url*/ None),
            Some(AuthManager::from_auth_for_testing(
                FirefamAuth::from_api_key("openai-api-key"),
    ...
    #[test]
    fn custom_non_firefamai_provider_returns_no_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo {
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_firefam_home(), /*config_model_catalog*/ None);
    ...
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model =
            firefam_models_manager::model_info::model_info_from_slug("custom-bedrock-model");
