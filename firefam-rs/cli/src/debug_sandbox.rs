...
#[cfg(target_os = "macos")]
use firefam_core::spawn::CODEX_SANDBOX_ENV_VAR;
...
use firefam_sandboxing::landlock::allow_network_for_proxy;
use firefam_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use firefam_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use firefam_sandboxing::seatbelt::create_seatbelt_command_args;
...
pub async fn run_command_under_seatbelt(
    ...
    _firefam_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    let network = network_proxy
        .as_ref()
        .map(firefam_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let firefam_linux_sandbox_exe = config
                .firefam_linux_sandbox_exe
                .expect("firefam-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use firefam_windows_sandbox::spawn_windows_sandbox_session_elevated;
    use firefam_windows_sandbox::spawn_windows_sandbox_session_legacy;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_firefam_home(
        cli_overrides,
        firefam_linux_sandbox_exe,
        options,
        /*firefam_home*/ None,
...
async fn load_debug_sandbox_config_with_firefam_home(
    ...
    // For legacy configs, `firefam sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            firefam_linux_sandbox_exe: firefam_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        firefam_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config(
    ...
    if let Some(firefam_home) = firefam_home {
        builder = builder
            .firefam_home(firefam_home.clone())
            .fallback_cwd(Some(firefam_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_firefam_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_firefam_home(
            ...
            Some(firefam_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = firefam_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
