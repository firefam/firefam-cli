...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the firefam tool call.
    #[serde(rename = "threadId")]
    pub thread_id: ThreadId,
    pub firefam_elicitation: String,
    pub firefam_mcp_tool_call_id: String,
    pub firefam_event_id: String,
    pub firefam_call_id: String,
    pub firefam_command: Vec<String>,
    pub firefam_cwd: PathBuf,
    pub firefam_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    firefam_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Firefam to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        firefam_mcp_tool_call_id: tool_call_id.clone(),
        firefam_event_id: event_id.clone(),
        firefam_call_id: call_id,
    ...
    {
        let firefam = firefam.clone();
        let approval_id = approval_id.clone();
        let event_id = event_id.clone();
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, firefam).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `firefam`.
