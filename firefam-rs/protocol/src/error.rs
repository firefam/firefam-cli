...
use crate::protocol::FirefamErrorInfo;
...
use chrono::Local;
use chrono::Utc;
use firefam_async_utils::CancelErr;
use firefam_utils_string::truncate_middle_chars;
use firefam_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, FirefamErr>;
...
pub enum FirefamErr {
    ...
    #[error(
        "Firefam ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `FirefamError::Spawn` variant.
    ...
    #[error("firefam-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for FirefamErr {
    fn from(_: CancelErr) -> Self {
        FirefamErr::TurnAborted
    }
}
...
impl FirefamErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            FirefamErr::TurnAborted
            | FirefamErr::Interrupted
            | FirefamErr::EnvVar(_)
            | FirefamErr::Fatal(_)
            | FirefamErr::UsageNotIncluded
            | FirefamErr::QuotaExceeded
            | FirefamErr::InvalidImageRequest()
            | FirefamErr::InvalidRequest(_)
            | FirefamErr::RefreshTokenFailed(_)
            | FirefamErr::UnsupportedOperation(_)
            | FirefamErr::Sandbox(_)
            | FirefamErr::LandlockSandboxExecutableNotProvided
            | FirefamErr::RetryLimit(_)
            | FirefamErr::ContextWindowExceeded
            | FirefamErr::ThreadNotFound(_)
            | FirefamErr::AgentLimitReached { .. }
            | FirefamErr::Spawn
            | FirefamErr::SessionConfiguredNotFirstEvent
            | FirefamErr::UsageLimitReached(_)
            | FirefamErr::ServerOverloaded
            | FirefamErr::CyberPolicy { .. } => false,
            FirefamErr::Stream(..)
            | FirefamErr::Timeout
            | FirefamErr::RequestTimeout
            | FirefamErr::UnexpectedStatus(_)
            | FirefamErr::ResponseStreamFailed(_)
            | FirefamErr::ConnectionFailed(_)
            | FirefamErr::InternalServerError
            | FirefamErr::InternalAgentDied
            | FirefamErr::Io(_)
            | FirefamErr::Json(_)
            | FirefamErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            FirefamErr::LandlockRuleset(_) | FirefamErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<FirefamErr>()` checks continue to compile
    ...
    pub fn to_firefam_protocol_error(&self) -> FirefamErrorInfo {
        match self {
            FirefamErr::ContextWindowExceeded => FirefamErrorInfo::ContextWindowExceeded,
            FirefamErr::UsageLimitReached(_)
            | FirefamErr::QuotaExceeded
            | FirefamErr::UsageNotIncluded => FirefamErrorInfo::UsageLimitExceeded,
            FirefamErr::ServerOverloaded => FirefamErrorInfo::ServerOverloaded,
            FirefamErr::CyberPolicy { .. } => FirefamErrorInfo::CyberPolicy,
            FirefamErr::RetryLimit(_) => FirefamErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            FirefamErr::ConnectionFailed(_) => FirefamErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            FirefamErr::ResponseStreamFailed(_) => {
                FirefamErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            FirefamErr::RefreshTokenFailed(_) => FirefamErrorInfo::Unauthorized,
            FirefamErr::SessionConfiguredNotFirstEvent
            | FirefamErr::InternalServerError
            | FirefamErr::InternalAgentDied => FirefamErrorInfo::InternalServerError,
            FirefamErr::UnsupportedOperation(_)
            | FirefamErr::ThreadNotFound(_)
            | FirefamErr::AgentLimitReached { .. } => FirefamErrorInfo::BadRequest,
            FirefamErr::Sandbox(_) => FirefamErrorInfo::SandboxError,
            _ => FirefamErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            firefam_error_info: Some(self.to_firefam_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            FirefamErr::RetryLimit(err) => Some(err.status),
            FirefamErr::UnexpectedStatus(err) => Some(err.status),
            FirefamErr::ConnectionFailed(err) => err.source.status(),
            FirefamErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("firefam")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/firefam/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/firefam/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &FirefamErr) -> String {
    let message = match e {
        FirefamErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        FirefamErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
