...
use firefam_api::RealtimeAudioFrame;
use firefam_api::RealtimeEvent;
use firefam_api::RealtimeEventParser;
use firefam_api::RealtimeSessionConfig;
use firefam_api::RealtimeSessionMode;
use firefam_api::RealtimeWebsocketClient;
use firefam_api::RealtimeWebsocketEvents;
use firefam_api::RealtimeWebsocketWriter;
...
use firefam_login::read_firefamai_api_key_from_env;
...
use firefam_protocol::protocol::RealtimeConversationClosedEvent;
use firefam_protocol::protocol::RealtimeConversationRealtimeEvent;
use firefam_protocol::protocol::RealtimeConversationSdpEvent;
use firefam_protocol::protocol::RealtimeConversationStartedEvent;
use firefam_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> FirefamResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> FirefamResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> FirefamResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(FirefamErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> FirefamResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| FirefamErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> FirefamResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> FirefamResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> FirefamResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> FirefamResult<PreparedRealtimeConversationStart> {
...
) -> FirefamResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> FirefamResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), FirefamErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(FirefamAuth::api_key) {
    ...
    if provider.is_firefamai()
        && let Some(api_key) = read_firefamai_api_key_from_env()
    {
        return Ok(api_key);
    }

    Err(FirefamErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    api_key: Option<&str>,
) -> FirefamResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            FirefamErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    firefam_error_info: FirefamErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
