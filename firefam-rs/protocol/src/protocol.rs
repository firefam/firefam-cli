...
use crate::firefamai_models::ReasoningEffort as ReasoningEffortConfig;
...
pub enum Op {
    ...
    /// Request to shut down firefam instance.
    Shutdown,
...
/// privileges of the agent (e.g. `.firefam`, `.git`, notably `.git/hooks`) under
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub firefam_error_info: Option<FirefamErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.firefam_error_info
            .as_ref()
            .is_none_or(FirefamErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Firefam),
...
impl From<crate::firefamai_models::TruncationPolicyConfig> for TruncationPolicy {
    fn from(config: crate::firefamai_models::TruncationPolicyConfig) -> Self {
        match config.mode {
            crate::firefamai_models::TruncationMode::Bytes => Self::Bytes(config.limit as usize),
            crate::firefamai_models::TruncationMode::Tokens => Self::Tokens(config.limit as usize),
        }
    }
}
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(firefam_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                firefam_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_firefam() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("firefam".to_string()).restriction_product(),
            Some(Product::Firefam)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Firefam])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Firefam]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_firefam.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_firefam.to_path_buf(),
