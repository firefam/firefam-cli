...
use firefam_analytics::CompactionImplementation;
use firefam_analytics::CompactionPhase;
use firefam_analytics::CompactionReason;
use firefam_analytics::CompactionStatus;
use firefam_analytics::CompactionStrategy;
use firefam_analytics::CompactionTrigger;
use firefam_analytics::FirefamCompactionEvent;
use firefam_analytics::now_unix_seconds;
use firefam_protocol::error::FirefamErr;
use firefam_protocol::error::Result as FirefamResult;
use firefam_protocol::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(FirefamErr::Interrupted) => {
                return Err(FirefamErr::Interrupted);
            }
            Err(e @ FirefamErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(FirefamCompactionEvent {
...
pub(crate) fn compaction_status_from_result<T>(result: &FirefamResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(FirefamErr::Interrupted | FirefamErr::TurnAborted) => CompactionStatus::Interrupted,
