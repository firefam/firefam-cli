...
use clap::ValueEnum;
use dirs::home_dir;
use firefam_state::LogQuery;
use firefam_state::LogRow;
...
async fn main() -> anyhow::Result<()> {
    ...
    let firefam_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(firefam_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let firefam_home = args
        .firefam_home
        .clone()
        .unwrap_or_else(default_firefam_home);
    Ok(firefam_state::logs_db_path(firefam_home.as_path()))
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["firefam-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["firefam-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["firefam-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["firefam-state-logs", "--level", "WARN"])
