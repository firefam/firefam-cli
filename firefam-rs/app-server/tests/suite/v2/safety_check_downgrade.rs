...
#[tokio::test]
async fn firefamai_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: firefam_app_server_protocol::TurnError {
                message: CYBER_POLICY_MESSAGE.to_string(),
                firefam_error_info: Some(FirefamErrorInfo::CyberPolicy),
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.firefam_error_info == Some(FirefamErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
