...
use firefam_client::ByteStream;
use firefam_client::StreamResponse;
...
const FIREFAMAI_MODEL_HEADER: &str = "openai-model";
...
impl ResponsesStreamEvent {
    ...
    pub fn response_model(&self) -> Option<String> {
        let response_headers_model = self
            ...
            .and_then(header_firefamai_model_value_from_json);
    ...
    pub(crate) fn model_verifications(&self) -> Option<Vec<ModelVerification>> {
        ...
        self.metadata
            .as_ref()
            .and_then(|metadata| metadata.get("firefamai_verification_recommendation"))
            .and_then(model_verifications_from_json_value)
    }
}

fn header_firefamai_model_value_from_json(value: &Value) -> Option<String> {
...
mod tests {
    ...
    use bytes::Bytes;
    use firefam_client::StreamResponse;
    use firefam_client::TransportError;
    ...
    async fn spawn_response_stream_ignores_model_verification_header() {
        let mut headers = HeaderMap::new();
        headers.insert(
            "firefamai-verification-recommendation",
    ...
    async fn process_sse_emits_model_verification_field() {
        let events = run_sse(vec![
            json!({
                ...
                "response_id": "resp-1",
                "metadata": {
                    "firefamai_verification_recommendation": [TRUSTED_ACCESS_FOR_CYBER_VERIFICATION]
                }
            }),
            json!({
                "type": "response.completed",
    ...
    fn responses_stream_event_model_verification_ignores_unknown_field() {
        let event = json!({
            "type": "response.metadata",
            "metadata": {
                "firefamai_verification_recommendation": ["unknown"]
            }
        });
        let event: ResponsesStreamEvent =
            serde_json::from_value(event).expect("expected event to deserialize");
