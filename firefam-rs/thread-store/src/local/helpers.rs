...
use firefam_rollout::ThreadItem;
...
pub(super) fn rollout_path_is_archived(firefam_home: &Path, path: &Path) -> bool {
    path.starts_with(firefam_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
