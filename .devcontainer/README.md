...
- `devcontainer.json` keeps the existing Firefam contributor setup for working on this repository.
- `devcontainer.secure.json` adds a customer-oriented profile with stricter outbound network controls.

## Firefam contributor profile

Use `devcontainer.json` when you are developing Firefam itself. This is the same lightweight arm64 container that already exists in the repo.

## Secure customer profile

Use `devcontainer.secure.json` when you want a stricter runtime profile for running Firefam inside a project container:

- installs the Firefam CLI plus common build tools
- installs bubblewrap in setuid mode for Firefam's Linux sandbox
- disables Docker's outer seccomp and AppArmor profiles so bubblewrap can construct Firefam's inner sandbox
...
This profile keeps the stricter networking isolated to the customer path instead of changing the default Firefam contributor container.
...
The secure profile's capability, seccomp, and AppArmor options are required when you want Firefam's bubblewrap sandbox to run inside Docker as the non-root devcontainer user. Without them, Docker's default runtime profile can block bubblewrap's namespace setup before Firefam's own seccomp filter is installed. This keeps the Docker relaxation explicit in the profile that is meant to run Firefam inside a project container, while the default contributor profile stays lightweight.
