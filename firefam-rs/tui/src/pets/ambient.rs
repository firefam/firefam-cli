//! Ambient terminal rendering for the Firefam companion.
//!
...
impl AmbientPet {
    ...
    /// FIREFAM_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_firefam_home(
            ...
            /*firefam_home*/ Some(firefam_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = firefam_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_firefam_app_vocabulary() {
