...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"FIREFAM_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"FIREFAM_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: firefam_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = firefam_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path { path: docs },
            access: firefam_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Special {
                value: firefam_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: firefam_protocol::permissions::FileSystemAccessMode::Write,
        },
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path {
                path: firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: firefam_protocol::permissions::FileSystemAccessMode::Read,
        },
...
fn windows_restricted_token_rejects_root_write_read_only_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Special {
                value: firefam_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: firefam_protocol::permissions::FileSystemAccessMode::Write,
        },
        firefam_protocol::permissions::FileSystemSandboxEntry {
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path { path: docs.clone() },
            access: firefam_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);

    // The legacy workspace-write root already protects top-level `.firefam`, so
...
fn windows_restricted_token_rejects_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path { path: blocked },
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path {
                path: firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        firefam_protocol::permissions::FileSystemSandboxEntry {
            path: firefam_protocol::permissions::FileSystemPath::Path {
                path: firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = firefam_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
    ...
    assert_eq!(
        select_process_exec_tool_sandbox_type(
            ...
            NetworkSandboxPolicy::Restricted,
            firefam_protocol::config_types::WindowsSandboxLevel::Disabled,
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: firefam_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };
