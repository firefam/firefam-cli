...
//!
//! # async fn example(state_db: std::sync::Arc<firefam_state::StateRuntime>) {
...
mod tests {
    ...
    fn temp_firefam_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("firefam-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let firefam_home = temp_firefam_home();
