...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Firefam sandbox network proxy.";
...
) -> firefam_exec_server::ExecEnvPolicy {
    firefam_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<firefam_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> firefam_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Firefam(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == firefam_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let firefam_home = crate::config::find_firefam_home().map_err(|err| {
                UnifiedExecError::create_process(format!(
            ...
            let spawned = match request.windows_sandbox_level {
                firefam_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    firefam_windows_sandbox::spawn_windows_sandbox_session_elevated(
                        policy_json.as_str(),
                ...
                firefam_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | firefam_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    firefam_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        policy_json.as_str(),
                        request.windows_sandbox_policy_cwd.as_path(),
                        firefam_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            firefam_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                firefam_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            firefam_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            FIREFAM_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Firefam(FirefamErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
