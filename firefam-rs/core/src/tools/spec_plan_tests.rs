...
use firefam_protocol::firefamai_models::ApplyPatchToolType;
use firefam_protocol::firefamai_models::ConfigShellToolType;
use firefam_protocol::firefamai_models::InputModality;
use firefam_protocol::firefamai_models::WebSearchToolType;
...
use firefam_tools::ResponsesApiTool;
use firefam_tools::ToolExposure;
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("firefam_app"),
        &["lookup".to_string()]
    );
    plain.assert_visible_lacks(&[
        firefam_code_mode::PUBLIC_TOOL_NAME,
        firefam_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("firefam_app"),
