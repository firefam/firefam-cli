"""Canonical Firefam package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "firefam-resources"
    path_dir = package_dir / "firefam-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    if inputs.firefam_command_runner_bin is not None:
        copy_executable(
            inputs.firefam_command_runner_bin,
            resources_dir / "firefam-command-runner.exe",
            is_windows=True,
        )

    if inputs.firefam_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.firefam_windows_sandbox_setup_bin,
            resources_dir / "firefam-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "firefam-resources",
        "pathDir": "firefam-path",
    }
    write_json(package_dir / "firefam-package.json", metadata)
...
def validate_package_dir(
    ...
    metadata_path = package_dir / "firefam-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: firefam-package.json")
    ...
    required_files = [
        ...
        Path("firefam-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if spec.is_linux:
        required_files.append(Path("firefam-resources") / "bwrap")
        executable_files.append(Path("firefam-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("firefam-resources") / "firefam-command-runner.exe",
                Path("firefam-resources") / "firefam-windows-sandbox-setup.exe",
            ]
        )
