...
use firefam_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use firefam_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
// Keep this comfortably above a normal sync attempt so we do not race another Firefam process.
...
pub fn curated_plugins_repo_path(firefam_home: &Path) -> PathBuf {
    firefam_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(firefam_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(firefam_home).as_path())
}

fn curated_plugins_sha_path(firefam_home: &Path) -> PathBuf {
    firefam_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_firefamai_plugins_repo(firefam_home: &Path) -> Result<String, String> {
...
fn sync_firefamai_plugins_repo_with_transport_overrides(
    firefam_home: &Path,
    git_binary: &str,
    api_base_url: &str,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    match sync_firefamai_plugins_repo_via_git(firefam_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_firefamai_plugins_repo_via_http(firefam_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(firefam_home) {
                        ...
                        let result = sync_firefamai_plugins_repo_via_backup_archive(
                            firefam_home,
                            backup_archive_api_url,
                        );
...
fn sync_firefamai_plugins_repo_via_git(
    firefam_home: &Path,
    git_binary: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(firefam_home);
    let sha_path = firefam_home.join(CURATED_PLUGINS_SHA_FILE);
...
fn sync_firefamai_plugins_repo_via_http(
    firefam_home: &Path,
    api_base_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(firefam_home);
...
fn sync_firefamai_plugins_repo_via_backup_archive(
    firefam_home: &Path,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(firefam_home);
    let sha_path = curated_plugins_sha_path(firefam_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(firefam_home: &Path) -> bool {
    curated_plugins_repo_path(firefam_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && firefam_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
async fn fetch_curated_repo_remote_sha(api_base_url: &str) -> Result<String, String> {
    ...
    let repo_url =
        format!("{api_base_url}/repos/{FIREFAMAI_PLUGINS_OWNER}/{FIREFAMAI_PLUGINS_REPO}");
