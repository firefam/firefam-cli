...
//!
//! If the keyring is not available or fails, we fall back to FIREFAM_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Firefam MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_firefam_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    struct TempFirefamHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempFirefamHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create FIREFAM_HOME temp dir");
            unsafe {
                std::env::set_var("FIREFAM_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
