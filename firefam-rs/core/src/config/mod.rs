...
use firefam_config::CloudRequirementsLoader;
use firefam_config::ConfigLayerSource;
use firefam_config::ConfigLayerStack;
use firefam_config::ConfigLayerStackOrdering;
use firefam_config::ConfigRequirements;
use firefam_config::ConfigRequirementsToml;
use firefam_config::ConstrainedWithSource;
use firefam_config::FeatureRequirementsToml;
use firefam_config::McpServerIdentity;
use firefam_config::McpServerRequirement;
use firefam_config::PluginRequirementsToml;
use firefam_config::ProfileV2Name;
use firefam_config::ResidencyRequirement;
use firefam_config::SandboxModeRequirement;
use firefam_config::Sourced;
use firefam_config::ThreadConfigLoader;
use firefam_config::config_toml::ConfigLockfileToml;
use firefam_config::config_toml::ConfigToml;
use firefam_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
...
use firefam_config::config_toml::ThreadStoreToml;
use firefam_config::config_toml::validate_model_providers;
use firefam_config::loader::load_config_layers_state;
use firefam_config::loader::project_trust_key;
use firefam_config::profile_toml::ConfigProfile;
use firefam_config::sandbox_mode_requirement_for_permission_profile;
use firefam_config::types::ApprovalsReviewer;
use firefam_config::types::AuthCredentialsStoreMode;
use firefam_config::types::History;
use firefam_config::types::McpServerConfig;
use firefam_config::types::McpServerDisabledReason;
...
use firefam_config::types::UriBasedFileOpener;
use firefam_config::types::WindowsSandboxModeToml;
use firefam_core_plugins::PluginsConfigInput;
use firefam_exec_server::ExecutorFileSystem;
use firefam_exec_server::LOCAL_FS;
use firefam_features::AppsMcpPathOverrideConfigToml;
use firefam_features::Feature;
use firefam_features::FeatureConfigSource;
use firefam_features::FeatureOverrides;
use firefam_features::FeatureToml;
use firefam_features::Features;
use firefam_features::FeaturesToml;
use firefam_features::MultiAgentV2ConfigToml;
use firefam_features::NetworkProxyConfigToml;
use firefam_git_utils::resolve_root_git_project_for_trust;
use firefam_login::AuthManagerConfig;
use firefam_mcp::McpConfig;
use firefam_memories_read::memory_root;
use firefam_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use firefam_model_provider_info::ModelProviderInfo;
use firefam_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use firefam_model_provider_info::built_in_model_providers;
use firefam_model_provider_info::merge_configured_model_providers;
use firefam_models_manager::ModelsManagerConfig;
use firefam_protocol::config_types::AltScreenMode;
...
use firefam_protocol::config_types::SandboxMode;
...
use firefam_protocol::config_types::WebSearchConfig;
...
use firefam_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_firefam_runtime;
...
#[cfg(test)]
mod schema;
pub use firefam_config::ConfigLoadOptions;
pub use firefam_config::Constrained;
pub use firefam_config::ConstraintError;
pub use firefam_config::ConstraintResult;
pub use firefam_config::LoaderOverrides;
pub use firefam_network_proxy::NetworkProxyAuditMetadata;
use firefam_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use firefam_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(firefam_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(firefam_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Firefam-only writable roots are intentionally excluded.
...
// when Firefam is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Firefam will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Firefam
    ...
    ///
    /// ```shell
    /// notify-send Firefam '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// Whether config lock replay ignores Firefam version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_firefam_version_mismatch: bool,
    ...
    /// Path to the current Firefam executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub firefam_self_exe: Option<PathBuf>,

    /// Path to the `firefam-linux-sandbox` executable. This must be set if
    /// [`firefam_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `firefam-linux-sandbox`.
    pub firefam_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `firefam-execve-wrapper` executable used for shell
    ...
    /// Base URL for requests to ChatGPT (as opposed to the FirefamAI API).
    ...
    pub otel: firefam_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn firefam_home(&self) -> PathBuf {
        self.firefam_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn firefam_home(mut self, firefam_home: PathBuf) -> Self {
        self.firefam_home = Some(firefam_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let firefam_home = match firefam_home {
            Some(firefam_home) => AbsolutePathBuf::from_absolute_path(firefam_home)?,
            None => find_firefam_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&firefam_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = firefam_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    firefam_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(firefam_config::io_error_from_config_error(
        ...
        {
            let allow_firefam_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_firefam_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_firefam_version_mismatch = allow_firefam_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &firefam_core_plugins::PluginsManager,
    ) -> McpConfig {
        ...
        McpConfig {
            ...
            firefam_home: self.firefam_home.to_path_buf(),
            ...
            firefam_linux_sandbox_exe: self.firefam_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.firefam_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_firefam_home(
            firefam_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Firefam home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_firefam_home(
        ...
        let cli_layer = firefam_config::build_cli_overrides_layer(&cli_overrides);
        firefam_config::merge_toml_values(&mut merged, &cli_layer);
        let firefam_home = AbsolutePathBuf::from_absolute_path_checked(firefam_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &firefam_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `firefam exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `firefam_self_exe`, `firefam_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, firefam_home).map_err(|e| {
...
pub fn set_default_oss_provider(firefam_home: &Path, provider: &str) -> std::io::Result<()> {
    firefam_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &firefam_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            firefam_protocol::permissions::FileSystemSandboxEntry {
                path: firefam_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: firefam_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            firefam_protocol::permissions::FileSystemSandboxEntry {
                path: firefam_protocol::permissions::FileSystemPath::Path { path },
                access: firefam_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&firefam_home);
        ...
        let (
            ...
            {
                // Keep Firefam runtime write access while storing the runtime
        ...
        let firefamai_base_url = cfg
            .firefamai_base_url
            .clone()
            .filter(|value| !value.is_empty());

        let model_providers =
            merge_configured_model_providers(built_in_model_providers(firefamai_base_url), cfg.model_providers)
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(firefam_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| firefam_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| firefam_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_firefam_runtime(
            &firefam_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_firefam_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_firefam_version_mismatch)
...
/// Returns the path to the folder where Firefam logs are stored. Does not verify
