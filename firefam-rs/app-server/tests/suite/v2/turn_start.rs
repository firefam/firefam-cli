...
use firefam_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use firefam_app_server::INVALID_PARAMS_ERROR_CODE;
use firefam_app_server_protocol::ByteRange;
...
use firefam_app_server_protocol::FileChangePatchUpdatedNotification;
...
use firefam_app_server_protocol::TextElement;
...
use firefam_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use firefam_core::test_support::all_model_presets;
...
const TEST_ORIGINATOR: &str = "firefam_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Firefam turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = firefam_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Firefam VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(firefam_home.path())?;
    let cache_path = firefam_home.path().join("models_cache.json");
    ...
    write_test_skill(firefam_home.path(), "alpha-skill")?;
    write_test_skill(firefam_home.path(), "beta-skill")?;
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "firefam_turn_event").await?;
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: firefam_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Firefam turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        firefam_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let firefam_home = tmp.path().to_path_buf();
    ...
    create_config_toml(
        firefam_home.as_path(),
        &server.uri(),
        "untrusted",
        &BTreeMap::default(),
    )?;

    let mut mcp = McpProcess::new(firefam_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(firefam_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(firefam_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(&firefam_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(firefam_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&firefam_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        firefam_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![firefam_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&firefam_home)?;
    let cache_path = firefam_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
fn write_test_skill(firefam_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = firefam_home.join("skills").join(name);
