...
use firefam_network_proxy::ConfigReloader;
use firefam_network_proxy::ConfigState;
use firefam_network_proxy::NetworkMode;
use firefam_network_proxy::NetworkProxy;
use firefam_network_proxy::NetworkProxyConfig;
use firefam_network_proxy::NetworkProxyConstraints;
use firefam_network_proxy::NetworkProxyState;
use firefam_network_proxy::build_config_state;
...
use firefam_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/firefam-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.firefam".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/firefam-readable");
    let unreadable = absolute_path("/tmp/firefam-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/firefam-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/firefam-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/firefam-proxy-use";
    let explicit_socket = "/tmp/firefam-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_firefam(/*managed_by_firefam*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_firefam_subpaths() {
    ...
    assert!(
        ...
        "expected explicit writable root .git/.firefam carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_firefam_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that writing a file to the folder containing .git and .firefam is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_firefam_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_firefam.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_firefam = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_firefam_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_firefam),
        "missing {expected_dot_firefam}: {args:#?}"
    );
