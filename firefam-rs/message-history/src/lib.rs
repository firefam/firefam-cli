...
//!
//! The history is stored at `~/.firefam/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.firefam`.
...
impl HistoryConfig {
    pub fn new(firefam_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            firefam_home: firefam_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.firefam_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.firefam/history.jsonl` and ensure the parent directory exists.
