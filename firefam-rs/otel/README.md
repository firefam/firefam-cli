...
`firefam-otel` is the OpenTelemetry integration crate for Firefam. It provides:

- Provider wiring for log/trace/metric exporters (`firefam_otel::OtelProvider`
  and `firefam_otel::provider`).
- Session-scoped business event emission via `firefam_otel::SessionTelemetry`.
- Low-level metrics APIs via `firefam_otel::metrics`.
- Trace-context helpers via `firefam_otel::trace_context` and crate-root re-exports.
...
```rust
use firefam_otel::config::OtelExporter;
use firefam_otel::config::OtelHttpProtocol;
use firefam_otel::config::OtelSettings;
use firefam_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "firefam-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    firefam_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Firefam
...
Firefam-specific session events. Rich session/business events should go through
...
`firefam-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Firefam-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use firefam_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("firefam.session_started", 1, &[("source", "tui")])?;
metrics.histogram("firefam.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("firefam.turns", 1, &[("model", "gpt-5.1")])?;
