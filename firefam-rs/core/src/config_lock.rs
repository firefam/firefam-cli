...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_firefam_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_firefam_version_mismatch
        && expected_lock.firefam_version != actual_lock.firefam_version
    {
        return Err(config_lock_error(format!(
            "config lock Firefam version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_firefam_version_mismatch=true to ignore this",
            expected_lock.firefam_version, actual_lock.firefam_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_firefam_version_mismatch {
        lockfile.firefam_version.clear();
    }
    lockfile
}
