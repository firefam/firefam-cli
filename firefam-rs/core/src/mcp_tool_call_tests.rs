...
use firefam_config::types::AppConfig;
use firefam_config::types::AppToolConfig;
use firefam_config::types::AppToolsConfig;
use firefam_config::types::ApprovalsReviewer;
use firefam_config::types::AppsConfigToml;
...
use firefam_rollout_trace::ToolDispatchInvocation;
use firefam_rollout_trace::ToolDispatchPayload;
use firefam_rollout_trace::ToolDispatchRequester;
...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        firefam_apps_meta: None,
        firefamai_file_input_params: None,
    }
}
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                firefam_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.firefam_home)
        .expect("create firefam home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.firefam_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = firefam_hooks::list_hooks(HooksConfig {
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        firefam_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_firefam_turn_started(turn_context.sub_id.as_str());
...
#[test]
fn firefamai_file_params_are_only_honored_for_firefam_apps() {
    ...
    assert_eq!(
        firefamai_file_input_params_for_server(FIREFAM_APPS_MCP_SERVER_NAME, meta),
        Some(vec!["file".to_string()])
    );
    assert_eq!(
        firefamai_file_input_params_for_server("minimaltest", meta),
        None
    );
}
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("firefam.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("firefam.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("firefam.mcp.target.id=")
            && !invalid_logs.contains("firefam.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("firefam.mcp.target.id=")
            && !missing_logs.contains("firefam.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("firefam.mcp.target.id=")
            && !no_meta_logs.contains("firefam.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("firefam.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn firefam_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        FIREFAM_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_firefam_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        FIREFAM_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn firefam_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: FIREFAM_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn firefam_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: FIREFAM_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_FIREFAM_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_FIREFAM_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[tokio::test]
async fn firefam_apps_tool_call_request_meta_includes_turn_metadata_and_firefam_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        firefam_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_firefam_apps metadata should be an object"),
        ),
        firefamai_file_input_params: None,
    };
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_FIREFAM_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_FIREFAM_APPS_META_KEY: {
...
#[tokio::test]
async fn firefam_apps_tool_call_request_meta_includes_call_id_without_existing_firefam_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
...
fn firefam_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn firefam_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn install_host_owned_firefam_apps_manager(session: &Session, turn_context: &TurnContext) {
    ...
    let (manager, _cancel_token) = firefam_mcp::McpConnectionManager::new(
        &HashMap::new(),
        ...
        firefam_mcp::McpRuntimeEnvironment::new(
            Some(environment),
        ...
        turn_context.config.firefam_home.to_path_buf(),
        firefam_mcp::firefam_apps_tools_cache_key(auth.as_ref()),
        /*host_owned_firefam_apps_enabled*/ true,
        rmcp::model::ElicitationCapability::default(),
        firefam_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
...
#[tokio::test]
async fn firefam_apps_auth_elicitation_feature_disabled_returns_original_result() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_firefam_apps_manager(&session, &turn_context).await;
    let result = firefam_apps_auth_failure_result();
    let metadata = firefam_apps_auth_failure_metadata();

    let returned = maybe_request_firefam_apps_auth_elicitation(
        &session,
        &turn_context,
        "call_123",
        FIREFAM_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn firefam_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
...
#[tokio::test]
async fn firefam_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_firefam_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn firefam_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_firefam_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn firefam_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_firefam_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_firefam_apps_auth_elicitation(
                &session,
                &turn_context,
                "call_123",
                FIREFAM_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(request.server_name, FIREFAM_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        firefam_protocol::mcp::RequestId::String("firefam_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        firefam_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            FIREFAM_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("firefam_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: firefam_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_firefam_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            FIREFAM_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_firefam_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");
    let config = ConfigBuilder::default()
        .firefam_home(tmp.path().to_path_buf())
        .build()
        .await
        .expect("load config");

    persist_firefam_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(firefam_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .firefam_home(firefam_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(firefam_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    ConfigEditsBuilder::new(&firefam_home)
        .set_project_trust_level(
            project_dir.path(),
            firefam_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_firefam_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            firefam_login::FirefamAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
