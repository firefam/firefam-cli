...
use firefam_app_server_protocol::SkillInterface;
...
pub const REMOTE_GLOBAL_MARKETPLACE_NAME: &str = "firefamai-curated-remote";
...
pub const REMOTE_GLOBAL_MARKETPLACE_DISPLAY_NAME: &str = "FirefamAI Curated Remote";
...
const FIREFAMAI_CURATED_REMOTE_COLLECTION_KEY: &str = "vertical";
...
pub async fn fetch_firefamai_curated_remote_collection_marketplace(
    config: &RemotePluginServiceConfig,
    auth: Option<&FirefamAuth>,
    ...
    let (directory_plugins, installed_plugins) = tokio::try_join!(
        fetch_directory_plugins_for_scope_with_collection(
            ...
            auth,
            scope,
            FIREFAMAI_CURATED_REMOTE_COLLECTION_KEY,
        ),
        fetch_installed_plugins_for_scope(config, auth, scope),
    )?;
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(firefam_home.clone())
    ...
    let legacy_remote_plugin_cache_root = firefam_home
...
fn ensure_chatgpt_auth(
    auth: Option<&FirefamAuth>,
) -> Result<&FirefamAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    Ok(request
        ...
        .headers(firefam_model_provider::auth_provider_from_auth(auth).to_auth_headers()))
}
