...
Runtime orchestration for Phase 1 and Phase 2 still lives in `firefam-core` under
`firefam-rs/core/src/memories/`.

## Crates

- `firefam-rs/memories/read` (`firefam-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `firefam-rs/memories/mcp` (`firefam-memories-mcp`) owns the read-only memory
  filesystem MCP server implementation.
- `firefam-rs/memories/write` (`firefam-memories-write`) owns the write path:
...
- In `firefam`, edit those undated template files in place.
- The dated snapshot-copy workflow is used in the separate `firefamai/project/agent_memory/write` harness repo, not here.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.firefam/memories/.git` by `firefam-git-utils`
