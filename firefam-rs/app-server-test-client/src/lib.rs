...
use firefam_app_server_protocol::DynamicToolSpec;
...
use firefam_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "firefam-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $FIREFAM_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Firefam app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Firefam", version, about = "Bootstrap Firefam app-server", long_about = None)]
struct Cli {
    /// Path to the `firefam` CLI binary. When set, requests use stdio by
    /// spawning `firefam app-server` as a child process.
    #[arg(long, env = "FIREFAM_BIN", global = true)]
    firefam_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--firefam-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "FIREFAM_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `firefam` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `firefam app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/firefam-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `firefam app-server --listen`.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "FIREFAM_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let firefam_bin = firefam_bin.unwrap_or_else(|| PathBuf::from("firefam"));
            serve(&firefam_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(firefam_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(firefam_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(firefam_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(firefam_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if firefam_bin.is_some() && url.is_some() {
        bail!("--firefam-bin and --url are mutually exclusive");
    }
    if let Some(firefam_bin) = firefam_bin {
        return Ok(Endpoint::SpawnFirefam(firefam_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if firefam_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --firefam-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(firefam_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(firefam_bin);
        if let Some(firefam_bin_parent) = firefam_bin.parent() {
            let mut path = OsString::from(firefam_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", firefam_bin.display()))?;
...
fn serve(firefam_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/firefam-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,firefam_=trace {}",
        shell_quote(&firefam_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started firefam app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnFirefam(firefam_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut FirefamClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = FirefamClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = FirefamClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (firefam_bin, url) {
        (Some(_), Some(_)) => bail!("--firefam-bin and --url are mutually exclusive"),
        (Some(firefam_bin), None) => {
            let server = BackgroundAppServer::spawn(&firefam_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve firefam-app-server-test-client binary path")?;
...
impl FirefamClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnFirefam(firefam_bin) => Self::spawn_stdio(firefam_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(firefam_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let firefam_bin_display = firefam_bin.display();
        let mut cmd = Command::new(firefam_bin);
        if let Some(firefam_bin_parent) = firefam_bin.parent() {
            let mut path = OsString::from(firefam_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut firefam_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{firefam_bin_display}` app-server"))?;

        let stdin = firefam_app_server
            .stdin
            .take()
            .context("firefam app-server stdin unavailable")?;
        let stdout = firefam_app_server
            .stdout
            .take()
            .context("firefam app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: firefam_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `firefam-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "firefam-toy-app-server".to_string(),
                    title: Some("Firefam Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: firefam_app_server_protocol::LoginAccountParams::Chatgpt {
                firefam_streamlined_login: false,
            },
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: firefam_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to firefam app-server")?;
                    return Ok(());
                }
                bail!("firefam app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from firefam app-server")?;
                if bytes == 0 {
                    bail!("firefam app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = firefam_core::otel_init::build_provider(
            &config,
...
impl Drop for FirefamClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[firefam app-server exited: {status}]");
            return;
        }
