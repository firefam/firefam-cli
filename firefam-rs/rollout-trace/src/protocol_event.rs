//! Mapping from Firefam protocol events into raw rollout-trace events.
//!
...
//! `firefam-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `firefam-rollout-trace`.
//!
...
use firefam_protocol::protocol::PatchApplyStatus;
use firefam_protocol::protocol::TurnAbortReason;
use serde::Serialize;

use crate::AgentThreadId;
use crate::ExecutionStatus;
use crate::FirefamTurnId;
use crate::RawTraceEventPayload;

pub(crate) struct FirefamTurnTraceEvent {
    pub context_turn_id: FirefamTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn firefam_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<FirefamTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let firefam_turn_id = event.turn_id.clone();
            Some(FirefamTurnTraceEvent {
                context_turn_id: firefam_turn_id.clone(),
                payload: RawTraceEventPayload::FirefamTurnStarted {
                    firefam_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(FirefamTurnTraceEvent {
                context_turn_id: firefam_turn_id.clone(),
                payload: RawTraceEventPayload::FirefamTurnEnded {
                    firefam_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a firefam_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a firefam_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a firefam_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a firefam_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a firefam_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a firefam_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a firefam_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a firefam_protocol::protocol::CollabCloseEndEvent),
}
