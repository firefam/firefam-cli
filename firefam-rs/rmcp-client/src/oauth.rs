...
//!
//! If the keyring is not available or fails, we fall back to AGENTS_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempFirefamHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create AGENTS_HOME temp dir");
            unsafe {
                std::env::set_var("AGENTS_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
