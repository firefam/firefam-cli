//! Default Firefam HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`FirefamHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`firefam_login::default_client`] from other crates in this
//! workspace.

use firefam_client::BuildCustomCaTransportError;
use firefam_client::FirefamHttpClient;
pub use firefam_client::FirefamRequestBuilder;
...
/// Parenthesis will be added by Firefam. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "firefam_cli_rs";
pub const FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-firefamai-internal-firefam-residency";

pub use firefam_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "firefam-tui"
        || originator_value == "firefam_vscode"
        || originator_value.starts_with("Firefam ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "firefam_atlas" || originator_value == "firefam_chatgpt_desktop"
}

pub fn get_firefam_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Firefam user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Firefam user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Firefam originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> FirefamHttpClient {
    let inner = build_reqwest_client();
    FirefamHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Firefam HTTP traffic.
///
/// This starts from the standard Firefam user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `FIREFAM_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary Firefam HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_firefam_user_agent()) {
