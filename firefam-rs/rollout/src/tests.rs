...
async fn insert_state_db_thread(
    ...
    let runtime = firefam_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let firefamai_id = Uuid::from_u128(1);
    ...
    let firefamai_id_str = firefamai_id.to_string();
    let none_id_str = none_id.to_string();
    let firefamai_filter = provider_vec(&["openai"]);
    let firefamai_sessions = get_threads(
        home,
        /*page_size*/ 10,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        NO_SOURCE_FILTER,
        Some(firefamai_filter.as_slice()),
        /*cwd_filters*/ None,
        "openai",
    )
    .await?;
    assert_eq!(firefamai_sessions.items.len(), 2);
    let firefamai_ids: Vec<_> = firefamai_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(firefamai_ids.contains(&firefamai_id_str));
    assert!(firefamai_ids.contains(&none_id_str));
