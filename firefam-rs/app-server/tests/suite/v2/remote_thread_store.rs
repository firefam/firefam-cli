...
//! the temporary `firefam_home` must not contain rollout session files or sqlite
...
use firefam_app_server::in_process::InProcessServerEvent;
...
async fn thread_start_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under firefam_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(firefam_home.path(), &server.uri(), &store_id)?;
    ...
    assert_no_local_persistence_artifacts(firefam_home.path())?;

    Ok(())
}

fn assert_no_local_persistence_artifacts(firefam_home: &Path) -> Result<()> {
    ...
    let mut entries = firefam_home_entries(firefam_home)?;
    // Bazel test runs may initialize shell snapshot storage under firefam_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in firefam_home"
    );

    Ok(())
}

fn firefam_home_entries(firefam_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(firefam_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
