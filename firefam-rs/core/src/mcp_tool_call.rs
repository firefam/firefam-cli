...
use crate::mcp_firefamai_file::rewrite_mcp_tool_arguments_for_firefamai_files;
...
use firefam_analytics::AppInvocation;
use firefam_analytics::InvocationType;
use firefam_analytics::build_track_events_context;
use firefam_app_server_protocol::ConfigLayerSource;
use firefam_app_server_protocol::McpElicitationObjectType;
...
use firefam_hooks::PermissionRequestDecision;
use firefam_mcp::FIREFAM_APPS_MCP_SERVER_NAME;
use firefam_mcp::MCP_TOOL_FIREFAM_APPS_META_KEY;
use firefam_mcp::McpPermissionPromptAutoApproveContext;
use firefam_mcp::SandboxState;
use firefam_mcp::auth_elicitation_completed_result;
use firefam_mcp::build_auth_elicitation_plan;
use firefam_mcp::declared_firefamai_file_input_param_names;
use firefam_mcp::mcp_permission_prompt_is_auto_approved;
use firefam_otel::sanitize_metric_tag_value;
use firefam_protocol::firefamai_models::InputModality;
use firefam_protocol::items::McpToolCallError;
use firefam_protocol::items::McpToolCallItem;
use firefam_protocol::items::McpToolCallStatus;
use firefam_protocol::items::TurnItem;
use firefam_protocol::mcp::CallToolResult;
use firefam_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use firefam_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use firefam_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use firefam_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use firefam_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use firefam_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use firefam_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use firefam_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use firefam_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use firefam_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use firefam_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use firefam_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use firefam_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use firefam_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
use firefam_protocol::protocol::AskForApproval;
use firefam_protocol::protocol::McpInvocation;
...
use firefam_protocol::request_user_input::RequestUserInputQuestion;
use firefam_protocol::request_user_input::RequestUserInputQuestionOption;
...
use firefam_utils_output_truncation::truncate_text;
...
const MCP_CALL_COUNT_METRIC: &str = "firefam.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "firefam.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "firefam/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "firefam.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "firefam.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == FIREFAM_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == FIREFAM_APPS_MCP_SERVER_NAME {
    ...
    if server == FIREFAM_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    let rewrite = rewrite_mcp_tool_arguments_for_firefamai_files(
        sess,
        turn_context,
        arguments_value.clone(),
        metadata.and_then(|metadata| metadata.firefamai_file_input_params.as_deref()),
    )
    .await;
    ...
    .await;
    maybe_track_firefam_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        firefam.mcp.target.id = Empty,
        firefam.mcp.server_user_flow.triggered = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_firefam_apps_auth_elicitation(
        sess,
        turn_context,
        call_id,
        &invocation.server,
...
async fn maybe_request_firefam_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    if !sess
        ...
        .read()
        .await
        .is_host_owned_firefam_apps_server(server)
    {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        firefam_connectors::metadata::connector_install_url(
    ...
    let params = McpServerElicitationRequestParams {
        ...
        server_name: FIREFAM_APPS_MCP_SERVER_NAME.to_string(),
    ...
    refresh_firefam_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

#[expect(
    clippy::await_holding_invalid_type,
    reason = "Firefam Apps cache refresh reads through the session-owned manager guard"
)]
async fn refresh_firefam_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    let mcp_tools_result = {
        let manager = sess.services.mcp_connection_manager.read().await;
        manager.hard_refresh_firefam_apps_tools_cache().await
    };
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        firefam_linux_sandbox_exe: turn_context.firefam_linux_sandbox_exe.clone(),
        #[allow(deprecated)]
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                firefam_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_firefam_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != FIREFAM_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    firefam_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    firefamai_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_FIREFAMAI_OUTPUT_TEMPLATE_META_KEY: &str = "openai/outputTemplate";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, firefam_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_FIREFAM_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == FIREFAM_APPS_MCP_SERVER_NAME {
        let mut firefam_apps_meta = metadata
            .and_then(|metadata| metadata.firefam_apps_meta.clone())
            .unwrap_or_default();
        firefam_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_FIREFAM_APPS_META_KEY.to_string(),
            serde_json::Value::Object(firefam_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__firefam_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == FIREFAM_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == FIREFAM_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        firefam_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_FIREFAM_APPS_META_KEY))
            .and_then(serde_json::Value::as_object)
            .cloned(),
        // Disallow custom MCPs from uploading files via fileParams.
        firefamai_file_input_params: firefamai_file_input_params_for_server(
            server,
...
fn firefamai_file_input_params_for_server(
    server: &str,
    meta: Option<&serde_json::Map<String, serde_json::Value>>,
) -> Option<Vec<String>> {
    (server == FIREFAM_APPS_MCP_SERVER_NAME)
        .then_some(declared_firefamai_file_input_param_names(meta))
...
fn get_mcp_app_resource_uri(
    ...
    meta.and_then(|meta| {
        meta.get("ui")
            ...
            .or_else(|| {
                meta.get(MCP_TOOL_FIREFAMAI_OUTPUT_TEMPLATE_META_KEY)
                    .and_then(serde_json::Value::as_str)
            })
            .map(str::to_string)
    })
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == FIREFAM_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_firefam_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_firefam_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, firefam_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
    ...
    config
        ...
        .find_map(|layer| {
            ...
            let servers = layer
                ...
                .cloned()
                .and_then(|value| {
                    HashMap::<String, firefam_config::types::McpServerConfig>::deserialize(value)
                        .ok()
                })?;
