...
pub enum Command {
    /// Submit a new Firefam Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Firefam Cloud task.
    Status(StatusCommand),
    /// List Firefam Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Firefam Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Firefam Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Firefam Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `firefam cloud` to browse).
    ...
    /// Git branch to run in Firefam Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Firefam Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Firefam Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Firefam Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
