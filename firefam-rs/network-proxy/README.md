# firefam-network-proxy

`firefam-network-proxy` is Firefam's local network policy enforcement proxy. It runs:
...
`firefam-network-proxy` reads from Firefam's merged `config.toml` (via `firefam-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $FIREFAM_HOME/proxy/ (ca.pem + ca.key).
...
# Use exact hosts or scoped wildcards like `*.firefam.ai` or `**.firefam.ai`.
...
```bash
cargo run -p firefam-network-proxy --
```
...
`firefam-network-proxy` can be embedded as a library with a thin API:

```rust
use firefam_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `firefam-network-proxy` is embedded in managed Firefam runtime, policy decisions emit structured
OTEL-compatible events with `target=firefam_otel.network_proxy`.

Event name:

- `firefam.network_proxy.policy_decision`
...
This section documents the protections implemented by `firefam-network-proxy`, and the boundaries of
