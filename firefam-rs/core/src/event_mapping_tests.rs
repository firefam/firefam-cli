...
fn skips_local_image_label_text() {
    ...
    let label = firefam_protocol::models::local_image_open_tag_text(/*label_number*/ 1);
...
fn skips_unnamed_image_label_text() {
    let image_url = "data:image/png;base64,abc".to_string();
    let label = firefam_protocol::models::image_open_tag_text();
    let user_text = "Please review this image.".to_string();
...
fn parses_agent_message() {
    let item = ResponseItem::Message {
        ...
        content: vec![ContentItem::OutputText {
            text: "Hello from Firefam".to_string(),
        }],
        phase: None,
    };
    ...
    match turn_item {
        TurnItem::AgentMessage(message) => {
            ...
            assert_eq!(text, "Hello from Firefam");
        }
        other => panic!("expected TurnItem::AgentMessage, got {other:?}"),
    }
}
