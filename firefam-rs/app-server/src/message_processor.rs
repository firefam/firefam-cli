...
use firefam_app_server_protocol::AuthMode as LoginAuthMode;
use firefam_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
use firefam_app_server_protocol::ExperimentalApi;
...
use firefam_app_server_protocol::experimental_required_message;
...
use firefam_login::auth::ExternalAuth;
use firefam_login::auth::ExternalAuthRefreshContext;
use firefam_login::auth::ExternalAuthRefreshReason;
use firefam_login::auth::ExternalAuthTokens;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: FirefamFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let firefam_request = serde_json::to_value(&request)
                    .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    .and_then(|request_json| {
                        serde_json::from_value::<ClientRequest>(request_json)
                            .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    });
                let result = match firefam_request {
                    Ok(firefam_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = firefam_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = firefam_request.experimental_reason()
        ...
        let serialization_scope = firefam_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: firefam_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match firefam_request
        {
