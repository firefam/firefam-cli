...
use firefam_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestFirefam, ResponseMock)> {
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__firefam_apps__calendar_timezone_option_99"
);
if (!tool) {
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "firefam_app_hidden_dynamic_tool");
const out = await tools.firefam_app_hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.firefam, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.firefam, |event| match event {
        EventMsg::DynamicToolCallRequest(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    assert_eq!(request.namespace.as_deref(), Some("firefam_app"));
    ...
    test.firefam
        ...
        .await?;
    wait_for_event(&test.firefam, |event| match event {
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "firefam_app_hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("A hidden dynamic tool.")
                    ...
                    && description.contains("firefam_app_hidden_dynamic_tool(args:")
            })
    );
