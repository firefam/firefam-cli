...
use firefam_login::default_client::USER_AGENT_SUFFIX;
...
use crate::firefam_tool_config::FirefamToolCallParam;
use crate::firefam_tool_config::FirefamToolCallReplyParam;
use crate::firefam_tool_config::create_tool_for_firefam_tool_call_param;
use crate::firefam_tool_config::create_tool_for_firefam_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_firefam_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            firefam_core::thread_store_from_config(config.as_ref(), state_db.clone()),
            state_db.clone(),
            installation_id,
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_firefam_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "firefam-mcp-server".to_string(),
            title: Some("Firefam".to_string()),
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_firefam_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_firefam_tool_call_param(),
                create_tool_for_firefam_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "firefam" => self.handle_tool_call_firefam(id, arguments).await,
            "firefam-reply" => {
                self.handle_tool_call_firefam_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_firefam(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<FirefamToolCallParam>(json_val) {
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for firefam tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        let running_requests_id_to_firefam_uuid = self.running_requests_id_to_firefam_uuid.clone();
        ...
        task::spawn(async move {
            ...
            crate::firefam_tool_runner::run_firefam_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_firefam_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_firefam_session_reply(
        &self,
        ...
        // parse arguments
        let firefam_tool_call_reply_param: FirefamToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<FirefamToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for firefam-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for firefam-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match firefam_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let firefam = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::firefam_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = firefam_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_firefam_uuid = running_requests_id_to_firefam_uuid.clone();

            async move {
                crate::firefam_tool_runner::run_firefam_tool_session_reply(
                    thread_id,
                    firefam,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_firefam_uuid.lock().await;
        ...
        let firefam_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        if let Err(e) = firefam_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: firefam_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
        ...
        self.running_requests_id_to_firefam_uuid
            .lock()
            .await
            .remove(&request_id);
    }
