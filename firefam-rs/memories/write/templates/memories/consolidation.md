...
The folder `{{ memory_root }}/` is a git repository managed by Firefam. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by Firefam for
...
- Retrieval bias: preserve distinctive nouns and verbatim strings that a future grep/search
  ...
  `firefam.ai/v1/files`, `FirefamAI Internal Slack`, etc.).
...
- Typical workflows and tools (how they like to work, how they use Firefam/agents, preferred formats)
