...
use core_test_support::test_firefam::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestFirefamHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestFirefamBuilder) -> TestFirefamBuilder,
) -> Result<TestFirefamHarness> {
    let builder = configure(test_firefam());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestFirefamHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestFirefamHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_firefam_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let firefam_linux_sandbox_exe = harness
        .test()
        .config
        .firefam_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include firefam-linux-sandbox helper");
    assert_eq!(
        firefam_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(FIREFAM_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&firefam_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
