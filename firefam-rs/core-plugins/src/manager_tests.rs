...
use crate::test_support::write_firefamai_curated_marketplace;
use firefam_app_server_protocol::ConfigLayerSource;
use firefam_config::AppToolApproval;
...
use firefam_config::McpServerConfig;
use firefam_config::McpServerOAuthConfig;
use firefam_config::McpServerToolConfig;
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "firefam-test@example.com"]);
    run_git(repo, &["config", "user.name", "Firefam Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(config_toml: &str, firefam_home: &Path) -> PluginLoadOutcome {
    write_file(&firefam_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(firefam_home, firefam_home).await;
    PluginsManager::new(firefam_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(firefam_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(firefam_home, cwd).await
}

fn remote_installed_linear_plugin() -> RemoteInstalledPlugin {
    RemoteInstalledPlugin {
        marketplace_name: "firefamai-curated-remote".to_string(),
        id: "plugins~Plugin_linear".to_string(),
        name: "linear".to_string(),
        enabled: true,
        install_policy: firefam_app_server_protocol::PluginInstallPolicy::Available,
        auth_policy: firefam_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: firefam_app_server_protocol::PluginAvailability::Available,
        interface: None,
        keywords: Vec::new(),
    }
}
...
"#;
    ...
    let outcome = load_plugins_from_config(config_toml, firefam_home.path()).await;
...
"#,
    ...
    let config = load_config(firefam_home.path(), firefam_home.path()).await;
    let manager = PluginsManager::new(firefam_home.path().to_path_buf());
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = firefam_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = firefam_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(firefam_app_server_protocol::PluginInterface {
    ...
    assert_eq!(marketplaces[0].name, "firefamai-curated-remote");
    assert_eq!(marketplaces[0].display_name, "FirefamAI Curated Remote");
    assert_eq!(marketplaces[0].plugins.len(), 1);
    let plugin = &marketplaces[0].plugins[0];
    assert_eq!(plugin.id, "linear@firefamai-curated-remote");
    ...
    assert_eq!(
        ...
        firefam_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        firefam_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let firefam_home = TempDir::new().unwrap();
    let plugin_a_root = firefam_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = firefam_home
        .path()
        .join("plugins/cache")
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(firefam_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![firefam_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![firefam_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(firefam_home.path(), firefam_home.path()).await;
    let outcome = PluginsManager::new(firefam_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugins_for_config_reloads_when_plugin_hooks_enablement_changes() {
  ...
  "hooks": {
    ...
    let config_without_plugin_hooks = load_config(firefam_home.path(), firefam_home.path()).await;
    ...
    let config_with_plugin_hooks = load_config(firefam_home.path(), firefam_home.path()).await;
...
#[tokio::test]
async fn install_firefamai_curated_plugin_uses_short_sha_cache_version() {
    let tmp = tempfile::tempdir().unwrap();
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/firefamai-curated/slack/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
    ));
    assert_eq!(
        result,
        PluginInstallOutcome {
            plugin_id: PluginId::new(
                "slack".to_string(),
                FIREFAMAI_CURATED_MARKETPLACE_NAME.to_string()
            )
            .unwrap(),
...
async fn sync_plugins_from_remote_reconciles_cache_and_config() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_firefamai_curated_marketplace(&curated_root, &["linear", "gmail", "calendar"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    write_plugin(
        &tmp.path().join("plugins/cache/firefamai-curated"),
        "linear/local",
        "linear",
    );
    write_plugin(
...
[plugins."linear@firefamai-curated"]
enabled = false

[plugins."gmail@firefamai-curated"]
enabled = false

[plugins."calendar@firefamai-curated"]
enabled = true
"#,
    );
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"linear","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":true},
  {"id":"2","name":"gmail","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":false}
]"#,
        ))
        .mount(&server)
        .await;
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&FirefamAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            enabled_plugin_ids: vec!["linear@firefamai-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec![
                "gmail@firefamai-curated".to_string(),
                "calendar@firefamai-curated".to_string(),
            ],
        }
    );
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/firefamai-curated/linear/local")
            .is_dir()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/firefamai-curated/gmail")
            .exists()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/firefamai-curated/calendar")
            .exists()
    );
    ...
    assert!(config.contains("enabled = true"));
    assert!(!config.contains(r#"[plugins."gmail@firefamai-curated"]"#));
    assert!(!config.contains(r#"[plugins."calendar@firefamai-curated"]"#));
    ...
    let curated_marketplace = manager
        ...
        .marketplaces
        .into_iter()
        .find(|marketplace| marketplace.name == FIREFAMAI_CURATED_MARKETPLACE_NAME)
        .unwrap();
    assert_eq!(
        curated_marketplace
            .plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            ("linear@firefamai-curated".to_string(), true, true),
            ("gmail@firefamai-curated".to_string(), false, false),
            ("calendar@firefamai-curated".to_string(), false, false),
        ]
    );
}
...
"#,
  ...
  {"id":"2","name":"gmail","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":false}
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/firefamai-curated/gmail/local")
            .is_dir()
    );
    assert!(
        tmp.path()
            .join("plugins/cache/firefamai-curated/calendar/local")
            .is_dir()
    );
...
"#,
    ...
    Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(
                r#"[
  {"id":"1","name":"plugin-one","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":true}
]"#,
            ))
            .mount(&server)
            .await;
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec!["linear@firefamai-curated".to_string()],
        }
    );
    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(!config.contains(r#"[plugins."linear@firefamai-curated"]"#));
    assert!(
        !tmp.path()
            .join("plugins/cache/firefamai-curated/linear")
            .exists()
    );
}
...
async fn sync_plugins_from_remote_keeps_existing_plugins_when_install_fails() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_firefamai_curated_marketplace(&curated_root, &["linear", "gmail"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"gmail","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":true}
]"#,
        ))
        .mount(&server)
        .await;
...
"#,
  ...
  {"id":"1","name":"gmail","marketplace_name":"firefamai-curated","version":"1.0.0","enabled":true}
    ...
    assert_eq!(
        result,
        RemotePluginSyncResult {
            installed_plugin_ids: vec!["gmail@firefamai-curated".to_string()],
            enabled_plugin_ids: vec!["gmail@firefamai-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: Vec::new(),
        }
    );
    assert_eq!(
        fs::read_to_string(tmp.path().join(format!(
            "plugins/cache/firefamai-curated/gmail/{TEST_CURATED_PLUGIN_CACHE_VERSION}/marker.txt"
        )))
        .unwrap(),
        "first"
    );
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_firefam() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "firefam"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["firefam-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["firefam-plugin".to_string()]);
}
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_short_sha_version() {
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/firefamai-curated/slack/local")
            .exists()
    );
    assert!(
        tmp.path()
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_firefam_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
[plugins."slack@firefamai-curated"]
enabled = true
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_firefam_home(tmp.path())
            .into_iter()
            .map(|plugin_id| plugin_id.as_key())
            .collect::<Vec<_>>(),
        vec!["slack@firefamai-curated".to_string()]
    );
...
fn refresh_curated_plugin_cache_migrates_full_sha_cache_version_to_short_version() {
    ...
    assert!(
        !tmp.path()
            .join(format!(
                "plugins/cache/firefamai-curated/slack/{TEST_CURATED_PLUGIN_SHA}"
            ))
            .exists()
    );
    assert!(
...
async fn load_plugins_ignores_project_config_files() {
    let firefam_home = TempDir::new().unwrap();
    let project_root = firefam_home.path().join("project");
    let plugin_root = firefam_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    let outcome = load_plugins_from_layer_stack(
        ...
        &PluginStore::new(firefam_home.path().to_path_buf()),
        Some(Product::Firefam),
