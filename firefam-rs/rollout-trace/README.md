...
> `FIREFAM_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
```mermaid
flowchart TD
    subgraph Runtime["firefam-core runtime"]
...
one from `FIREFAM_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Firefam session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `firefam debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Firefam rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
firefam debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`firefam_rollout_trace::replay_bundle` directly.
