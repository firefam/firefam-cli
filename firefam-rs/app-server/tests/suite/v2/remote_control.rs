...
use firefam_app_server_protocol::RemoteControlDisableResponse;
use firefam_app_server_protocol::RemoteControlEnableResponse;
use firefam_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let _backend = BlockingRemoteControlBackend::start(firefam_home.path()).await?;
    let mut mcp = McpProcess::new(firefam_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_status_read_returns_connecting_status_after_enable() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(firefam_home.path()).await?;
    let mut mcp = McpProcess::new(firefam_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl BlockingRemoteControlBackend {
    async fn start(firefam_home: &std::path::Path) -> Result<Self> {
