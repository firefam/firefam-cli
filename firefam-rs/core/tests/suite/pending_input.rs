...
async fn build_firefam(server: &StreamingSseServer) -> Arc<FirefamThread> {
    test_firefam()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming Firefam test session: {err}"))
        .firefam
}

async fn submit_user_input(firefam: &FirefamThread, text: &str) {
    firefam
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestFirefam, text: &str) {
...
async fn steer_user_input(firefam: &FirefamThread, text: &str) {
    firefam
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(firefam: &FirefamThread, text: &str) {
    firefam
...
async fn wait_for_reasoning_item_started(firefam: &FirefamThread) {
    wait_for_event(firefam, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(firefam: &FirefamThread, text: &str) {
...
async fn wait_for_turn_complete(firefam: &FirefamThread) {
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let firefam = build_firefam(&server).await;

    submit_user_input(&firefam, "first prompt").await;

    wait_for_reasoning_item_started(&firefam).await;

    submit_queue_only_agent_mail(&firefam, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&firefam, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&firefam, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    let firefam = test_firefam()
        .with_model("gpt-5.4")
        .with_config(|config| {
            config.model_provider.name = "FirefamAI (test)".to_string();
    ...
    submit_user_input(&firefam, "first prompt").await;
    submit_user_input(&firefam, "second prompt").await;

    wait_for_agent_message(&firefam, "resumed old task").await;
    wait_for_turn_complete(&firefam).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&firefam, "processed steered prompt").await;
    wait_for_turn_complete(&firefam).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&firefam, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&firefam, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
