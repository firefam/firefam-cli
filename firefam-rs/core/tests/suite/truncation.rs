    ...
    let mut builder = test_firefam().with_config(move |config| {
        ...
        servers.insert(
            ...
            firefam_config::types::McpServerConfig {
                transport: firefam_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    // 1x1 PNG data URL
    let firefamai_png = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMB/ee9bQAAAABJRU5ErkJggg==";
    ...
    assert_eq!(
        arr[1],
        json!({"type": "input_image", "image_url": firefamai_png, "detail": "high"})
    );
