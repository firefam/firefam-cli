...
use firefam_analytics::CompactionPhase;
use firefam_analytics::CompactionReason;
use firefam_analytics::InvocationType;
use firefam_analytics::TurnResolvedConfigFact;
use firefam_analytics::build_track_events_context;
use firefam_async_utils::OrCancelExt;
use firefam_features::Feature;
use firefam_git_utils::get_git_repo_root;
use firefam_git_utils::get_git_repo_root_with_fs;
...
use firefam_protocol::items::PlanItem;
...
use firefam_protocol::protocol::AgentMessageContentDeltaEvent;
use firefam_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use firefam_protocol::protocol::PlanDeltaEvent;
use firefam_protocol::protocol::ReasoningContentDeltaEvent;
use firefam_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use firefam_tools::filter_request_plugin_install_discoverable_tools_for_client;
use firefam_utils_stream_parser::AssistantTextChunk;
use firefam_utils_stream_parser::AssistantTextStreamParser;
use firefam_utils_stream_parser::ProposedPlanSegment;
use firefam_utils_stream_parser::extract_proposed_plan_text;
use firefam_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<firefam_extension_api::ExtensionData>,
    input: Vec<UserInput>,
    ...
    let pre_sampling_compact =
        match run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
            ...
            Err(err) => {
                if err.to_firefam_protocol_error() == FirefamErrorInfo::UsageLimitExceeded
                    && let Err(err) = sess
    ...
    // Although from the perspective of firefam.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if token_limit_reached && needs_follow_up {
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            if err.to_firefam_protocol_error()
                                == FirefamErrorInfo::UsageLimitExceeded
                                && let Err(err) = sess
                                    .goal_runtime_apply(GoalRuntimeEvent::UsageLimitReached {
                                        turn_context: turn_context.as_ref(),
                                    })
            ...
            Err(FirefamErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(FirefamErr::InvalidImageRequest()) => {
                {
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                if e.to_firefam_protocol_error() == FirefamErrorInfo::UsageLimitExceeded
                    && let Err(err) = sess
                        .goal_runtime_apply(GoalRuntimeEvent::UsageLimitReached {
                            turn_context: turn_context.as_ref(),
                        })
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = firefam_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
...
) -> FirefamResult<PreSamplingCompactResult> {
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = firefam_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<firefam_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    turn_metadata_header: Option<&str>,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> FirefamResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(FirefamErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(FirefamErr::ContextWindowExceeded);
            }
            Err(FirefamErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(FirefamErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                FirefamErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &firefam_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            firefam_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: firefam_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(firefam_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, FirefamResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> FirefamResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, FirefamResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: FirefamResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            firefam.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            firefam.usage.reasoning_output_tokens = field::Empty,
            firefam.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(firefam_async_utils::CancelErr::Cancelled) => break Err(FirefamErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![firefam_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(FirefamErr::TurnAborted);
    }
