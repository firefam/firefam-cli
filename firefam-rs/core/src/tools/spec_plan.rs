...
use firefam_tools::ResponsesApiNamespace;
use firefam_tools::ResponsesApiNamespaceTool;
use firefam_tools::TOOL_SEARCH_TOOL_NAME;
use firefam_tools::ToolCall as ExtensionToolCall;
use firefam_tools::ToolEnvironmentMode;
use firefam_tools::ToolExecutor;
use firefam_tools::ToolName;
use firefam_tools::ToolOutput;
use firefam_tools::ToolSpec;
use firefam_tools::can_request_original_image_detail;
use firefam_tools::collect_code_mode_exec_prompt_tool_definitions;
use firefam_tools::collect_request_plugin_install_entries;
use firefam_tools::default_namespace_description;
use firefam_tools::request_user_input_available_modes;
use firefam_tools::shell_command_backend_for_features;
use firefam_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if code_mode_enabled(turn_context)
        ...
        && firefam_code_mode::is_code_mode_nested_tool(spec.name())
    {
        firefam_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn image_generation_tool_enabled(turn_context: &TurnContext) -> bool {
    turn_context
        .auth_manager
        .as_deref()
        .is_some_and(AuthManager::current_auth_uses_firefam_backend)
...
fn is_hidden_by_code_mode_only(
    ...
    code_mode_only_enabled(turn_context)
        && exposure != ToolExposure::DirectModelOnly
        && firefam_code_mode::is_code_mode_nested_tool(
            &firefam_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, firefam_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| firefam_code_mode::ToolNamespaceDescription {
...
fn add_dynamic_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    for tool in context.dynamic_tools {
        let Some(handler) = DynamicToolHandler::new(tool) else {
            tracing::error!(
                "Failed to convert dynamic tool {:?} to FirefamAI tool",
                tool.name
            );
            continue;
        };
...
fn append_extension_tool_executors(
    ...
    let mut reserved_tool_names = planned_tools
        ...
        .collect::<HashSet<_>>();
    if code_mode_enabled(turn_context) {
        reserved_tool_names.insert(ToolName::plain(firefam_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(firefam_code_mode::WAIT_TOOL_NAME));
    }
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    ) -> Result<Box<dyn ToolOutput>, firefam_tools::FunctionCallError> {
...
fn compare_code_mode_tools(
    left: &firefam_code_mode::ToolDefinition,
    right: &firefam_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, firefam_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &firefam_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, firefam_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
