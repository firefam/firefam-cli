...
use firefam_analytics::GuardianReviewDecision;
use firefam_analytics::GuardianReviewFailureReason;
use firefam_analytics::GuardianReviewTerminalStatus;
use firefam_analytics::GuardianReviewTrackContext;
...
use firefam_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &firefam_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        firefam_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
pub(super) async fn run_guardian_review_session(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(firefam_models_manager::manager::RefreshStrategy::Offline)
        .await;
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == firefam_protocol::firefamai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == firefam_protocol::firefamai_models::ReasoningEffort::Low
                }),
