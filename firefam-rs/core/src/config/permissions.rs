...
use firefam_config::permissions_toml::NetworkUnixSocketPermissionToml;
use firefam_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use firefam_features::NetworkProxyDomainPermissionToml;
use firefam_features::NetworkProxyModeToml;
use firefam_features::NetworkProxyUnixSocketPermissionToml;
use firefam_network_proxy::NetworkMode;
use firefam_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use firefam_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use firefam_protocol::permissions::project_roots_glob_pattern;
...
/// for Firefam to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_firefam_runtime(
    firefam_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(firefam_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Firefam versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Firefam. Filesystem access will remain restricted. Upgrade Firefam if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Firefam and will be ignored. Upgrade Firefam if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Firefam and will be ignored. Upgrade Firefam if this path is required."
            ),
        },
    );
}
