...
pub const FIREFAMAI_FILE_URI_PREFIX: &str = "sediment://";
pub const FIREFAMAI_FILE_UPLOAD_LIMIT_BYTES: u64 = 512 * 1024 * 1024;

const FIREFAMAI_FILE_REQUEST_TIMEOUT: Duration = Duration::from_secs(60);
const FIREFAMAI_FILE_FINALIZE_TIMEOUT: Duration = Duration::from_secs(30);
const FIREFAMAI_FILE_FINALIZE_RETRY_DELAY: Duration = Duration::from_millis(250);
const FIREFAMAI_FILE_USE_CASE: &str = "firefam";
...
pub enum OpenAiFileError {
    ...
    #[error("failed to send FirefamAI file request to {url}: {source}")]
    Request {
        url: String,
        #[source]
        source: reqwest::Error,
    },
    #[error("FirefamAI file request to {url} failed with status {status}: {body}")]
    UnexpectedStatus {
        url: String,
        status: StatusCode,
        body: String,
    },
    #[error("failed to parse FirefamAI file response from {url}: {source}")]
    Decode {
        url: String,
        #[source]
        source: serde_json::Error,
    },
    #[error("FirefamAI file upload for `{file_id}` is not ready yet")]
    UploadNotReady { file_id: String },
    #[error("FirefamAI file upload for `{file_id}` failed: {message}")]
...
pub fn firefamai_file_uri(file_id: &str) -> String {
    format!("{FIREFAMAI_FILE_URI_PREFIX}{file_id}")
}
...
pub async fn upload_local_file(
    ...
    if metadata.len() > FIREFAMAI_FILE_UPLOAD_LIMIT_BYTES {
        return Err(OpenAiFileError::FileTooLarge {
            path: path.to_path_buf(),
            size_bytes: metadata.len(),
            limit_bytes: FIREFAMAI_FILE_UPLOAD_LIMIT_BYTES,
        });
    }
    ...
    let create_response = authorized_request(auth, reqwest::Method::POST, &create_url)
        .json(&serde_json::json!({
            ...
            "use_case": FIREFAMAI_FILE_USE_CASE,
        }))
        .send()
        .await
    ...
    let upload_response = build_reqwest_client()
        ...
        .timeout(FIREFAMAI_FILE_REQUEST_TIMEOUT)
        ...
        match finalize_payload.status.as_str() {
            "success" => {
                return Ok(UploadedOpenAiFile {
                    ...
                    uri: firefamai_file_uri(&create_payload.file_id),
            ...
            "retry" => {
                if finalize_started_at.elapsed() >= FIREFAMAI_FILE_FINALIZE_TIMEOUT {
                    return Err(OpenAiFileError::UploadNotReady {
                        file_id: create_payload.file_id,
                    });
                }
                tokio::time::sleep(FIREFAMAI_FILE_FINALIZE_RETRY_DELAY).await;
            }
            _ => {
...
fn build_reqwest_client() -> reqwest::Client {
    build_reqwest_client_with_custom_ca(reqwest::Client::builder()).unwrap_or_else(|error| {
        tracing::warn!(error = %error, "failed to build FirefamAI file upload client");
        reqwest::Client::new()
    })
}
