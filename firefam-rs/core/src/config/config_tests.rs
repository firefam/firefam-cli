...
use firefam_config::ConfigLayerEntry;
use firefam_config::ProfileV2Name;
use firefam_config::RequirementSource;
use firefam_config::config_toml::AgentRoleToml;
use firefam_config::config_toml::AgentsToml;
use firefam_config::config_toml::AutoReviewToml;
use firefam_config::config_toml::ConfigToml;
use firefam_config::config_toml::ProjectConfig;
use firefam_config::config_toml::RealtimeAudioConfig;
use firefam_config::config_toml::RealtimeConfig;
use firefam_config::config_toml::RealtimeToml;
use firefam_config::config_toml::RealtimeTransport;
use firefam_config::config_toml::RealtimeWsMode;
use firefam_config::config_toml::RealtimeWsVersion;
use firefam_config::config_toml::ToolsToml;
use firefam_config::loader::project_trust_key;
use firefam_config::permissions_toml::FilesystemPermissionToml;
use firefam_config::permissions_toml::FilesystemPermissionsToml;
use firefam_config::permissions_toml::NetworkDomainPermissionToml;
use firefam_config::permissions_toml::NetworkDomainPermissionsToml;
use firefam_config::permissions_toml::NetworkToml;
use firefam_config::permissions_toml::PermissionProfileToml;
use firefam_config::permissions_toml::PermissionsToml;
use firefam_config::permissions_toml::WorkspaceRootsToml;
use firefam_config::profile_toml::ConfigProfile;
use firefam_config::types::AppToolApproval;
use firefam_config::types::ApprovalsReviewer;
use firefam_config::types::BundledSkillsConfig;
use firefam_config::types::FeedbackConfigToml;
use firefam_config::types::HistoryPersistence;
use firefam_config::types::McpServerEnvVar;
use firefam_config::types::McpServerOAuthConfig;
use firefam_config::types::McpServerToolConfig;
use firefam_config::types::McpServerTransportConfig;
use firefam_config::types::MemoriesConfig;
use firefam_config::types::MemoriesToml;
use firefam_config::types::ModelAvailabilityNuxConfig;
use firefam_config::types::Notice;
use firefam_config::types::NotificationCondition;
use firefam_config::types::NotificationMethod;
use firefam_config::types::Notifications;
use firefam_config::types::OtelConfig;
use firefam_config::types::OtelConfigToml;
use firefam_config::types::OtelExporterKind;
use firefam_config::types::SandboxWorkspaceWrite;
use firefam_config::types::SessionPickerViewMode;
use firefam_config::types::SkillsConfig;
use firefam_config::types::ToolSuggestDisabledTool;
use firefam_config::types::ToolSuggestDiscoverableType;
use firefam_config::types::Tui;
use firefam_config::types::TuiKeymap;
use firefam_config::types::TuiNotificationSettings;
use firefam_config::types::TuiPetAnchor;
use firefam_config::types::WindowsSandboxModeToml;
use firefam_config::types::WindowsToml;
use firefam_core_plugins::PluginsManager;
use firefam_exec_server::LOCAL_FS;
use firefam_features::Feature;
use firefam_features::FeaturesToml;
use firefam_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use firefam_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use firefam_model_provider_info::WireApi;
...
use firefam_protocol::models::ManagedFileSystemPermissions;
...
use firefam_protocol::permissions::NetworkSandboxPolicy;
use firefam_protocol::protocol::NetworkAccess;
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let firefam_home = tempdir()?;
    std::fs::write(
        firefam_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = firefam_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .firefam_home(firefam_home.path().to_path_buf())
        .fallback_cwd(Some(firefam_home.path().to_path_buf()))
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(firefam_config::ConfigRequirementsToml {
                network: Some(firefam_config::NetworkRequirementsToml {
                    enabled: Some(true),
                    ..Default::default()
                }),
                ..Default::default()
...
async fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = firefam_home.path().join("memories").abs();
    assert_eq!(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        firefam_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let firefam_home = temp_dir.path().join("firefam-home");
...
-> std::io::Result<()> {
    ...
    let memories_root = AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(
        firefam_home.path().join("memories"),
    )?)?;
    assert!(
        config
            .permissions
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Firefam and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Firefam and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Firefam."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            firefam_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, firefam_home.path());
    let project_dot_firefam =
        AbsolutePathBuf::resolve_path_against_base("project/.firefam", firefam_home.path());
    ...
    let requirements_toml = firefam_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = firefam_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                firefam_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                firefam_app_server_protocol::ConfigLayerSource::Project {
                    dot_firefam_folder: project_dot_firefam.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                firefam_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(firefam_home.path().to_path_buf()),
            ..Default::default()
        },
        firefam_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                firefam_app_server_protocol::ConfigLayerSource::Project {
                    dot_firefam_folder: project_dot_firefam,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                firefam_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
"#,
    ...
    let requirements = firefam_config::ConfigRequirementsToml {
...
#[tokio::test]
async fn sqlite_home_defaults_to_firefam_home_for_workspace_write() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, firefam_home.path().to_path_buf());
...
async fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let memories_root = firefam_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn cli_override_takes_precedence_over_profile_sandbox_mode() -> std::io::Result<()> {
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, firefam_home.abs()).await?;
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    ...
    let config_path = firefam_home.path().join(CONFIG_TOML_FILE);
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &firefam_config::NoopThreadConfigLoader,
    )
    .await?;
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(firefam_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(firefam_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(firefam_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/firefam-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/firefam-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/firefam-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-firefam-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-firefam-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-firefam-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(firefam_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(firefam_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let firefam_home = TempDir::new()?;
    let base_config = firefam_home.path().join(CONFIG_TOML_FILE);
    let selected_config = firefam_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(firefam_home.path(), &profile_name),
        firefam_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
struct PrecedenceTestFixture {
    cwd: TempDir,
    firefam_home: TempDir,
    cfg: ConfigToml,
    model_provider_map: HashMap<String, ModelProviderInfo>,
    firefamai_provider: ModelProviderInfo,
    firefamai_custom_provider: ModelProviderInfo,
}
...
impl PrecedenceTestFixture {
    ...
    fn firefam_home(&self) -> AbsolutePathBuf {
        self.firefam_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let workspace = firefam_home.path().join("workspace");
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        firefam_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let missing_path = firefam_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let role_config_path = firefam_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = firefam_config::ConfigLayerStack::new(
        vec![firefam_config::ConfigLayerEntry::new(
            firefam_app_server_protocol::ConfigLayerSource::User {
                file: firefam_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        firefam_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let researcher_path = firefam_home.path().join("agents").join("researcher.toml");
    let reviewer_path = firefam_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = firefam_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let catalog_path = firefam_home.path().join("catalog.json");
...
[model_providers.firefamai-custom]
name = "FirefamAI custom"
...
[profiles.gpt3]
model = "gpt-3.5-turbo"
model_provider = "firefamai-custom"
...
"#;
    ...
    let firefam_home_temp_dir = TempDir::new().unwrap();

    let firefamai_custom_provider = ModelProviderInfo {
        name: "FirefamAI custom".to_string(),
    ...
    let model_provider_map = {
        let mut model_provider_map =
            built_in_model_providers(/* firefamai_base_url */ /*firefamai_base_url*/ None);
        model_provider_map.insert(
            "firefamai-custom".to_string(),
            firefamai_custom_provider.clone(),
        );
        model_provider_map
    };

    let firefamai_provider = model_provider_map
        .get("openai")
        .expect("firefamai provider should exist")
        .clone();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        firefam_home: firefam_home_temp_dir,
        cfg,
        model_provider_map,
        firefamai_provider,
        firefamai_custom_provider,
    })
}
...
async fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            model_provider: fixture.firefamai_provider.clone(),
            ...
            firefam_home: fixture.firefam_home(),
            sqlite_home: fixture.firefam_home().to_path_buf(),
            log_dir: fixture.firefam_home().join("log").to_path_buf(),
            config_lock_export_dir: None,
            config_lock_allow_firefam_version_mismatch: false,
            ...
            firefam_self_exe: None,
            firefam_linux_sandbox_exe: None,
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: firefam_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let firefam_home = fixture.firefam_home();
...
async fn test_precedence_fixture_with_gpt3_profile() -> std::io::Result<()> {
    ...
    let expected_gpt3_profile_config = Config {
        ...
        model_auto_compact_token_limit_scope: AutoCompactTokenLimitScope::Total,
        service_tier: None,
        model_provider_id: "firefamai-custom".to_string(),
        model_provider: fixture.firefamai_custom_provider.clone(),
        permissions: Permissions {
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = firefam_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![firefam_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = firefam_config::RequirementSource::Unknown;
    ...
    let requirements = firefam_config::ConfigRequirements {
        web_search_mode: firefam_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        firefam_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/firefam4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/firefam3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/firefam2");
...
[projects."/Users/mbolin/code/firefam4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/firefam3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let firefam_home = temp_dir.path();
    let config_path = firefam_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(firefam_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(firefam_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(firefam_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(firefam_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
-> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(firefam_config::ConfigRequirementsToml {
                allowed_sandbox_modes: Some(vec![firefam_config::SandboxModeRequirement::ReadOnly]),
                ..Default::default()
            }))
        }))
        .build()
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let cwd = firefam_home.path().join("workspace");
    let outside_root = firefam_home.path().join("outside-write");
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(firefam_config::ConfigRequirementsToml {
                allowed_web_search_modes: Some(vec![
                    firefam_config::WebSearchModeRequirement::Cached,
                ]),
                ..Default::default()
            }))
        }))
...
async fn feature_requirements_normalize_effective_feature_values() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(firefam_config::ConfigRequirementsToml {
                feature_requirements: Some(firefam_config::FeatureRequirementsToml {
                    entries: BTreeMap::from([
...
export_dir = "locks"
allow_firefam_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_firefam_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let firefam_home = TempDir::new()?;
    let lock_path = firefam_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
firefam_version = "older-version"
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@firefamai-curated" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@firefamai-curated".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),
