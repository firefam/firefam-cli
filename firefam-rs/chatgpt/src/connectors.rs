...
use firefam_connectors::DirectoryListResponse;
use firefam_connectors::filter::filter_disallowed_connectors;
use firefam_connectors::merge::merge_connectors;
use firefam_connectors::merge::merge_plugin_connectors;
use firefam_core::config::Config;
pub use firefam_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use firefam_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use firefam_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use firefam_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use firefam_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use firefam_core::connectors::with_app_enabled_state;
...
async fn connector_auth(config: &Config) -> anyhow::Result<FirefamAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_firefam_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Firefam backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(config: &Config) -> Option<Vec<AppInfo>> {
    ...
    let connectors = firefam_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = firefam_connectors::list_all_connectors_with_options(
        cache_context,
...
async fn plugin_apps_for_config(config: &Config) -> Vec<AppConnectorId> {
    ...
    PluginsManager::new(config.firefam_home.to_path_buf())
...
mod tests {
    use super::*;
    use firefam_connectors::metadata::connector_install_url;
    use firefam_plugin::AppConnectorId;
