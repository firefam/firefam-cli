# firefam-responses-api-proxy

#### tl;dr:

```
# Launch the proxy, dump request/response pairs to /tmp/proxy
cd path/to/firefam/firefam-rs
cargo build
echo $OPENAI_API_KEY | ./target/debug/firefam-responses-api-proxy \
    --port 60001 \
    --dump-dir /tmp/proxy
...
[profiles.proxy]
model_provider = "firefam-responses-api-proxy"
...
A strict HTTP proxy that only forwards `POST` requests to `/v1/responses` to the FirefamAI API (`https://api.openai.com`), injecting the `Authorization: Bearer $OPENAI_API_KEY` header. Everything else is rejected with `403 Forbidden`.
...
A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `firefam-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | env -u OPENAI_API_KEY firefam-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run Firefam as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
firefam exec -c "model_providers.firefamai-proxy={ name = 'FirefamAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
    -c model_provider="firefamai-proxy" \
    'Your prompt here'
```
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | firefam-responses-api-proxy`).
...
```
firefam-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>] [--dump-dir <DIR>]
```
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Firefam CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_OPENAI_API_KEY | env -u AZURE_OPENAI_API_KEY firefam-responses-api-proxy \
  --http-shutdown \
...
- We leverage [`firefam_process_hardening`](https://github.com/firefam/firefam-cli/blob/main/firefam-rs/process-hardening/README.md) so `firefam-responses-api-proxy` is run with standard process-hardening techniques.
