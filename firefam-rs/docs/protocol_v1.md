...
These are entities exit on the firefam backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Firefam` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Firefam`
   - The core engine of firefam
...
2. `Session`
   - The `Firefam`'s current configuration and state
   - `Firefam` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Firefam` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Firefam` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Firefam`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Firefam`, as `Firefam` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Firefam` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Firefam`)
...
- `Event`
  - These are messages sent on the `EQ` (`Firefam` -> UI)
...
The `response_id` returned from each turn matches the FirefamAI `response_id` stored in the API's `/responses` endpoint. It can be stored and used in future `Sessions` to resume threads of work.
...
sequenceDiagram
    ...
    box Daemon
    participant firefam as Firefam
    ...
    user->>firefam: Op::ConfigureSession
    firefam-->>session: create session
    firefam->>user: Event::SessionConfigured
