...
mod tests {
    ...
    fn filter_disallowed_connectors_allows_non_disallowed_connectors() {
        let filtered =
            filter_disallowed_connectors(vec![app("asdk_app_hidden"), app("alpha")], "firefam_cli");
        assert_eq!(filtered, vec![app("asdk_app_hidden"), app("alpha")]);
    }

    #[test]
    fn filter_disallowed_connectors_allows_firefamai_prefix() {
        let filtered = filter_disallowed_connectors(
            vec![
                app("connector_firefamai_foo"),
                app("connector_firefamai_bar"),
                app("gamma"),
            ],
            "firefam_cli",
        );
