...
use firefam_app_server_protocol::AppConfig;
use firefam_app_server_protocol::AppToolApproval;
use firefam_app_server_protocol::AppsConfig;
...
use firefam_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use firefam_app_server_protocol::ToolsV2;
...
use firefam_protocol::config_types::WebSearchContextSize;
use firefam_protocol::config_types::WebSearchLocation;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("firefam")));
    assert_eq!(
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_firefam_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let firefam_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let firefam_home = tmp_dir.path().canonicalize()?;
...
fn assert_layers_user_then_optional_system(
    layers: &[firefam_app_server_protocol::ConfigLayer],
