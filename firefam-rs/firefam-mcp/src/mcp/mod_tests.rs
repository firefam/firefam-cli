...
fn test_mcp_config(firefam_home: PathBuf) -> McpConfig {
    McpConfig {
...
#[test]
fn firefam_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
    ...
    assert_eq!(
        firefam_apps_mcp_url_for_base_url(
            "http://localhost:8080/api/firefam",
            /*apps_mcp_path_override*/ None,
        ),
        "http://localhost:8080/api/firefam/apps"
    );
    assert_eq!(
        firefam_apps_mcp_url_for_base_url(
            "http://localhost:8080",
...
#[test]
fn firefam_apps_mcp_url_uses_legacy_firefam_apps_path() {
    let config = test_mcp_config(PathBuf::from("/tmp"));

    assert_eq!(
        firefam_apps_mcp_url(&config),
        "https://chatgpt.com/backend-api/wham/apps"
    );
}

#[test]
fn firefam_apps_server_config_uses_legacy_firefam_apps_path() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    let auth = FirefamAuth::create_dummy_chatgpt_auth_for_testing();

    let mut servers = with_firefam_apps_mcp(HashMap::new(), /*auth*/ None, &config);
    assert!(!servers.contains_key(FIREFAM_APPS_MCP_SERVER_NAME));

    config.apps_enabled = true;

    servers = with_firefam_apps_mcp(servers, Some(&auth), &config);
    let server = servers
        .get(FIREFAM_APPS_MCP_SERVER_NAME)
        .expect("firefam apps should be present when apps is enabled");
    let config = server
        .configured_config()
        .expect("firefam apps should use configured transport");
    let url = match &config.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for firefam apps"),
    };
...
#[test]
fn firefam_apps_server_config_uses_configured_apps_mcp_path_override() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    config.apps_mcp_path_override = Some("/custom/mcp".to_string());
    config.apps_enabled = true;
    let auth = FirefamAuth::create_dummy_chatgpt_auth_for_testing();

    let servers = with_firefam_apps_mcp(HashMap::new(), Some(&auth), &config);
    let server = servers
        .get(FIREFAM_APPS_MCP_SERVER_NAME)
        .expect("firefam apps should be present when apps is enabled");
    let config = server
...
#[test]
fn firefam_apps_server_config_forwards_configured_product_sku_header() {
    let mut config = test_mcp_config(PathBuf::from("/tmp"));
    ...
    match &config.transport {
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                http_headers,
                &Some(HashMap::from([(
                    "X-FirefamAI-Product-Sku".to_string(),
                    "tpp".to_string(),
                )]))
            );
...
#[tokio::test]
async fn effective_mcp_servers_preserve_user_servers_and_add_firefam_apps() {
    let firefam_home = tempfile::tempdir().expect("tempdir");
    let mut config = test_mcp_config(firefam_home.path().to_path_buf());
    config.apps_enabled = true;
    let auth = FirefamAuth::create_dummy_chatgpt_auth_for_testing();
    ...
    let firefam_apps = effective
        .get(FIREFAM_APPS_MCP_SERVER_NAME)
        .expect("firefam apps server should exist");
