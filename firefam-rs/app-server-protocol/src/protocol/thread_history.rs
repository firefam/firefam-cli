...
use firefam_protocol::protocol::AgentReasoningEvent;
use firefam_protocol::protocol::AgentReasoningRawContentEvent;
...
use firefam_protocol::protocol::ContextCompactedEvent;
use firefam_protocol::protocol::DynamicToolCallResponseEvent;
use firefam_protocol::protocol::ErrorEvent;
use firefam_protocol::protocol::EventMsg;
use firefam_protocol::protocol::ExecCommandBeginEvent;
use firefam_protocol::protocol::ExecCommandEndEvent;
use firefam_protocol::protocol::GuardianAssessmentEvent;
use firefam_protocol::protocol::GuardianAssessmentStatus;
use firefam_protocol::protocol::ImageGenerationBeginEvent;
use firefam_protocol::protocol::ImageGenerationEndEvent;
use firefam_protocol::protocol::ItemCompletedEvent;
use firefam_protocol::protocol::ItemStartedEvent;
use firefam_protocol::protocol::McpToolCallBeginEvent;
use firefam_protocol::protocol::McpToolCallEndEvent;
use firefam_protocol::protocol::PatchApplyBeginEvent;
use firefam_protocol::protocol::PatchApplyEndEvent;
...
use firefam_protocol::protocol::ViewImageToolCallEvent;
use firefam_protocol::protocol::WebSearchBeginEvent;
use firefam_protocol::protocol::WebSearchEndEvent;
...
#[cfg(test)]
use firefam_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use firefam_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `firefam-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &firefam_protocol::models::ResponseItem) {
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            firefam_protocol::items::TurnItem::Plan(plan) => {
            ...
            firefam_protocol::items::TurnItem::UserMessage(_)
            | firefam_protocol::items::TurnItem::HookPrompt(_)
            | firefam_protocol::items::TurnItem::AgentMessage(_)
            | firefam_protocol::items::TurnItem::Reasoning(_)
            | firefam_protocol::items::TurnItem::WebSearch(_)
            | firefam_protocol::items::TurnItem::ImageView(_)
            | firefam_protocol::items::TurnItem::ImageGeneration(_)
            | firefam_protocol::items::TurnItem::FileChange(_)
            | firefam_protocol::items::TurnItem::McpToolCall(_)
            | firefam_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &firefam_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &firefam_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &firefam_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &firefam_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &firefam_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &firefam_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &firefam_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &firefam_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(
        &mut self,
        payload: &firefam_protocol::protocol::CollabCloseEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &firefam_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &firefam_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &firefam_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &firefam_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            firefam_error_info: payload.firefam_error_info.clone().map(Into::into),
...
fn convert_dynamic_tool_content_items(
    items: &[firefam_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            firefam_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text,
            } => DynamicToolCallOutputContentItem::InputText { text },
            firefam_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
mod tests {
    ...
    use firefam_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use firefam_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use firefam_protocol::items::TurnItem as CoreTurnItem;
    use firefam_protocol::items::UserMessageItem as CoreUserMessageItem;
    ...
    use firefam_protocol::models::MessagePhase as CoreMessagePhase;
    use firefam_protocol::models::WebSearchAction as CoreWebSearchAction;
    use firefam_protocol::parse_command::ParsedCommand;
    ...
    use firefam_protocol::protocol::DynamicToolCallResponseEvent;
    use firefam_protocol::protocol::ExecCommandEndEvent;
    use firefam_protocol::protocol::ExecCommandSource;
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                firefam_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("firefam_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    firefam_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(firefam_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(firefam_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: firefam_protocol::firefamai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(firefam_protocol::firefamai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                firefam_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                firefam_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                firefam_error_info: Some(FirefamErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                firefam_error_info: Some(FirefamErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                firefam_error_info: Some(
                    crate::protocol::v2::FirefamErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(firefam_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
