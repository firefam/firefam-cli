...
use firefam_app_server_protocol::SkillsChangedNotification;
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(firefam_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_firefam_plugins_disabled() -> Result<()>
{
    let firefam_home = TempDir::new()?;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(firefam_home.path().join(&relative_cwd))?;
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&firefam_home, "demo")?;

    let mut mcp = McpProcess::new_with_env(
        firefam_home.path(),
        &[(FIREFAM_EXEC_SERVER_URL_ENV_VAR, None)],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
