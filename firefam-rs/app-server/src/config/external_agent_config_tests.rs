...
fn service_for_paths(
    ...
    ExternalAgentConfigService::new_for_test(firefam_home, external_agent_home)
}
...
async fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, external_agent_home, firefam_home) = fixture_paths();
    let agents_skills = firefam_home
        .parent()
    ...
    let items = service_for_paths(external_agent_home.clone(), firefam_home.clone())
    ...
    let expected = vec![
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                firefam_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, firefam_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), firefam_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), firefam_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        firefam_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            external_agent_home.join("settings.json"),
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"firefam","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
        .expect("write settings");
    fs::write(
        external_agent_home
    ...
    service_for_paths(external_agent_home, firefam_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
    ...
    assert_eq!(
        fs::read_to_string(firefam_home.join("AGENTS.md")).expect("read agents"),
        "Firefam guidance"
    );
...
async fn import_local_plugins_returns_completed_status() {
    ...
    let outcome = service_for_paths(external_agent_home, firefam_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, firefam_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_firefam_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, firefam_home)
...
"#,
    ...
    .expect("write project firefam config");
...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, firefam_home)
        .import_plugins(
            /*cwd*/ None,
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(external_agent_home, firefam_home)
