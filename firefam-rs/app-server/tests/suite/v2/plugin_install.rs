...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(firefam_home.path(), &server)?;
    ...
    assert!(
        !firefam_home
            .path()
            .join(format!(
                "plugins/cache/firefamai-curated-remote/{REMOTE_PLUGIN_ID}/1.2.3"
            ))
            .exists()
    );
    Ok(())
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_firefam_plugins_disabled() -> Result<()> {
    let firefam_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Firefam plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                firefam_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(firefam_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    write_analytics_config(firefam_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        firefam_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
        ...
        json!({
            "events": [{
                ...
                "event_params": {
                    ...
                    "plugin_name": "linear",
                    "marketplace_name": "firefamai-curated-remote",
                    "has_skills": true,
                    "mcp_server_count": 0,
                    "connector_ids": [],
                    "product_client_id": DEFAULT_CLIENT_NAME,
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/firefam/analytics-events/events"))
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/firefam/analytics-events/events")
            }) {
...
fn configure_remote_plugin_test(firefam_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(firefam_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        firefam_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
