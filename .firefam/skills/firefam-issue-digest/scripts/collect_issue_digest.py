#!/usr/bin/env python3
"""Collect recent firefamai/firefam issue activity for owner-focused digests."""
...
def parse_args():
    ...
    parser.add_argument(
        "--repo", default="firefamai/firefam", help="OWNER/REPO, default firefamai/firefam"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(firefam(?: desktop| app|\.app| cli)?|desktop|windows firefam app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
