...
use firefam_core::REVIEW_PROMPT;
use firefam_core::config::Config;
use firefam_core::review_format::render_review_output_text;
use firefam_protocol::models::ContentItem;
use firefam_protocol::models::ResponseItem;
use firefam_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use firefam_protocol::protocol::EventMsg;
use firefam_protocol::protocol::ExitedReviewModeEvent;
use firefam_protocol::protocol::Op;
use firefam_protocol::protocol::ReviewCodeLocation;
use firefam_protocol::protocol::ReviewFinding;
use firefam_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Firefam sandbox network restrictions.
    skip_if_no_network!();
    ...
    let firefam = new_conversation_for_server(&server, firefam_home.clone(), |_| {}).await;
    ...
    let _entered =
        wait_for_event(&firefam, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&firefam, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = firefam.rollout_path().expect("rollout path");
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let firefam = new_conversation_for_server(&server, firefam_home.clone(), |cfg| {
    ...
    let _entered =
        wait_for_event(&firefam, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&firefam, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = firefam_home.path().join("resume.jsonl");
    {
        ...
        let user = firefam_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = firefam_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let firefam =
        resume_conversation_for_server(&server, firefam_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let firefam = new_conversation_for_server(&server, firefam_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, firefam_home, resume_path)
        .await
