...
const FIREFAM_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/firefam-app-prod/Firefam.dmg";
const FIREFAM_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/firefam-app-prod/Firefam-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_firefam_app_path() {
        eprintln!(
            "Opening Firefam Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_firefam_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("Firefam Desktop not found; downloading installer...");
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            FIREFAM_DMG_URL_ARM64
        } else {
            FIREFAM_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_firefam_to_user_applications(&download_url)
        .await
        .context("failed to download/install Firefam Desktop")?;
    eprintln!(
        "Launching Firefam Desktop from {installed_app}...",
        installed_app = installed_app.display()
    );
    open_firefam_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_firefam_app_path() -> Option<PathBuf> {
    candidate_firefam_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_firefam_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/Firefam.app")];
    if let Some(home) = std::env::var_os("HOME") {
        paths.push(PathBuf::from(home).join("Applications").join("Firefam.app"));
    }
    paths
}

async fn open_firefam_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
...
async fn download_and_install_firefam_to_user_applications(
    dmg_url: &str,
) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("firefam-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Firefam.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting Firefam Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_firefam_app_in_mount(&mount_point)
            .context("failed to locate Firefam.app in mounted dmg")?;
        install_firefam_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_firefam_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing Firefam Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("Firefam.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Firefam.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Firefam.app to any applications directory");
}
...
fn find_firefam_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Firefam.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tFirefam\t/Volumes/Firefam\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Firefam")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tFirefam Installer\t/Volumes/Firefam Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Firefam Installer")
        );
    }
}
