...
use firefam_tui::LocalStateDbStartupError;
...
pub(crate) fn confirm_repair(startup_error: &LocalStateDbStartupError) -> std::io::Result<bool> {
    eprintln!("Firefam couldn't start because its local database appears to be damaged.");
    eprintln!("Firefam can try a safe repair by backing up those files and rebuilding them.");
    print_technical_details(startup_error);
    crate::confirm("Repair Firefam local data now? [y/N]: ")
}
...
pub(crate) async fn repair_files(
    ...
    let repair_suffix = format!("firefam-repair-{timestamp}");
    ...
    for path in firefam_state::runtime_db_paths(sqlite_home)
        .into_iter()
    ...
    if backups.is_empty() {
        return Err(std::io::Error::other(
            "no repairable Firefam local data files were found",
        ));
    }
...
pub(crate) fn print_repair_backups(backups: &[PathBuf]) {
    eprintln!("Backed up Firefam local data before repair:");
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Firefam couldn't start because its local database appears to be damaged.");
    eprintln!("Run `firefam doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Firefam couldn't start because another Firefam process is using its local data.");
    eprintln!("Quit any other copies of Firefam that may still be running, then try again.");
    print_technical_details(startup_error);
}
...
mod tests {
    ...
    async fn repair_backs_up_owned_database_files() -> std::io::Result<()> {
        ...
        let state_path = firefam_state::state_db_path(temp_dir.path());
        let logs_path = firefam_state::logs_db_path(temp_dir.path());
        let goals_path = firefam_state::goals_db_path(temp_dir.path());
    ...
    async fn repair_replaces_blocking_sqlite_home_file() -> std::io::Result<()> {
        ...
        let startup_error = LocalStateDbStartupError::new(
            firefam_state::state_db_path(sqlite_home.as_path()),
