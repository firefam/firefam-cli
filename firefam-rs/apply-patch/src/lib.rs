...
/// Special argv[1] flag used when the Firefam executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `firefam-apply-patch` (to avoid forcing
/// `firefam-arg0` to depend on `firefam-core`), it remains part of the "firefam core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const FIREFAM_CORE_APPLY_PATCH_ARG1: &str = "--firefam-run-as-apply-patch";
