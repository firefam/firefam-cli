...
async fn replace_mcp_servers_serializes_per_tool_approval_overrides() -> anyhow::Result<()> {
    ...
    let firefam_home = std::env::temp_dir().join(format!(
        "firefam-config-mcp-edit-test-{}-{unique_suffix}",
        std::process::id()
    ));
...
"#
    ...
    let loaded = load_global_mcp_servers(&firefam_home).await?;
    assert_eq!(loaded, servers);

    std::fs::remove_dir_all(&firefam_home)?;
...
async fn replace_mcp_servers_serializes_oauth_client_id() -> anyhow::Result<()> {
    let unique_suffix = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let firefam_home = std::env::temp_dir().join(format!(
        "firefam-config-mcp-oauth-edit-test-{}-{unique_suffix}",
        std::process::id()
    ));
    let servers = BTreeMap::from([(
