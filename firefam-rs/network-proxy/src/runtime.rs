...
const NETWORK_POLICY_VIOLATION_PREFIX: &str = "FIREFAM_NETWORK_POLICY_VIOLATION";
...
impl NetworkProxyState {
    ...
    pub async fn current_cfg(&self) -> Result<NetworkProxyConfig> {
        ...
        // `config.toml` (including Firefam-managed writes) take effect without a restart.
...
mod tests {
    ...
    fn blocked_request_violation_log_line_serializes_payload() {
        ...
        assert_eq!(
            ...
            r#"FIREFAM_NETWORK_POLICY_VIOLATION {"host":"google.com","reason":"not_allowed","client":"127.0.0.1","method":"GET","mode":"full","protocol":"http","decision":"ask","source":"decider","port":80,"timestamp":1735689600}"#
        );
    }

    #[tokio::test]
    async fn host_blocked_subdomain_wildcards_exclude_apex() {
        let state = network_proxy_state_for_policy(network_settings(&["*.firefam.ai"], &[]));
        ...
        assert_eq!(
            state.host_blocked("firefam.ai", /*port*/ 80).await.unwrap(),
            HostBlockDecision::Blocked(HostBlockReason::NotAllowed)
        );
    }
    ...
    fn compile_globset_excludes_apex_for_subdomain_patterns() {
        let patterns = vec!["*.firefam.ai".to_string()];
        let set = compile_denylist_globset(&patterns).unwrap();
        assert!(set.is_match("api.openai.com"));
        assert!(!set.is_match("firefam.ai"));
        assert!(!set.is_match("evilfirefam.ai"));
    }
