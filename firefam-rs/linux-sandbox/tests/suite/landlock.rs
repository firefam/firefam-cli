...
use firefam_core::exec::process_exec_tool_call;
...
fn firefam_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_firefam-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> firefam_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<firefam_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let firefam_linux_sandbox_exe = Some(firefam_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &firefam_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(FirefamErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(FirefamErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<firefam_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> firefam_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(FirefamErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let firefam_linux_sandbox_exe: Option<PathBuf> = Some(firefam_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://firefam.ai"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://firefam.ai"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_firefam_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let firefam_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", firefam_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".firefam write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(firefam_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_firefam_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let firefam_output = expect_denied(
        ...
        .await,
        ".firefam symlink replacement should be denied",
    );
    assert_ne!(firefam_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_firefam_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(FirefamErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".firefam symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|firefam|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = firefam_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "firefam.ai"]).await;
}
