...
use firefam_config::AppRequirementToml;
use firefam_config::AppToolRequirementToml;
use firefam_config::AppToolsRequirementsToml;
...
use firefam_config::types::AppsDefaultConfig;
use firefam_connectors::merge::plugin_connector_to_app_info;
use firefam_connectors::metadata::connector_install_url;
use firefam_connectors::metadata::sanitize_name;
...
fn firefam_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{FIREFAM_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| FIREFAM_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            AppInfo {
                id: "connector_firefamai_hidden".to_string(),
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__firefam_apps__calendar".to_string(),
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let firefam_home = tempdir().expect("tempdir should succeed");
    std::fs::write(firefam_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(firefam_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .firefam_home(firefam_home.path().to_path_buf())
        .fallback_cwd(Some(firefam_home.path().to_path_buf()))
        .build()
