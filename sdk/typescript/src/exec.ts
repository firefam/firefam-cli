...
const PLATFORM_PACKAGE_BY_TARGET: Record<string, string> = {
  ...
  "aarch64-unknown-linux-musl": "firefam-linux-arm64",
  "x86_64-apple-darwin": "firefam-darwin-x64",
  "aarch64-apple-darwin": "firefam-darwin-arm64",
  "x86_64-pc-windows-msvc": "firefam-win32-x64",
  "aarch64-pc-windows-msvc": "firefam-win32-arm64",
};
  ...
  async *run(args: CodexExecArgs): AsyncGenerator<string> {
    ...
    try {
      ...
      if (code !== 0 || signal) {
        ...
        throw new Error(`Firefam Exec exited with ${detail}: ${stderrBuffer.toString("utf8")}`);
      }
    } finally {
      rl.close();
...
function flattenConfigOverrides(
  ...
  if (!isPlainObject(value)) {
    if (prefix) {
      ...
      throw new Error("Firefam config overrides must be a plain object");
    }
  }
  ...
  for (const [key, child] of entries) {
    if (!key) {
      throw new Error("Firefam config override keys must be non-empty strings");
    }
    if (child === undefined) {
      continue;
    }
...
function toTomlValue(value: CodexConfigValue, path: string): string {
  if (typeof value === "string") {
    ...
    if (!Number.isFinite(value)) {
      throw new Error(`Firefam config override at ${path} must be a finite number`);
    }
    return `${value}`;
    ...
    throw new Error(`Firefam config override at ${path} cannot be null`);
  } else {
    const typeName = typeof value;
    throw new Error(`Unsupported Firefam config override value at ${path}: ${typeName}`);
  }
}
...
function findCodexPath() {
  ...
  const firefamBinaryName = process.platform === "win32" ? "firefam.exe" : "firefam";
  const binaryPath = path.join(archRoot, "firefam", firefamBinaryName);
