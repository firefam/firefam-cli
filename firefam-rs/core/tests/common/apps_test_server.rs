use crate::test_firefam::TestFirefamBuilder;
use crate::test_firefam::test_firefam;
use anyhow::Result;
...
const FIREFAM_APPS_META_KEY: &str = "_firefam_apps";
const PROTOCOL_VERSION: &str = "2025-11-25";
const SERVER_NAME: &str = "firefam-apps-test";
...
pub const DIRECT_CALENDAR_CREATE_EVENT_TOOL: &str = "mcp__firefam_apps__calendar_create_event";
pub const DIRECT_CALENDAR_LIST_EVENTS_TOOL: &str = "mcp__firefam_apps__calendar_list_events";
pub const DIRECT_CALENDAR_EXTRACT_TEXT_TOOL: &str = "mcp__firefam_apps__calendar_extract_text";
pub const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__firefam_apps__calendar";
...
pub fn apps_enabled_builder(apps_base_url: impl Into<String>) -> TestFirefamBuilder {
...
pub fn search_capable_apps_builder(apps_base_url: impl Into<String>) -> TestFirefamBuilder {
    let apps_base_url = apps_base_url.into();
    test_firefam()
        .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
...
async fn recorded_apps_tool_calls(server: &MockServer) -> Vec<Value> {
    server
        ...
        .filter_map(|request| {
            ...
            (request.url.path() == "/api/firefam/apps"
...
async fn mount_streamable_http_json_rpc(
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/firefam/apps/?$"))
        .respond_with(FirefamAppsJsonRpcResponder {
            connector_name,
...
struct FirefamAppsJsonRpcResponder {
    connector_name: String,
    connector_description: String,
    searchable: bool,
}

impl Respond for FirefamAppsJsonRpcResponder {
        ...
        match method {
            ...
            "tools/call" => {
                ...
                let file_id = body
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or_default();
                let firefam_apps_meta = body.pointer("/params/_meta/_firefam_apps").cloned();
                ...
                ResponseTemplate::new(200).set_body_json(json!({
                    ...
                    "result": {
                        ...
                        "structuredContent": {
                            "_firefam_apps": firefam_apps_meta,
                        },
                        "isError": false
                    }
                }))
