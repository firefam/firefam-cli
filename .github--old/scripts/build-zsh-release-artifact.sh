...
work_root="$(mktemp -d "${temp_root%/}/firefam-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/firefam-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${FIREFAM_WRAPPER_LOG:?missing FIREFAM_WRAPPER_LOG}"
printf '%s\n' "$@" > "$FIREFAM_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
FIREFAM_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" firefam-zsh)
