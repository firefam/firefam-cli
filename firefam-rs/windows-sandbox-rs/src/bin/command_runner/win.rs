...
use anyhow::Context;
use anyhow::Result;
use firefam_windows_sandbox::ErrorPayload;
use firefam_windows_sandbox::ExitPayload;
use firefam_windows_sandbox::FramedMessage;
use firefam_windows_sandbox::LocalSid;
use firefam_windows_sandbox::Message;
use firefam_windows_sandbox::OutputPayload;
use firefam_windows_sandbox::OutputStream;
use firefam_windows_sandbox::PipeSpawnHandles;
use firefam_windows_sandbox::ResizePayload;
use firefam_windows_sandbox::SandboxPolicy;
use firefam_windows_sandbox::SpawnReady;
use firefam_windows_sandbox::SpawnRequest;
use firefam_windows_sandbox::StderrMode;
use firefam_windows_sandbox::StdinMode;
use firefam_windows_sandbox::allow_null_device;
use firefam_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use firefam_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use firefam_windows_sandbox::decode_bytes;
use firefam_windows_sandbox::encode_bytes;
use firefam_windows_sandbox::get_current_token_for_restriction;
use firefam_windows_sandbox::hide_current_user_profile_dir;
use firefam_windows_sandbox::log_note;
use firefam_windows_sandbox::parse_policy;
use firefam_windows_sandbox::read_frame;
use firefam_windows_sandbox::read_handle_loop;
use firefam_windows_sandbox::spawn_process_with_pipes;
use firefam_windows_sandbox::to_wide;
use firefam_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\FirefamSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<firefam_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.firefam_home.clone();
    hide_current_user_profile_dir(req.firefam_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = firefam_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
