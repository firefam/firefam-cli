...
mod tests {
    ...
    fn expect_interrupt_only(rx: &mut tokio::sync::mpsc::UnboundedReceiver<AppEvent>) {
        ...
        let AppEvent::FirefamOp(op) = event else {
            panic!("expected FirefamOp");
        };
    ...
    fn options_can_submit_empty_when_unanswered() {
        ...
        let AppEvent::FirefamOp(Op::UserInputAnswer { id, response, .. }) = event else {
    ...
    fn enter_commits_default_selection_on_last_option_question() {
        ...
        let event = rx.try_recv().expect("expected AppEvent");
        let AppEvent::FirefamOp(Op::UserInputAnswer { response, .. }) = event else {
            panic!("expected UserInputAnswer");
        };
        let answer = response.answers.get("q1").expect("answer missing");
