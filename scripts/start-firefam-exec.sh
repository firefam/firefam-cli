...
remote_path='~/code/firefam-sync'
local_exec_server_port="${FIREFAM_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${FIREFAM_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/firefam-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/firefam-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/firefam-sync"
remote_firefam_rs="$remote_repo_root/firefam-rs"

cd "${remote_firefam_rs}"
cargo build -p firefam-cli --bin firefam

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/firefam exec-server --listen ws://127.0.0.1:0 \
...
echo "Start firefam via: "
printf '  FIREFAM_EXEC_SERVER_URL=ws://127.0.0.1:%s firefam -C %q\n' \
