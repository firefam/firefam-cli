...
use firefam_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use firefam_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "firefam-cli")])?;
    ...
    let metric =
        find_metric(&resource_metrics, "firefam.session_started").expect("counter metric missing");
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("firefam.session_started", /*inc*/ 1, &[]);
    manager.shutdown_metrics()?;
...
fn manager_records_plugin_install_elicitation_sent_metric() -> Result<()> {
    ...
    manager.record_plugin_install_elicitation_sent("plugin", "slack@firefamai-curated", "Slack");
    manager.shutdown_metrics()?;
