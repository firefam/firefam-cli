...
//! - sensitive subpaths such as `.git`, `.agents`, and `.firefam` remain
...
use firefam_protocol::permissions::is_protected_metadata_name;
...
use firefam_protocol::protocol::FileSystemSpecialPath;
use firefam_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.firefam`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".firefam")
                {
                    return None;
                }
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(FirefamErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_firefam = temp_dir.path().join("real-firefam");
        let logical_firefam = logical_home.join(".firefam");
        let real_memories = real_firefam.join("memories");
        let logical_memories = logical_firefam.join("memories");
        ...
        std::os::unix::fs::symlink(&real_firefam, &logical_firefam)
            .expect("create symlinked firefam home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".firefam"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_firefam = path_to_string(&temp_dir.path().join(".firefam"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_firefam));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_firefam)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
