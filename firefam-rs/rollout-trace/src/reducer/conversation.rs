...
impl TraceReducer {
    ...
    pub(super) fn reduce_inference_request(
        ...
        // history, not the pre-compaction prompt. Any repeated developer/context prefix that Firefam
    ...
    pub(super) fn reduce_inference_response(
        ...
        let Some((thread_id, firefam_turn_id)) = self
            .rollout
            .inference_calls
            ...
            .map(|inference| {
                (
                    ...
                    inference.firefam_turn_id.clone(),
                )
            })
        else {
        ...
        let response_item_ids = self.reconcile_conversation_items(
            ...
            ReconcileItems {
                thread_id: &thread_id,
                firefam_turn_id: &firefam_turn_id,
                wall_time_unix_ms,
        ...
        for (offset, item) in items.into_iter().enumerate() {
            ...
            let item_id = if let Some(previous_item_id) = previous_snapshot.get(index) {
                if self.item_matches(previous_item_id, &item) {
                    ...
                    self.find_matching_snapshot_item(&previous_snapshot, &item_ids, &item)
                        .unwrap_or_else(|| {
                            self.create_conversation_item(
                                context.thread_id,
                                Some(context.firefam_turn_id.to_string()),
                    ...
                    let firefam_turn_id = context.firefam_turn_id;
                    let thread_id = context.thread_id;
                    bail!(
                        "model conversation mismatch while reducing turn {firefam_turn_id} for \
    ...
    pub(super) fn reduce_compaction_checkpoint(
        ...
        let marker_item_id = self.create_conversation_item(
            thread_id,
            Some(firefam_turn_id.to_string()),
            wall_time_unix_ms,
