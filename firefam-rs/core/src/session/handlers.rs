...
use firefam_protocol::protocol::ThreadSettingsAppliedEvent;
use firefam_protocol::protocol::ThreadSettingsOverrides;
use firefam_protocol::protocol::ThreadSettingsSnapshot;
...
use firefam_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: firefam_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        firefam_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        firefam_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        firefam_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Firefam will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
    ...
    if let Err(err) = persist_thread_memory_mode_update(sess, mode).await {
        ...
        let event = Event {
            ...
            msg: EventMsg::Error(ErrorEvent {
                ...
                firefam_error_info: Some(FirefamErrorInfo::Other),
            }),
        };
...
async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(firefam_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Firefam instance");
    ...
    sess.services.session_telemetry.counter(
        "firefam.conversation.turn.count",
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(firefam_rollout_trace::RolloutStatus::Completed);
    true
}
