//! Types used to define loaded and effective Firefam configuration values.
...
use firefam_protocol::config_types::EnvironmentVariablePattern;
pub use firefam_protocol::config_types::ModeKind;
pub use firefam_protocol::config_types::Personality;
pub use firefam_protocol::config_types::ServiceTier;
use firefam_protocol::config_types::ShellEnvironmentPolicy;
use firefam_protocol::config_types::ShellEnvironmentPolicyInherit;
...
/// Determine where Firefam should store CLI auth credentials.
...
/// Determine where Firefam should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Firefam product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Firefam product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Firefam rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Firefam does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Firefam cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// transcript exceeds this cap. Omit to use Firefam's terminal-specific default. Set to `0` to
...
/// (primarily the Firefam IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Firefam-managed fast defaults.
    ...
    pub hide_gpt_5_1_firefam_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Firefam skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Firefam successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Firefam last successfully activated for this marketplace.
    #[serde(default)]
...
impl From<SandboxWorkspaceWrite> for firefam_app_server_protocol::SandboxSettings {
