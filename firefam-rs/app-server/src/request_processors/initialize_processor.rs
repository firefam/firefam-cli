...
use firefam_login::default_client::SetOriginatorError;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["firefam_app_server_daemon", "firefam-backend"];
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let firefam_home = self.config.firefam_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE,
        ...
        let user_agent = get_firefam_user_agent();
