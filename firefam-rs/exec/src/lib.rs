...
pub use event_processor_with_jsonl_output::FirefamStatus;
...
use firefam_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use firefam_app_server_client::EnvironmentManager;
use firefam_app_server_client::ExecServerRuntimePaths;
use firefam_app_server_client::InProcessAppServerClient;
use firefam_app_server_client::InProcessClientStartArgs;
use firefam_app_server_client::InProcessServerEvent;
...
use firefam_app_server_protocol::Thread as AppServerThread;
use firefam_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use firefam_cloud_requirements::cloud_requirements_loader_for_storage;
use firefam_config::ConfigLoadError;
use firefam_config::ConfigLoadOptions;
use firefam_config::LoaderOverrides;
use firefam_config::format_config_error_with_source;
use firefam_core::StateDbHandle;
use firefam_core::check_execpolicy_for_warnings;
...
use firefam_core::config::load_config_as_toml_with_cli_and_load_options;
use firefam_core::config::resolve_oss_provider;
use firefam_core::config::resolve_profile_v2_config_path;
use firefam_core::find_thread_meta_by_name_str;
use firefam_core::format_exec_policy_error_with_source;
use firefam_core::path_utils;
use firefam_feedback::FirefamFeedback;
use firefam_git_utils::get_git_repo_root;
use firefam_login::AuthConfig;
use firefam_login::default_client::set_default_client_residency_requirement;
use firefam_login::default_client::set_default_originator;
use firefam_login::enforce_login_restrictions;
use firefam_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use firefam_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use firefam_otel::set_parent_from_context;
use firefam_otel::traceparent_context_from_env;
...
use firefam_protocol::protocol::ReviewTarget;
use firefam_protocol::protocol::RolloutItem;
use firefam_protocol::protocol::RolloutLine;
...
use firefam_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use firefam_utils_cli::SharedCliOptions;
use firefam_utils_oss::ensure_oss_provider_ready;
use firefam_utils_oss::get_default_model_for_oss_provider;
...
enum StdinPromptBehavior {
    ...
    /// `firefam exec` behavior for `firefam exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `firefam exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("firefam_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set firefam exec originator override {err:?}"
        );
    }
    ...
    #[allow(clippy::print_stderr)]
    let firefam_home = match find_firefam_home() {
        Ok(firefam_home) => firefam_home,
        Err(err) => {
            eprintln!("Error finding firefam home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&firefam_home, profile_v2));
    ...
    let cloud_requirements = cloud_requirements_loader_for_storage(
        ...
        /*enable_firefam_api_key_env*/ false,
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        firefam_core::otel_init::build_provider(
            &config,
    ...
    firefam_core::otel_init::record_process_start(otel.as_ref(), "firefam_exec");
    firefam_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "firefam_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.firefam_self_exe.clone(),
        arg0_paths.firefam_linux_sandbox_exe.clone(),
    )?;
    let state_db = firefam_core::init_state_db(&config).await;
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: FirefamFeedback::new(),
        log_db: None,
        state_db: state_db.clone(),
        environment_manager: std::sync::Arc::new(environment_manager),
        config_warnings,
        session_source: SessionSource::Exec,
        enable_firefam_api_key_env: true,
        client_name: "firefam_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = firefam_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    if !json_mode
        && let Some(message) =
            firefam_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Firefam initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        firefam_app_server_protocol::TurnStatus::Failed
                            | firefam_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        FirefamStatus::Running => {}
                        FirefamStatus::InitiateShutdown => {
...
fn session_configured_from_thread_response(
    session_id: &str,
    thread_id: &str,
    thread_source: Option<firefam_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<firefam_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.firefam_home, session_id, Some(state_db.as_ref()))
                .await?
