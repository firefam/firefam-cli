...
`firefam-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `firefam-core`.
...
compatibility-sensitive orchestration in `firefam-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `firefam-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `firefam-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `firefam-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `firefam-core`.
3. Leave compatibility-sensitive adapters in `firefam-core` while downstream
...
`firefam-core`, that is a sign to revisit the extraction boundary before adding
more here.
