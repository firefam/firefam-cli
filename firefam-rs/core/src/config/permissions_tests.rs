...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let sibling_arg0_dir = arg0_root.join("firefam-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("firefam-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&firefam_home)?,
    )
    .await?;
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(firefam_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        ...
        Some(firefam_network_proxy::NetworkDomainPermissions {
            entries: vec![firefam_network_proxy::NetworkDomainPermissionEntry {
                pattern: "firefam.ai".to_string(),
                permission: firefam_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
