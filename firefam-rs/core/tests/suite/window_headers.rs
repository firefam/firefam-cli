...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let mut builder = test_firefam().with_config(|config| {
        config.model_provider.name = "Non-FirefamAI Model provider".to_string();
        config.compact_prompt = Some(SUMMARIZATION_PROMPT.to_string());
    });
    let initial = builder.build(&server).await?;
    let initial_thread = Arc::clone(&initial.firefam);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.firefam, "after resume").await?;
    shutdown_thread(&resumed.firefam).await?;
...
async fn submit_user_turn(firefam: &Arc<FirefamThread>, text: &str) -> Result<()> {
    firefam
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(firefam: &Arc<FirefamThread>) -> Result<()> {
    firefam.submit(Op::Compact).await?;
    let warning_event =
        wait_for_event(firefam, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(firefam: &Arc<FirefamThread>) -> Result<()> {
    firefam.submit(Op::Shutdown).await?;
    wait_for_event(firefam, |event| matches!(event, EventMsg::ShutdownComplete)).await;
    Ok(())
}

fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-firefam-window-id")
        .expect("missing x-firefam-window-id header");
