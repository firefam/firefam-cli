WARNING: this code is mainly generated by Firefam and should not be used in production

# firefam-debug-client

A tiny interactive client for `firefam app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `firefam app-server` itself):

```
cargo run -p firefam-debug-client -- \
  --firefam-bin firefam \
...
```
cargo run -p firefam-debug-client -- --thread-id thr_123
```

### CLI flags

- `--firefam-bin <path>`: path to the `firefam` binary (default: `firefam`).
- `-c, --config key=value`: pass through `--config` overrides to `firefam`.
