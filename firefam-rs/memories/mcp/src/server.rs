...
pub async fn run_server<T, E, A>(firefam_home: &AbsolutePathBuf, transport: T) -> anyhow::Result<()>
where
    T: rmcp::transport::IntoTransport<rmcp::RoleServer, E, A>,
    E: std::error::Error + Send + Sync + 'static,
{
    let backend = LocalMemoriesBackend::from_firefam_home(firefam_home);
...
pub async fn run_stdio_server(firefam_home: &AbsolutePathBuf) -> anyhow::Result<()> {
    run_server(firefam_home, (tokio::io::stdin(), tokio::io::stdout())).await
}
