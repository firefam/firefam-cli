...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "firefam-tui";
#[cfg(any(unix, windows))]
const OPEN_IDE_HINT: &str =
    "Open this project in VS Code or Cursor with the Firefam extension active.";
#[cfg(any(unix, windows))]
const IDE_DID_NOT_PROVIDE_CONTEXT_HINT: &str = "The IDE extension did not provide context.";
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "Firefam will keep trying on future messages.";
    ...
    pub(crate) fn prompt_skip_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "Firefam timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            IdeContextError::RequestFailed(error) if error == "client-disconnected" => {
                hint_with_retry("The IDE connection changed while Firefam was requesting context.")
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("Firefam lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("Firefam received an unexpected IDE context response.")
            }
...
#[cfg(windows)]
fn default_ipc_socket_path() -> PathBuf {
    PathBuf::from(r"\\.\pipe\firefam-ipc")
}
...
mod tests {
    ...
    fn validate_unix_socket_path_rejects_unsafe_parent_directory() {
        ...
        let socket_path = tempdir.path().join("firefam-ipc.sock");
