...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<firefam_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: firefam_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["firefamai-bundled"];
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![firefam_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, firefam_protocol::mcp::Tool>,
    HashMap<String, Vec<firefam_protocol::mcp::Resource>>,
    HashMap<String, Vec<firefam_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                firefam_app_server_protocol::McpAuthStatus::Unsupported => {
                    McpAuthStatus::Unsupported
                }
                firefam_app_server_protocol::McpAuthStatus::NotLoggedIn => {
                    McpAuthStatus::NotLoggedIn
                }
                firefam_app_server_protocol::McpAuthStatus::BearerToken => {
                    McpAuthStatus::BearerToken
                }
                firefam_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_firefamai_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "firefamai-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/firefamai-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "firefamai-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/firefamai-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],
