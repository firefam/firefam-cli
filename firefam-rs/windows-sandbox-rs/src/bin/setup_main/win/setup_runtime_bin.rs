...
pub(super) fn ensure_firefam_app_runtime_bin_readable(
    ...
    // Firefam desktop copies bundled Windows binaries out of WindowsApps to this
    // fixed LocalAppData cache before launching firefam.exe.
    let runtime_bin_dir = local_app_data.join("FirefamAI").join("Firefam").join("bin");
