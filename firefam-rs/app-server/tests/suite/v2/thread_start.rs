...
use firefam_app_server_protocol::ThreadStatusChangedNotification;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    create_config_toml_without_approval_policy(firefam_home.path(), &server.uri())?;
    let global_agents_path = firefam_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(firefam_home.path(), &server.uri())?;
