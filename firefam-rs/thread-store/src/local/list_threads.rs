...
use firefam_rollout::find_thread_names_by_ids;
use firefam_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => firefam_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => firefam_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => firefam_rollout::SortDirection::Asc,
        SortDirection::Desc => firefam_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        firefam_home: store.config.firefam_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.firefam_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.firefam_home.as_path(), &thread_ids).await
    {
...
async fn list_rollout_threads(
    state_db: Option<firefam_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&firefam_rollout::Cursor>,
    sort_key: firefam_rollout::ThreadSortKey,
    sort_direction: firefam_rollout::SortDirection,
) -> ThreadStoreResult<firefam_rollout::ThreadsPage> {
