...
use chrono::Utc;
use firefam_core::RolloutRecorder;
use firefam_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(firefam_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = firefam_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(firefam_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(firefam_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(firefam_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = firefam_home.to_path_buf();
...
async fn find_handles_gitignore_covering_firefam_home_directory() {
    ...
    let found =
        find_thread_path_by_id_str(&firefam_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
