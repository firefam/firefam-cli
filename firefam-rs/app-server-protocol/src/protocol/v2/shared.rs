use firefam_experimental_api_macros::ExperimentalApi;
use firefam_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use firefam_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use firefam_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
/// This translation layer make sure that we expose firefam error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `firefamErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum FirefamErrorInfo {
...
impl From<CoreFirefamErrorInfo> for FirefamErrorInfo {
    fn from(value: CoreFirefamErrorInfo) -> Self {
        match value {
            CoreFirefamErrorInfo::ContextWindowExceeded => FirefamErrorInfo::ContextWindowExceeded,
            CoreFirefamErrorInfo::UsageLimitExceeded => FirefamErrorInfo::UsageLimitExceeded,
            CoreFirefamErrorInfo::ServerOverloaded => FirefamErrorInfo::ServerOverloaded,
            CoreFirefamErrorInfo::CyberPolicy => FirefamErrorInfo::CyberPolicy,
            CoreFirefamErrorInfo::HttpConnectionFailed { http_status_code } => {
                FirefamErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreFirefamErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                FirefamErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreFirefamErrorInfo::InternalServerError => FirefamErrorInfo::InternalServerError,
            CoreFirefamErrorInfo::Unauthorized => FirefamErrorInfo::Unauthorized,
            CoreFirefamErrorInfo::BadRequest => FirefamErrorInfo::BadRequest,
            CoreFirefamErrorInfo::ThreadRollbackFailed => FirefamErrorInfo::ThreadRollbackFailed,
            CoreFirefamErrorInfo::SandboxError => FirefamErrorInfo::SandboxError,
            CoreFirefamErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                FirefamErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreFirefamErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                FirefamErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreFirefamErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                FirefamErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreFirefamErrorInfo::Other => FirefamErrorInfo::Other,
        }
    }
}
