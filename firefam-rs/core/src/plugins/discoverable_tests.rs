...
async fn list_tool_suggest_discoverable_plugins_returns_uninstalled_curated_plugins() {
    ...
    write_firefamai_curated_marketplace(
        &curated_root,
        &["sample", "slack", "firefamai-developers"],
    );
    ...
    assert_eq!(
        ...
        vec![
            DiscoverablePluginInfo {
                id: "firefamai-developers@firefamai-curated".to_string(),
                name: "firefamai-developers".to_string(),
                description: Some(
...
async fn list_tool_suggest_discoverable_plugins_returns_microsoft_curated_plugins() {
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            "outlook-calendar@firefamai-curated".to_string(),
            "outlook-email@firefamai-curated".to_string(),
            "sharepoint@firefamai-curated".to_string(),
            "teams@firefamai-curated".to_string(),
        ]
    );
}
...
async fn list_tool_suggest_discoverable_plugins_deduplicates_allowlisted_configured_plugin() {
        ...
        .find(|plugin_id| {
            plugin_id
                ...
                .is_some_and(|(_plugin_name, marketplace_name)| {
                    marketplace_name == FIREFAMAI_BUNDLED_MARKETPLACE_NAME
                })
        })
    ...
    let marketplace_root = firefam_home
        .path()
...
"#
    ...
    write_file(
        &firefam_home.path().join(crate::config::CONFIG_TOML_FILE),
        &format!(
            r#"[features]
plugins = true
...
"#
    ...
    assert_eq!(discoverable_plugins.len(), 1);
    assert_eq!(discoverable_plugins[0].id, "slack@firefamai-curated");
}
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    PluginsManager::new(firefam_home.path().to_path_buf())
...
[tool_suggest]
discoverables = [{ type = "plugin", id = "sample@firefamai-curated" }]
"#,
    );
