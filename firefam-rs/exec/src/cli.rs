...
#[command(
    version,
    override_usage = "firefam exec [OPTIONS] [PROMPT]\n       firefam exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Firefam outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "firefam resume --last <prompt>" treats the positional as a prompt, not a session id.
