...
use firefam_execpolicy::RuleRef;
use firefam_execpolicy::rule::PatternToken;
use firefam_execpolicy::rule::PrefixPattern;
use firefam_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Firefam merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `firefam-execpolicy`.
