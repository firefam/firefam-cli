...
use firefam_app_server_protocol::CommandMigration;
...
use firefam_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use firefam_app_server_protocol::ExternalAgentConfigMigrationItemType;
use firefam_app_server_protocol::HookMigration;
use firefam_app_server_protocol::JSONRPCErrorError;
use firefam_app_server_protocol::McpServerMigration;
use firefam_app_server_protocol::MigrationDetails;
use firefam_app_server_protocol::PluginsMigration;
...
use firefam_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use firefam_external_agent_sessions::ImportedExternalAgentSession;
use firefam_external_agent_sessions::PendingSessionImport;
use firefam_external_agent_sessions::prepare_validated_session_imports;
use firefam_external_agent_sessions::record_imported_session;
...
impl ExternalAgentConfigRequestProcessor {
    pub(crate) fn new(
        ...
        Self {
            outgoing,
            migration_service: ExternalAgentConfigService::new(firefam_home.clone()),
            firefam_home,
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| firefam_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
                        ...
                        subagents: details
                            .subagents
                            .into_iter()
                            .map(|subagent| firefam_app_server_protocol::SubagentMigration {
                                name: subagent.name,
                            })
                            .collect(),
                        commands: details
    ...
    async fn import_external_agent_session(
        ...
        if let Some(title) = title
            && let Some(name) = firefam_core::util::normalize_thread_name(&title)
        {
            imported_thread
                .thread
    ...
    fn prepare_validated_session_imports(
        ...
        prepare_validated_session_imports(&self.firefam_home, sessions)
    }

    fn record_imported_session(&self, source_path: &std::path::Path, imported_thread_id: ThreadId) {
        if let Err(err) =
            record_imported_session(&self.firefam_home, source_path, imported_thread_id)
        {
            tracing::warn!(
                error = %err,
