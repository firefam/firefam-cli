...
*/
use crate::exec_env::FIREFAM_THREAD_ID_ENV_VAR;
...
#[cfg(target_os = "macos")]
use firefam_network_proxy::FIREFAM_PROXY_GIT_SSH_COMMAND_MARKER;
use firefam_network_proxy::PROXY_ACTIVE_ENV_KEY;
use firefam_network_proxy::PROXY_ENV_KEYS;
#[cfg(target_os = "macos")]
use firefam_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
...
pub(crate) fn exec_env_for_sandbox_permissions(
        ...
        && env.contains_key(PROXY_ACTIVE_ENV_KEY)
    {
        ...
        // Only macOS injects a Firefam-owned SSH wrapper for the managed SOCKS proxy.
        #[cfg(target_os = "macos")]
        if env
            .get(PROXY_GIT_SSH_COMMAND_ENV_KEY)
            .is_some_and(|command| command.starts_with(FIREFAM_PROXY_GIT_SSH_COMMAND_MARKER))
        {
...
/// runtime-only vars like `FIREFAM_THREAD_ID` without pretending they came from
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(FIREFAM_THREAD_ID_ENV_VAR) {
        override_env.insert(FIREFAM_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    build_override_exports_for_keys("__FIREFAM_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__FIREFAM_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__FIREFAM_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__FIREFAM_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_firefam_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_firefam_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", FIREFAM_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__FIREFAM_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
