...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_corrupt_plugin_share_local_path_mapping(firefam_home.path())?;
...
async fn plugin_share_checkout_adds_personal_marketplace_entry() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareCheckoutResponse {
            remote_plugin_id: "plugins_123".to_string(),
            plugin_id: "demo-plugin@firefam-curated".to_string(),
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                firefam_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
        })
        .await?;
    let response: JSONRPCResponse = timeout(
    ...
    assert_eq!(response.marketplaces[0].name, "firefam-curated");
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: firefam_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(firefam_home.path().join("local-plugin"))?;
    write_plugin_share_local_path_mapping(firefam_home.path(), "plugins_123", &local_plugin_path)?;
...
fn write_corrupt_plugin_share_local_path_mapping(firefam_home: &Path) -> std::io::Result<()> {
    write_file(
