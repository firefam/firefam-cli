...
const MEMO_URI: &str = "memo://firefam/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "firefam/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Firefam TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Firefam TUI):
    /// - Build this binary: `cargo build -p firefam-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `firefam mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Firefam TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://firefam/{slug}".to_string(),
            name: "firefam-memo".to_string(),
            title: Some("Firefam Memo".to_string()),
            description: Some(
                "Template for memo://firefam/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Firefam TUI handling of MCP image outputs.
///
