...
use firefam_app_server_protocol::HookErrorInfo;
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: plugins_test_detail(
                ...
                &[
                    (firefam_app_server_protocol::HookEventName::PreToolUse, 1),
                    (firefam_app_server_protocol::HookEventName::Stop, 2),
                ],
...
#[tokio::test]
async fn plugins_popup_firefamai_curated_tab_omits_marketplace_in_rows() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("FirefamAI Curated marketplace."),
        "expected FirefamAI Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected FirefamAI Curated tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
...
async fn apps_popup_keeps_existing_full_snapshot_while_partial_refresh_loads() {
        ...
        Ok(ConnectorsSnapshot {
            connectors: vec![
                ...
                AppInfo {
                    ...
                    name: "Hidden FirefamAI".to_string(),
                    ...
                    app_metadata: None,
                    labels: None,
                    install_url: Some("https://example.test/hidden-firefamai".to_string()),
                    is_accessible: true,
                    is_enabled: true,
                    plugin_display_names: Vec::new(),
                },
    ...
    assert!(
        !popup.contains("Hidden FirefamAI"),
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(FirefamErrorInfo::ServerOverloaded),
    );
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &firefam_feedback::FeedbackDiagnostics::new(vec![firefam_feedback::FeedbackDiagnostic {
