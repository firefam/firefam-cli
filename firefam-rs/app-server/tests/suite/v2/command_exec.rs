...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", firefam_home.path().display()),
            stderr: String::new(),
        }
    );
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${FIREFAM_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    create_config_toml(firefam_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(firefam_home.path(), Some("networked"))?;
    let mut mcp = McpProcess::new(firefam_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let firefam_home = TempDir::new()?;
    let command_dir = firefam_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !firefam_home.path().join("parent.txt").exists(),
...
"#
    ...
    insert_command_exec_config(firefam_home, &inserted_config)?;
    Ok(())
}
