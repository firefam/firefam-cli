...
#[ignore = "requires tmux and a locally built firefam binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let firefam = firefam_binary(&repo_root)?;
    let firefam_home = tempdir()?;
    let server = MockServer::start().await;
    let _response_mock = responses::mount_sse_once(&server, resize_reflow_sse()).await;
    let firefamai_base_url_config = format!("firefamai_base_url=\"{}/v1\"", server.uri());
    write_config(
        firefam_home.path(),
        &repo_root,
        /*terminal_resize_reflow_enabled*/ true,
    )?;
    write_auth(firefam_home.path())?;

    let session_name = format!("firefam-resize-reflow-smoke-{}", std::process::id());
    ...
    let firefam_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!firefam_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&firefam_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&firefam_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("firefam-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&firefam_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("firefam-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn firefam_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = firefam_utils_cargo_bin::cargo_bin("firefam") {
        return Ok(path);
    }

    let fallback = repo_root.join("firefam-rs/target/debug/firefam");
    anyhow::ensure!(
        fallback.is_file(),
        "firefam binary is unavailable; run `cargo build -p firefam-cli` first"
    );
    Ok(fallback)
}
...
fn write_auth(firefam_home: &Path) -> Result<()> {
    std::fs::write(
