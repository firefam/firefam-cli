...
async fn run_turn(test: &TestFirefam, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestFirefam, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_firefam_with_test_tool(
    server: &wiremock::MockServer,
) -> anyhow::Result<TestFirefam> {
    let mut builder = test_firefam().with_model("test-gpt-5.1-codex");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_firefam_with_test_tool(&server).await?;
