...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    if !config.check_for_update_on_startup || is_source_build_version(FIREFAM_CLI_VERSION) {
        return None;
    }
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, FIREFAM_CLI_VERSION).unwrap_or(false) {
...
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/firefam/firefam-cli/releases/latest";
...
fn version_filepath(config: &Config) -> PathBuf {
    config.firefam_home.join(VERSION_FILENAME).into_path_buf()
}
