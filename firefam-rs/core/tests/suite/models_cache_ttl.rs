...
use firefam_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "firefam-test-ttl";
const VERSIONED_MODEL: &str = "firefam-test-versioned";
const MISSING_VERSION_MODEL: &str = "firefam-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "firefam-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let firefam = Arc::clone(&test.firefam);
    ...
    let cache_path = config.firefam_home.join(CACHE_FILE);
    ...
    firefam
        .submit(Op::UserInput {
            ...
            thread_settings: firefam_protocol::protocol::ThreadSettingsOverrides {
                ...
                approval_policy: Some(firefam_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&firefam, |event| matches!(event, EventMsg::TurnComplete(_))).await;
