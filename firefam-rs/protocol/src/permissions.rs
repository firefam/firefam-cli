...
use firefam_utils_absolute_path::canonicalize_preserving_symlinks;
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_FIREFAM_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_FIREFAM_PATH_NAME)
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Firefam degrades to warn-and-ignore
    /// instead of failing to load. Firefam 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "firefam-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_firefam*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_firefam*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_firefam*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_firefam = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_firefam)
                    .into_iter()
            ...
            // Example: if `<root>/.firefam -> <root>/decoy`, bwrap must still see
            // `<root>/.firefam`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.firefam` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_firefam: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .firefam read-only to the agent by
    ...
    let top_level_firefam = writable_root.join(PROTECTED_METADATA_FIREFAM_PATH_NAME);
    if protect_missing_dot_firefam || top_level_firefam.as_path().is_dir() {
        subpaths.push(top_level_firefam);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "FIREFAM_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_firefam() {
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_firefam_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_firefam.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_firefam_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_firefam_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_firefam*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let expected_dot_firefam = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
