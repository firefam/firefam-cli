...
use firefam_protocol::protocol::ElicitationAction;
        ...
        .submit(Op::UserInput {
            ...
            thread_settings: firefam_protocol::protocol::ThreadSettingsOverrides {
                ...
                collaboration_mode: collaboration_mode.or({
                    Some(firefam_protocol::config_types::CollaborationMode {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request() -> Result<()> {
    ...
    let EventMsg::ElicitationRequest(request) = wait_for_event(&test.firefam, |event| {
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-firefam-turn-metadata/user_input_requested_during_turn"),
        Some(&json!(true))
    );
