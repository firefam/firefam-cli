...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(FirefamErrorInfo::CyberPolicy),
    );
...
async fn completed_plan_table_tail_skips_provisional_history_insert() {
    ...
    controller.push("| --- | --- |\n");
    controller.push("| Verify | Firefam |\n");
    assert!(
        controller.has_live_tail(),
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    crate::pets::write_test_pack(&cfg.firefam_home);
...
async fn prefetch_rate_limits_is_gated_on_chatgpt_auth_provider() {
    ...
    chat.config.model_provider.requires_firefamai_auth = false;
    assert!(!chat.should_prefetch_rate_limits());
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        ...
        limit_name: Some("firefam".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20,
            window_duration_mins: Some(300),
            resets_at: Some(100),
    ...
    let firefam = chat
        .rate_limit_snapshots_by_limit_id
        .get("firefam")
        .expect("firefam snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("firefam_other")
        .expect("firefam_other snapshot should exist");

    assert_eq!(firefam.primary.as_ref().map(|w| w.used_percent), Some(20.0));
    assert_eq!(
        firefam
            .credits
            .as_ref()
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_firefam_limit() {
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            RateLimitReachedType::WorkspaceOwnerCreditsDepleted,
            RateLimitErrorKind::Generic,
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Firefam.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using Firefam.",
        ),
    ];
...
async fn ctrl_c_interrupt_pauses_active_goal_turn() {
    ...
    let mut goal = test_thread_goal(
        firefam_app_server_protocol::ThreadGoalStatus::Active,
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            firefam_app_server_protocol::ThreadGoalUpdatedNotification {
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::pets::write_test_pack(&chat.config.firefam_home);
    chat.set_tui_pet(Some("firefam".to_string()));
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            firefam_app_server_protocol::HookEventName::PreToolUse,
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            firefam_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    chat.refresh_status_line();
    let mut goal = test_thread_goal(
        firefam_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        firefam_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            firefam_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            firefam_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: firefam_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> firefam_app_server_protocol::ThreadGoal {
    firefam_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        firefam_app_server_protocol::HookEventName::PostToolUse,
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![firefam_app_server_protocol::HookOutputEntry {
                kind: firefam_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![firefam_app_server_protocol::HookOutputEntry {
                kind: firefam_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            firefam_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![firefam_app_server_protocol::HookOutputEntry {
                kind: firefam_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            firefam_app_server_protocol::HookEventName::SessionStart,
...
fn hook_run_summary(
    ...
    firefam_app_server_protocol::HookRunSummary {
        ...
        started_at: 1,
        completed_at: (status != firefam_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != firefam_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
