//! Configuration object accepted by the `firefam` MCP tool-call.
...
/// Client-supplied configuration for a `firefam` tool-call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case")]
pub struct FirefamToolCallParam {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<FirefamToolCallApprovalPolicy>,
...
#[serde(rename_all = "kebab-case")]
pub enum FirefamToolCallApprovalPolicy {
    Untrusted,
    OnFailure,
    OnRequest,
    Never,
...
impl From<FirefamToolCallApprovalPolicy> for AskForApproval {
    fn from(value: FirefamToolCallApprovalPolicy) -> Self {
        match value {
            FirefamToolCallApprovalPolicy::Untrusted => AskForApproval::UnlessTrusted,
            FirefamToolCallApprovalPolicy::OnFailure => AskForApproval::OnFailure,
            FirefamToolCallApprovalPolicy::OnRequest => AskForApproval::OnRequest,
            FirefamToolCallApprovalPolicy::Never => AskForApproval::Never,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum FirefamToolCallSandboxMode {
    ReadOnly,
    WorkspaceWrite,
    DangerFullAccess,
}

impl From<FirefamToolCallSandboxMode> for SandboxMode {
    fn from(value: FirefamToolCallSandboxMode) -> Self {
        match value {
            FirefamToolCallSandboxMode::ReadOnly => SandboxMode::ReadOnly,
            FirefamToolCallSandboxMode::WorkspaceWrite => SandboxMode::WorkspaceWrite,
            FirefamToolCallSandboxMode::DangerFullAccess => SandboxMode::DangerFullAccess,
        }
    }
}

/// Builds a `Tool` definition (JSON schema etc.) for the Firefam tool-call.
pub(crate) fn create_tool_for_firefam_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<FirefamToolCallParam>();

    let input_schema = create_tool_input_schema(schema, "Firefam tool schema should serialize");

    Tool {
        name: "firefam".into(),
        title: Some("Firefam".to_string()),
        input_schema,
        output_schema: Some(firefam_tool_output_schema()),
        description: Some(
...
fn firefam_tool_output_schema() -> Arc<JsonObject> {
...
impl FirefamToolCallParam {
    ...
    pub async fn into_config(
        ...
        // Build the `ConfigOverrides` recognized by firefam-core.
...
pub struct FirefamToolCallReplyParam {
...
impl FirefamToolCallReplyParam {
...
/// Builds a `Tool` definition for the `firefam-reply` tool-call.
pub(crate) fn create_tool_for_firefam_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
        ...
        .into_generator()
        .into_root_schema_for::<FirefamToolCallReplyParam>();

    let input_schema =
        create_tool_input_schema(schema, "Firefam reply tool schema should serialize");

    Tool {
        name: "firefam-reply".into(),
        title: Some("Firefam Reply".to_string()),
        input_schema,
        output_schema: Some(firefam_tool_output_schema()),
        description: Some(
...
mod tests {
    ...
    #[test]
    fn verify_firefam_tool_json_schema() {
        let tool = create_tool_for_firefam_tool_call_param();
    ...
    #[test]
    fn verify_firefam_tool_reply_json_schema() {
        let tool = create_tool_for_firefam_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
