...
use firefam_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://firefam/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://firefam/resource.bin";
...
async fn start_resource_apps_mcp_server() -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/firefam/apps", mcp_service);
