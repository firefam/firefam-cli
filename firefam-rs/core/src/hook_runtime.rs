...
use firefam_analytics::CompactionTrigger;
use firefam_analytics::HookRunFact;
use firefam_analytics::build_track_events_context;
use firefam_hooks::PermissionRequestDecision;
use firefam_hooks::PermissionRequestOutcome;
use firefam_hooks::PermissionRequestRequest;
use firefam_hooks::PostToolUseOutcome;
use firefam_hooks::PostToolUseRequest;
use firefam_hooks::PreToolUseOutcome;
use firefam_hooks::PreToolUseRequest;
use firefam_hooks::SessionStartOutcome;
use firefam_hooks::StartHookTarget;
use firefam_hooks::StopOutcome;
use firefam_hooks::StopRequest;
use firefam_hooks::UserPromptSubmitOutcome;
use firefam_hooks::UserPromptSubmitRequest;
use firefam_otel::HOOK_RUN_DURATION_METRIC;
use firefam_otel::HOOK_RUN_METRIC;
...
use firefam_protocol::protocol::HookCompletedEvent;
use firefam_protocol::protocol::HookEventName;
use firefam_protocol::protocol::HookRunStatus;
use firefam_protocol::protocol::HookRunSummary;
use firefam_protocol::protocol::HookSource;
use firefam_protocol::protocol::HookStartedEvent;
    ...
    let target = match &turn_context.session_source {
        SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
            if matches!(
                ...
                firefam_hooks::SessionStartSource::Startup
            ) =>
        {
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = firefam_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = firefam_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(firefam_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: firefam_hooks::HookEvent::AfterAgent {
                event: firefam_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            firefam_hooks::HookResult::Success => continue,
            firefam_hooks::HookResult::FailedContinue(error) => (error, false),
            firefam_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(firefam_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (firefam_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use firefam_protocol::protocol::HookEventName;
    use firefam_protocol::protocol::HookExecutionMode;
    use firefam_protocol::protocol::HookHandlerType;
    use firefam_protocol::protocol::HookRunStatus;
    use firefam_protocol::protocol::HookScope;
    use firefam_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    firefam_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
