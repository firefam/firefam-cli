...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncFirefam needed by async login handles."""
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Firefam.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Firefam.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncFirefam.login_chatgpt()`."""

    _firefam: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._firefam._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._firefam._ensure_initialized()
        return await self._firefam._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncFirefam.login_chatgpt_device_code()`."""
