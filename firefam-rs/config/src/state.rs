...
use firefam_app_server_protocol::ConfigLayer;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("firefam-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, firefam_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_firefam_folder } => Some(dot_firefam_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_firefam_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_firefam_folder: current_project_dot_firefam_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_firefam_folder {
                ...
                if previous == current_project_dot_firefam_folder
                    || !current_project_dot_firefam_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_firefam_folder = Some(current_project_dot_firefam_folder);
        }
    }
