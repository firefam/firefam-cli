...
/// `$FIREFAM_HOME/config.toml`; profile-v2 callers should resolve their target
...
impl ConfigEditsBuilder {
    pub fn new(firefam_home: &Path) -> Self {
        Self::for_config_path(&firefam_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(firefam_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.firefam_home.join(CONFIG_TOML_FILE).to_path_buf());
