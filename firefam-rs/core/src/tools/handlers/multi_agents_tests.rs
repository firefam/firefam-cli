...
use firefam_protocol::protocol::FileSystemAccessMode;
use firefam_protocol::protocol::FileSystemPath;
use firefam_protocol::protocol::FileSystemSandboxEntry;
use firefam_protocol::protocol::FileSystemSandboxPolicy;
use firefam_protocol::protocol::InitialHistory;
use firefam_protocol::protocol::InterAgentCommunication;
use firefam_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.firefam_home)
        .await
        .expect("firefam home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    firefam_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    let mut config = (*turn.config).clone();
    let provider_info =
        built_in_model_providers(/* firefamai_base_url */ /*firefamai_base_url*/ None)["ollama"]
            .clone();
...
async fn spawn_agent_role_service_tier_falls_back_to_supported_parent_tier() {
    ...
    tokio::fs::create_dir_all(&turn.config.firefam_home)
        .await
        .expect("firefam home should be created");
    let role_config_path = turn.config.firefam_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.firefam.session.new_default_turn().await;
    child_thread
        .firefam
        .session
        .send_event(
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    let first_turn = thread.firefam.session.new_default_turn().await;
    thread
        .firefam
        .session
        .send_event(
    ...
    let second_turn = thread.firefam.session.new_default_turn().await;
    thread
        .firefam
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.firefam.session.new_default_turn().await;
    thread
        .firefam
        .session
        .send_event(
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(FirefamAuth::from_api_key("dummy")),
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.firefam.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.firefam.session.clone();
    ...
    let operator_session = operator.thread.firefam.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.firefam.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.firefam_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    #[allow(deprecated)]
    ...
    expected.firefam_linux_sandbox_exe = turn.firefam_linux_sandbox_exe.clone();
    #[allow(deprecated)]
    {
