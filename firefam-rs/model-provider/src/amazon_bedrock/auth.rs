...
use firefam_api::AuthError;
use firefam_api::AuthProvider;
use firefam_api::SharedAuthProvider;
use firefam_aws_auth::AwsAuthContext;
use firefam_aws_auth::AwsAuthError;
use firefam_aws_auth::AwsRequestToSign;
use firefam_client::Request;
use firefam_client::RequestBody;
use firefam_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_firefam_error)?;
...
fn aws_auth_error_to_firefam_error(error: AwsAuthError) -> FirefamErr {
    FirefamErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    // The Bedrock Mantle front door does not preserve legacy FirefamAI
    ...
    // richer Firefam agent requests fail even though raw Responses requests work.
...
/// AWS SigV4 auth provider for Bedrock Mantle FirefamAI-compatible requests.
#[derive(Debug)]
