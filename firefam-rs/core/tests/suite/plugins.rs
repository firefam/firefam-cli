...
async fn build_analytics_plugin_test_firefam(
    server: &MockServer,
    firefam_home: Arc<TempDir>,
) -> Result<Arc<firefam_core::FirefamThread>> {
...
async fn build_apps_enabled_plugin_test_firefam(
    server: &MockServer,
    firefam_home: Arc<TempDir>,
...
async fn wait_for_sample_mcp_ready(firefam: &firefam_core::FirefamThread) -> Result<()> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let firefam_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(firefam_home.as_ref());
    write_plugin_app_plugin(firefam_home.as_ref());
    let firefam = build_apps_enabled_plugin_test_firefam(
        &server,
        Arc::clone(&firefam_home),
    ...
    firefam
        .submit(Op::UserInput {
            environments: None,
            items: vec![firefam_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(firefam_home.as_ref());
    write_plugin_mcp_plugin(firefam_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(firefam_home.as_ref());

    let firefam =
        build_apps_enabled_plugin_test_firefam(&server, firefam_home, apps_server.chatgpt_base_url)
            .await?;
    wait_for_sample_mcp_ready(&firefam).await?;

    firefam
        .submit(Op::UserInput {
            environments: None,
            items: vec![firefam_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__firefam_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__firefam_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let firefam_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(firefam_home.as_ref());
    let firefam = build_analytics_plugin_test_firefam(&server, firefam_home).await?;
    ...
    let plugin_event = loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/firefam/analytics-events/events")
            .find_map(|request| {
                let payload: serde_json::Value = serde_json::from_slice(&request.body).ok()?;
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "firefam_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(firefam_login::default_client::originator().value)
    );
