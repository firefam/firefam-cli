...
use firefam_api::RetryConfig as ApiRetryConfig;
...
use firefam_protocol::error::EnvVarError;
...
const FIREFAMAI_PROVIDER_NAME: &str = "FirefamAI";
pub const FIREFAMAI_PROVIDER_ID: &str = "openai";
pub const CHATGPT_FIREFAM_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "firefam";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/firefam/firefam-cli/discussions/7782";
...
pub enum WireApi {
    /// The Responses API exposed by FirefamAI at `/v1/responses`.
    #[default]
    Responses,
}
...
pub struct ModelProviderInfo {
    ...
    #[serde(default)]
    pub name: String,
    /// Base URL for the provider's FirefamAI-compatible API.
    ...
    /// Does this provider require an FirefamAI API Key or ChatGPT login token? If true,
    /// user is presented with login screen on first run, and login preference and token/key
    /// are stored in auth.json. If false (which is the default), login screen is skipped,
    /// and API key (if needed) comes from the "env_key" environment variable.
    #[serde(default)]
    pub requires_firefamai_auth: bool,
...
impl ModelProviderInfo {
    pub fn validate(&self) -> std::result::Result<(), String> {
        if self.aws.is_some() {
            ...
            if self.requires_firefamai_auth {
                conflicts.push("requires_firefamai_auth");
            }
    ...
    fn build_header_map(&self) -> FirefamResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> FirefamResult<ApiProvider> {
        let default_base_url = if matches!(
            auth_mode,
            Some(AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens | AuthMode::AgentIdentity)
        ) {
            CHATGPT_FIREFAM_BASE_URL
        } else {
    ...
    pub fn api_key(&self) -> FirefamResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    pub fn create_firefamai_provider(base_url: Option<String>) -> ModelProviderInfo {
        ModelProviderInfo {
            name: FIREFAMAI_PROVIDER_NAME.into(),
            base_url,
            env_key: None,
    ...
    pub fn is_firefamai(&self) -> bool {
        self.name == FIREFAMAI_PROVIDER_NAME
    }
    ...
    pub fn supports_remote_compaction(&self) -> bool {
        self.is_firefamai() || is_azure_responses_provider(&self.name, self.base_url.as_deref())
    }
...
pub fn built_in_model_providers(
    firefamai_base_url: Option<String>,
) -> HashMap<String, ModelProviderInfo> {
    use ModelProviderInfo as P;
    let firefamai_provider = P::create_firefamai_provider(firefamai_base_url);
    let amazon_bedrock_provider = P::create_amazon_bedrock_provider(/*aws*/ None);

    // We do not want to be in the business of adjucating which third-party
    // providers are bundled with Firefam CLI, so we only include the FirefamAI and
    // open source ("oss") providers by default. Users are encouraged to add to
    // `model_providers` in config.toml to add their own providers.
    [
        (FIREFAMAI_PROVIDER_ID, firefamai_provider),
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These FIREFAM_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_firefam_oss_base_url = format!(
        "http://localhost:{firefam_oss_port}/v1",
        firefam_oss_port = std::env::var("FIREFAM_OSS_PORT")
            .ok()
    ...
    let firefam_oss_base_url = std::env::var("FIREFAM_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_firefam_oss_base_url);
    create_oss_provider_with_base_url(&firefam_oss_base_url, wire_api)
}
