...
impl McpProcess {
    pub async fn new(firefam_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(firefam_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = firefam_utils_cargo_bin::cargo_bin("firefam-mcp-server")
            .context("should find binary for firefam-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = firefam_login::default_client::originator().value;
    ...
    pub async fn send_firefam_tool_call(
        &mut self,
        params: FirefamToolCallParam,
    ) -> anyhow::Result<i64> {
        let firefam_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "firefam".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(firefam_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "firefam/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "firefam/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `firefam-mcp-server` child process.
        //
