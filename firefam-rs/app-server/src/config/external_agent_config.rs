...
use firefam_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use firefam_core_plugins::marketplace::find_marketplace_manifest_path;
use firefam_core_plugins::marketplace_add::MarketplaceAddRequest;
use firefam_core_plugins::marketplace_add::add_marketplace;
use firefam_core_plugins::marketplace_add::is_local_marketplace_source;
use firefam_external_agent_migration::build_mcp_config_from_external;
use firefam_external_agent_migration::count_missing_commands;
use firefam_external_agent_migration::count_missing_subagents;
use firefam_external_agent_migration::hook_migration_event_names;
use firefam_external_agent_migration::import_commands;
use firefam_external_agent_migration::import_hooks;
use firefam_external_agent_migration::import_subagents;
use firefam_external_agent_migration::missing_command_names;
use firefam_external_agent_migration::missing_subagent_names;
use firefam_external_agent_sessions::ExternalAgentSessionMigration;
use firefam_external_agent_sessions::detect_recent_sessions;
...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "firefam.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "firefam.external_agent_config.import";
...
impl ExternalAgentConfigService {
    pub(crate) fn new(firefam_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(firefam_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            firefam_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.firefam_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .firefam_home(self.firefam_home.clone())
                .fallback_cwd(Some(self.firefam_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.firefam_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.firefam_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.firefam_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.firefam_home.clone(), request).await;
            ...
            marketplace
                ...
                .filter(|plugin| {
                    plugin.policy.products.as_deref().is_none_or(|products| {
                        Product::Firefam.matches_product_restriction(products)
                    })
                })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Firefam");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    let Some(metrics) = firefam_otel::global() else {
        return;
    };
