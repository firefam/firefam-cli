...
//! interactive-session layers. Direct `firefam login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `firefam-login.log` artifact that
...
use firefam_login::ServerOptions;
use firefam_login::login_with_access_token;
use firefam_login::login_with_api_key;
use firefam_login::logout_with_revoke;
use firefam_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `firefam login` flows.
///
...
/// command produce a durable `firefam-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match firefam_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("firefam-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("firefam_cli=info,firefam_core=info,firefam_login=info")
    });
    ...
    // Direct `firefam login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `firefam login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | firefam login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv FIREFAM_ACCESS_TOKEN | firefam login --with-access-token`.",
...
/// `firefam login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match FirefamAuth::from_auth_storage(
        &config.firefam_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
    )
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout_with_revoke(&config.firefam_home, config.cli_auth_credentials_store_mode).await {
        Ok(true) => {
