...
//!
//! # Relationship to `firefam-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `firefam-app-server-client`,
...
use firefam_analytics::AppServerRpcTransport;
...
use firefam_app_server_protocol::Result;
...
pub use firefam_rollout::StateDbHandle;
pub use firefam_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `firefam-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_firefam_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.firefam_home).await?;
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_firefam_api_key_env)
                .await;
        ...
        let config_manager = ConfigManager::new(
            args.config.firefam_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(firefam_home.path()).await);
        let state_db = firefam_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "firefam-in-process-test".to_string(),
                    title: None,
        ...
        client._test_firefam_home = Some(firefam_home);
        client
    }
