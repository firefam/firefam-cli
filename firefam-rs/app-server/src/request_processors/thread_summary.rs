...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match firefam_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: firefam_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> firefam_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        firefam_protocol::protocol::SessionSource::SubAgent(
...
) -> Option<firefam_app_server_protocol::ActivePermissionProfile> {
    active_permission_profile.map(Into::into)
}

pub(super) fn thread_response_sandbox_policy(
    permission_profile: &firefam_protocol::models::PermissionProfile,
    cwd: &Path,
) -> firefam_app_server_protocol::SandboxPolicy {
    let file_system_policy = permission_profile.file_system_sandbox_policy();
    let sandbox_policy = firefam_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
        &file_system_policy,
