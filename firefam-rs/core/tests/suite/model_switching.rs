...
use firefam_config::types::Personality;
...
use firefam_protocol::firefamai_models::default_input_modalities;
...
fn read_only_user_turn(test: &TestFirefam, items: Vec<UserInput>, model: String) -> Op {
...
fn image_generation_artifact_path(firefam_home: &Path, session_id: &str, call_id: &str) -> PathBuf {
...
async fn model_change_from_image_to_text_strips_prior_image_content() -> Result<()> {
    ...
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &firefam_protocol::models::image_open_tag_text()),
        "second request should preserve the image open tag text"
    );
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &firefam_protocol::models::image_close_tag_text()),
...
async fn thread_rollback_after_generated_image_drops_entire_image_turn_history() -> Result<()> {
    ...
    test.firefam
        ...
        .await?;
    wait_for_event(&test.firefam, |ev| {
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.firefam, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.firefam, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.firefam, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
