...
use firefam_utils_cargo_bin::CargoBinError;
...
use firefam_config::NetworkRequirementsToml;
...
pub use firefam_utils_absolute_path::test_support::PathBufExt;
pub use firefam_utils_absolute_path::test_support::PathExt;
...
pub mod test_firefam;
pub mod test_firefam_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    firefam_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    firefam_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(firefam_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = firefam_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("firefam-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\firefam\AppData\Local\Temp"))
}
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        firefam_linux_sandbox_exe: Some(
            find_firefam_linux_sandbox_exe().expect("should find binary for firefam-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_firefam_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    firefam_utils_cargo_bin::cargo_bin("firefam-linux-sandbox")
}

pub async fn wait_for_event<F>(
    firefam: &FirefamThread,
    predicate: F,
) -> firefam_protocol::protocol::EventMsg
where
    F: FnMut(&firefam_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(firefam, predicate, Duration::from_secs(1)).await
}
...
pub async fn submit_thread_settings(
    ...
    let submission_id = firefam
        ...
        .await?;
    loop {
        let ev = timeout(Duration::from_secs(10), firefam.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(firefam: &FirefamThread, matcher: F) -> T
where
    F: Fn(&firefam_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(firefam, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&firefam_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), firefam.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    firefam_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    firefam_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "FIREFAM_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    firefam_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    firefam_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! firefam_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_firefam_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("firefam-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
