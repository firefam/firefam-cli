...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        firefam_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    use firefam_state::ThreadMetadataBuilder;
    ...
    fn validate_dynamic_tools_accepts_responses_compatible_identifiers() {
        let tools = vec![ApiDynamicToolSpec {
            namespace: Some("Firefam-App_2".to_string()),
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        assert!(err.contains("firefam_app"), "unexpected error: {err}");
        assert!(err.contains("my_tool"), "unexpected error: {err}");
    }
    ...
    fn validate_dynamic_tools_rejects_namespace_not_supported_by_responses() {
        let tools = vec![ApiDynamicToolSpec {
            namespace: Some("firefam.app".to_string()),
        ...
        assert!(err.contains("firefam.app"), "unexpected error: {err}");
        assert!(
            err.contains("Responses API") && err.contains("^[a-zA-Z0-9_-]+$"),
            "unexpected error: {err}"
        );
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(firefam_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn requested_permissions_trust_project_uses_permission_profile_intent() {
        ...
        let full_access_profile = firefam_protocol::models::PermissionProfile::Disabled;
        let workspace_write_profile =
            firefam_protocol::models::PermissionProfile::workspace_write();
        let read_only_profile = firefam_protocol::models::PermissionProfile::read_only();
        let split_write_profile =
            firefam_protocol::models::PermissionProfile::from_runtime_permissions(
    ...
    async fn derive_config_from_params_uses_session_thread_config_model_provider() -> Result<()> {
        ...
        let session_provider = ModelProviderInfo {
            ...
            base_url: Some("http://127.0.0.1:8061/api/firefam".to_string()),
            env_key: None,
            ...
            .load_with_overrides(
                Some(HashMap::from([
                    ...
                    (
                        ...
                        json!({
                            "name": "request",
                            "base_url": "http://127.0.0.1:9999/api/firefam",
                            "wire_api": "responses",
                        }),
                    ),
                ])),
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: firefam_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: firefam_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: firefam_protocol::models::PermissionProfile::Disabled,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            firefam_protocol::protocol::SessionSource::default(),
        );
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(firefam_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(firefam_protocol::protocol::ThreadSource::Subagent),
