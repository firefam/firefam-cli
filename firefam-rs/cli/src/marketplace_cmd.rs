...
#[derive(Debug, Parser)]
#[command(bin_name = "firefam plugin marketplace")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "firefam plugin marketplace add",
    after_help = "Examples:\n  firefam plugin marketplace add ./path/to/marketplace\n  firefam plugin marketplace add owner/repo --ref main\n  firefam plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "firefam plugin marketplace upgrade",
    after_help = "Examples:\n  firefam plugin marketplace upgrade\n  firefam plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "firefam plugin marketplace remove",
    after_help = "Example:\n  firefam plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>) -> Result<()> {
    ...
    let default_install_root = marketplace_install_root(config.firefam_home.as_path());
...
async fn run_upgrade(
    ...
    let config = Config::load_with_cli_overrides(overrides)
        ...
        .context("failed to load configuration")?;
    let firefam_home = find_firefam_home().context("failed to resolve FIREFAM_HOME")?;
    let manager = PluginsManager::new(firefam_home.to_path_buf());
