...
impl RuntimeDbSpec {
    fn path(self, firefam_home: &Path) -> PathBuf {
        firefam_home.join(self.filename)
    }
}
...
impl StateRuntime {
    ...
    ///
    /// This opens (and migrates) the SQLite databases under `firefam_home`,
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(firefam_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = STATE_DB.path(firefam_home.as_path());
        let logs_path = LOGS_DB.path(firefam_home.as_path());
        let goals_path = GOALS_DB.path(firefam_home.as_path());
...
pub fn state_db_path(firefam_home: &Path) -> PathBuf {
    STATE_DB.path(firefam_home)
}
...
pub fn logs_db_path(firefam_home: &Path) -> PathBuf {
    LOGS_DB.path(firefam_home)
}
...
pub fn goals_db_path(firefam_home: &Path) -> PathBuf {
    GOALS_DB.path(firefam_home)
}

pub fn runtime_db_paths(firefam_home: &Path) -> Vec<RuntimeDbPath> {
    RUNTIME_DBS
        .iter()
        .map(|spec| RuntimeDbPath {
            label: spec.label,
            path: spec.path(firefam_home),
        })
        .collect()
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        tokio::fs::create_dir_all(&firefam_home)
            .await
            .expect("create firefam home");
        let path = state_db_path(firefam_home.as_path());
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&firefam_home)
            .await
            .expect("create firefam home");
        let state_path = state_db_path(firefam_home.as_path());
        let pool = SqlitePool::connect_with(
            SqliteConnectOptions::new()
                .filename(&state_path)
                .create_if_missing(true),
