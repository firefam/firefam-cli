...
RUNTIME_PKG_NAME = "firefamai-firefam-cli-bin"
...
def _installed_firefam_path() -> Path:
    try:
        ...
        raise FileNotFoundError(
            ...
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.firefam_bin "
            "explicitly."
        ) from exc

    return bundled_firefam_path()


@dataclass(frozen=True)
class FirefamBinResolverOps:
    installed_firefam_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_firefam_bin_resolver_ops() -> FirefamBinResolverOps:
    return FirefamBinResolverOps(
        installed_firefam_path=_installed_firefam_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_firefam_bin(config: "AppServerConfig", ops: FirefamBinResolverOps) -> Path:
    if config.firefam_bin is not None:
        firefam_bin = Path(config.firefam_bin)
        if not ops.path_exists(firefam_bin):
            raise FileNotFoundError(
                f"Firefam binary not found at {firefam_bin}. Set AppServerConfig.firefam_bin "
                "to a valid binary path."
            )
        return firefam_bin

    return ops.installed_firefam_path()


def _resolve_firefam_bin(config: "AppServerConfig") -> Path:
    return resolve_firefam_bin(config, _default_firefam_bin_resolver_ops())


@dataclass(slots=True)
class AppServerConfig:
    firefam_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "firefam_python_sdk"
    client_title: str = "Firefam Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class AppServerClient:
    """Synchronous typed JSON-RPC client for `firefam app-server` over stdio."""
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            firefam_bin = _resolve_firefam_bin(self.config)
            args = [str(firefam_bin)]
