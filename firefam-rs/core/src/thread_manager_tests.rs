...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.firefam_home = temp_dir.path().join("firefam-home").abs();
    config.cwd = config.firefam_home.abs();
    std::fs::create_dir_all(&config.firefam_home).expect("create firefam home");
...
#[tokio::test]
async fn start_thread_uses_all_default_environments_from_firefam_home() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.firefam_home = temp_dir.path().join("firefam-home").abs();
    config.cwd = config.firefam_home.abs();
    std::fs::create_dir_all(&config.firefam_home).expect("create firefam home");
    std::fs::write(
        config.firefam_home.join("environments.toml"),
        r#"
default = "dev"
...
"#,
    ...
    .expect("runtime paths");
    let environment_manager = Arc::new(
        firefam_exec_server::EnvironmentManager::from_firefam_home(
    ...
    let prompt_items = crate::prompt_debug::build_prompt_input_from_session(
        thread.thread.firefam.session.as_ref(),
        Vec::<UserInput>::new(),
    )
    .await
    .expect("prompt input");
...
#[tokio::test]
async fn explicit_installation_id_skips_firefam_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.firefam_home = temp_dir.path().join("firefam-home").abs();
    config.cwd = config.firefam_home.abs();
    ...
    assert!(!config.firefam_home.join(INSTALLATION_ID_FILENAME).exists());
    assert_eq!(
        thread.thread.firefam.session.installation_id,
        installation_id
    );
...
async fn resumed_thread_keeps_paused_goal_paused() -> anyhow::Result<()> {
    ...
    assert_eq!(firefam_state::ThreadGoalStatus::Paused, goal.status);
    assert!(
        resumed
            .thread
            .firefam
