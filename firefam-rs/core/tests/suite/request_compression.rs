...
async fn request_body_is_zstd_compressed_for_firefam_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/firefam/v1", server.uri());
    ...
    let firefam = builder.build(&server).await?.firefam;
