...
//! Session orchestration stays in `firefam-core`.

use firefam_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_FIREFAM_APPS_META_KEY: &str = "_firefam_apps";
...
pub struct FirefamAppsConnectorAuthFailure {
...
pub struct FirefamAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct FirefamAppsAuthElicitationPlan {
    pub auth_failure: FirefamAppsConnectorAuthFailure,
    pub elicitation: FirefamAppsAuthElicitation,
}

#[derive(Serialize)]
struct FirefamAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<FirefamAppsConnectorAuthFailure> {
    ...
    Some(FirefamAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<FirefamAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(FirefamAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &FirefamAppsConnectorAuthFailure,
) -> FirefamAppsAuthElicitation {
    FirefamAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_FIREFAM_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: FirefamAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("firefam_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &FirefamAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Firefam.",
            auth_failure.connector_name
        ),
        _ => format!(
