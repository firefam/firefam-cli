...
use firefam_otel::GOAL_BLOCKED_METRIC;
use firefam_otel::GOAL_BUDGET_LIMITED_METRIC;
use firefam_otel::GOAL_COMPLETED_METRIC;
use firefam_otel::GOAL_CREATED_METRIC;
use firefam_otel::GOAL_DURATION_SECONDS_METRIC;
use firefam_otel::GOAL_RESUMED_METRIC;
use firefam_otel::GOAL_TOKEN_COUNT_METRIC;
use firefam_otel::GOAL_USAGE_LIMITED_METRIC;
...
use firefam_protocol::protocol::ThreadGoal;
use firefam_protocol::protocol::ThreadGoalStatus;
use firefam_protocol::protocol::ThreadGoalUpdatedEvent;
use firefam_protocol::protocol::TokenUsage;
use firefam_protocol::protocol::validate_thread_goal_objective;
use firefam_rollout::state_db::reconcile_rollout;
use firefam_thread_store::LocalThreadStore;
use firefam_utils_template::Template;
...
pub struct ExternalGoalPreviousGoal {
    goal_id: String,
    status: firefam_state::ThreadGoalStatus,
    objective: String,
}

impl From<&firefam_state::ThreadGoal> for ExternalGoalPreviousStatus {
    fn from(goal: &firefam_state::ThreadGoal) -> Self {
        Self::Existing(ExternalGoalPreviousGoal::from(goal))
    }
}

impl From<&firefam_state::ThreadGoal> for ExternalGoalPreviousGoal {
    fn from(goal: &firefam_state::ThreadGoal) -> Self {
        Self {
...
pub struct ExternalGoalSet {
    pub goal: firefam_state::ThreadGoal,
...
impl Session {
    ...
    pub(crate) async fn set_thread_goal(
                ...
                state_db
                    ...
                    .replace_thread_goal(
                        ...
                        status
                            ...
                            .unwrap_or(firefam_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == firefam_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != firefam_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != firefam_state::ThreadGoalStatus::Active {
    ...
    async fn apply_external_thread_goal_status(self: &Arc<Self>, external_set: ExternalGoalSet) {
        ...
        match status {
            ...
            firefam_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            firefam_state::ThreadGoalStatus::Paused
            | firefam_state::ThreadGoalStatus::Blocked
            | firefam_state::ThreadGoalStatus::UsageLimited
    ...
    fn emit_goal_resumed_metric_if_status_changed(
        &self,
        previous_status: Option<firefam_state::ThreadGoalStatus>,
        goal_status: firefam_state::ThreadGoalStatus,
    ) {
        if goal_status == firefam_state::ThreadGoalStatus::Active
            && matches!(
                previous_status,
                Some(
                    firefam_state::ThreadGoalStatus::Paused
    ...
    fn emit_goal_terminal_metrics_if_status_changed(
        ...
        let counter = match goal.status {
            firefam_state::ThreadGoalStatus::Blocked => GOAL_BLOCKED_METRIC,
            firefam_state::ThreadGoalStatus::UsageLimited => GOAL_USAGE_LIMITED_METRIC,
            firefam_state::ThreadGoalStatus::BudgetLimited => GOAL_BUDGET_LIMITED_METRIC,
            firefam_state::ThreadGoalStatus::Complete => GOAL_COMPLETED_METRIC,
            firefam_state::ThreadGoalStatus::Active | firefam_state::ThreadGoalStatus::Paused => {
                return;
            }
        };
    ...
    ) -> anyhow::Result<Option<firefam_state::ThreadGoalStatus>> {
        let goal = state_db
            .thread_goals()
            .get_thread_goal(self.conversation_id)
            .await?;
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            firefam_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    firefam_state::ThreadGoalStatus::Active => false,
                    firefam_state::ThreadGoalStatus::BudgetLimited => {
                    ...
                    | firefam_state::ThreadGoalStatus::Blocked
                    | firefam_state::ThreadGoalStatus::UsageLimited
                    | firefam_state::ThreadGoalStatus::Complete => true,
                };
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
            ...
            firefam_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == firefam_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != firefam_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        &self,
        state_db: &StateDbHandle,
        mode: firefam_state::ThreadGoalAccountingMode,
        terminal_metric_emission: TerminalMetricEmission,
            ...
            .await?
        {
            ...
            firefam_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
                ...
                .await
            {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == firefam_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
    ...
    async fn goal_continuation_candidate_if_active(
        ...
        if goal.status != firefam_state::ThreadGoalStatus::Active {
...
pub(crate) fn protocol_goal_from_state(goal: firefam_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
pub(crate) fn protocol_goal_status_from_state(
    ...
    match status {
        firefam_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        firefam_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        firefam_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        firefam_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        firefam_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        firefam_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

pub(crate) fn state_goal_status_from_protocol(
    status: ThreadGoalStatus,
) -> firefam_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => firefam_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => firefam_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => firefam_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => firefam_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => firefam_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => firefam_state::ThreadGoalStatus::Complete,
    }
}
