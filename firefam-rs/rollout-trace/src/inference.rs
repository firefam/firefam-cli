...
const INFERENCE_CALL_ID_HEADER: &str = "x-firefam-inference-call-id";
...
///
/// A Firefam turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Firefam turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        firefam_turn_id: FirefamTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                firefam_turn_id: attempt.context.firefam_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps firefam-core focused on
    ...
    /// Records a provider stream that Firefam intentionally stopped consuming.
    ///
...
/// Firefam received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        firefam_turn_id: Some(context.firefam_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.firefam_turn_id, "turn-1");
