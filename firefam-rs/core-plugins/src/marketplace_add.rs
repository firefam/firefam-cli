...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || add_marketplace_sync(firefam_home.as_path(), request))
        .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(firefam_home, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let install_root = marketplace_install_root(firefam_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(firefam_home, &install_root, &install_metadata)?
    {
        let marketplace_name = validate_marketplace_source_root(&existing_root)?;
        record_added_marketplace_entry(firefam_home, &marketplace_name, &install_metadata)?;
    ...
    if let MarketplaceSource::Local { path } = &source {
        ...
        if marketplace_name == FIREFAMAI_CURATED_MARKETPLACE_NAME {
            return Err(MarketplaceAddError::InvalidRequest(format!(
                "marketplace '{FIREFAMAI_CURATED_MARKETPLACE_NAME}' is reserved and cannot be added from this source"
            )));
        }
        if find_marketplace_root_by_name(firefam_home, &install_root, &marketplace_name)?.is_some()
        {
            return Err(MarketplaceAddError::InvalidRequest(format!(
...
mod tests {
    ...
    fn add_marketplace_sync_installs_marketplace_and_updates_config() -> Result<()> {
        ...
        let config =
            fs::read_to_string(firefam_home.path().join(firefam_config::CONFIG_TOML_FILE))?;
    ...
    fn add_marketplace_sync_treats_existing_local_directory_source_as_already_added() -> Result<()>
    {
        ...
        let first_result =
            add_marketplace_sync_with_cloner(firefam_home.path(), request.clone(), {
                |_url, _ref_name, _sparse_paths, _destination| {
                    panic!("git cloner should not be called for local marketplace sources")
                }
            })?;
        let second_result = add_marketplace_sync_with_cloner(firefam_home.path(), request, {
            |_url, _ref_name, _sparse_paths, _destination| {
                panic!("git cloner should not be called for local marketplace sources")
            }
        })?;
