...
fn write_firefamai_curated_marketplace(root: &Path, plugin_names: &[&str]) {
...
fn write_curated_plugin_sha(firefam_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(firefam_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(firefam_home.join(".tmp")) else {
        return false;
    };
...
async fn mount_github_repo_and_ref(server: &MockServer, sha: &str) {
    Mock::given(method("GET"))
        .and(path("/repos/firefamai/plugins"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"{"default_branch":"main"}"#))
        .mount(server)
        .await;
    Mock::given(method("GET"))
        .and(path("/repos/firefamai/plugins/git/ref/heads/main"))
        .respond_with(
...
async fn mount_github_zipball(server: &MockServer, sha: &str, bytes: Vec<u8>) {
    Mock::given(method("GET"))
        .and(path(format!("/repos/firefamai/plugins/zipball/{sha}")))
        .respond_with(
            ResponseTemplate::new(200)
...
async fn run_sync_with_transport_overrides(
    ...
    tokio::task::spawn_blocking(move || {
        sync_firefamai_plugins_repo_with_transport_overrides(
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_firefamai_plugins_repo_via_http(firefam_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
#[test]
fn curated_plugins_repo_path_uses_firefam_home_tmp_dir() {
...
#[cfg(unix)]
#[test]
fn sync_firefamai_plugins_repo_prefers_git_when_available() {
    let tmp = tempdir().expect("tempdir");
...
"#
    ...
    let synced_sha = sync_firefamai_plugins_repo_with_transport_overrides(
        tmp.path(),
...
#[cfg(unix)]
#[test]
fn sync_firefamai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    let tmp = tempdir().expect("tempdir");
    ...
    std::fs::write(
        ...
        r#"{"name":"firefamai-curated","plugins":[{"name":"gmail","source":{"source":"local","path":"./plugins/gmail"}}]}"#,
    )
    ...
    let commit_status = Command::new("git")
        ...
        .arg(&work_repo)
        .arg("-c")
        .arg("user.name=Firefam Test")
        .arg("-c")
        .arg("user.email=firefam@example.com")
        .arg("commit")
        .arg("-m")
        .arg("init")
        .status()
    ...
    let synced_sha =
        sync_firefamai_plugins_repo_via_git(tmp.path(), git_wrapper.to_str().expect("utf8 path"))
            .expect("git sync should succeed");
...
#[tokio::test]
async fn sync_firefamai_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    let tmp = tempdir().expect("tempdir");
...
#[cfg(unix)]
#[tokio::test]
async fn sync_firefamai_plugins_repo_falls_back_to_http_when_git_sync_fails() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
#[cfg(unix)]
#[test]
fn sync_firefamai_plugins_repo_via_git_cleans_up_staged_dir_on_clone_failure() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
"#
    ...
    let err =
        sync_firefamai_plugins_repo_via_git(tmp.path(), git_path.to_str().expect("utf8 path"))
...
#[tokio::test]
async fn sync_firefamai_plugins_repo_via_http_cleans_up_staged_dir_on_extract_failure() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
    let sha = "0123456789abcdef0123456789abcdef01234567";
...
#[tokio::test]
async fn sync_firefamai_plugins_repo_falls_back_to_export_archive_when_no_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
...
#[tokio::test]
async fn sync_firefamai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
    ...
    let root = format!("firefamai-plugins-{sha}");
    writer
