...
use firefam_core::SESSIONS_SUBDIR;
use firefam_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use firefam_core::personality_migration::PersonalityMigrationStatus;
use firefam_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(firefam_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(firefam_home.join("firefam-config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(firefam_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = firefam_home
...
async fn write_archived_session_with_user_event(firefam_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = firefam_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(firefam_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = firefam_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
...
async fn migration_marker_exists_no_sessions_no_change() -> io::Result<()> {
    ...
    assert_eq!(
        tokio::fs::try_exists(temp.path().join("firefam-config.toml")).await?,
        false
    );
    Ok(())
}
...
async fn no_marker_sessions_preserves_existing_config_fields() -> io::Result<()> {
    let temp = TempDir::new()?;
    write_session_with_user_event(temp.path()).await?;
    tokio::fs::write(temp.path().join("firefam-config.toml"), "model = \"gpt-5.4\"\n").await?;
