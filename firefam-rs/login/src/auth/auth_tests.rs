...
use firefam_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let firefam_home = tempdir().unwrap();
...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.firefamai_api_key.as_deref(), Some("sk-new"));
...
async fn login_with_access_token_writes_only_token() {
    ...
    assert!(auth.tokens.is_none(), "tokens should be cleared");
    assert!(
        auth.firefamai_api_key.is_none(),
...
#[tokio::test]
#[serial(firefam_auth_env)]
async fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    let auth = FirefamAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
fn logout_removes_auth_file() -> Result<(), std::io::Error> {
    ...
    let auth_dot_json = AuthDotJson {
        ...
        firefamai_api_key: Some("sk-test-key".to_string()),
        tokens: None,
        last_refresh: None,
        agent_identity: None,
    };
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = FirefamAuth::from_auth_dot_json(
        firefam_home.path(),
...
struct AuthFileParams {
    firefamai_api_key: Option<String>,
    chatgpt_plan_type: Option<String>,
    chatgpt_account_id: Option<String>,
}

fn write_auth_file(params: AuthFileParams, firefam_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(firefam_home);
    let auth_json_data = json!({
        "OPENAI_API_KEY": params.firefamai_api_key,
        "tokens": {
            "id_token": fake_jwt,
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(FIREFAM_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let _access_token_guard = EnvVarGuard::set(FIREFAM_ACCESS_TOKEN_ENV_VAR, &agent_identity);

    let chatgpt_base_url = format!("{}/backend-api", server.uri());
    let _authapi_guard =
        EnvVarGuard::set("FIREFAM_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = super::load_auth(
        firefam_home.path(),
        /*enable_firefam_api_key_env*/ false,
        AuthCredentialsStoreMode::File,
    ...
    let FirefamAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(firefam_home.path()).exists(),
...
#[tokio::test]
#[serial(firefam_auth_env)]
async fn load_auth_keeps_firefam_api_key_env_precedence() {
    let firefam_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(FIREFAM_ACCESS_TOKEN_ENV_VAR, &agent_identity);
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        firefam_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/firefam-backend/agent-identity",
        "aud": "firefam-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let _authapi_guard =
        EnvVarGuard::set("FIREFAM_AGENT_IDENTITY_AUTHAPI_BASE_URL", &chatgpt_base_url);
    let auth = FirefamAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
