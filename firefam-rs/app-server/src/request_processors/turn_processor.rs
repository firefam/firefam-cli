...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = firefam_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if has_any_overrides {
            thread
                .preview_thread_settings_overrides(FirefamThreadSettingsOverrides {
                    cwd: cwd.clone(),
        ...
        let thread_settings = firefam_protocol::protocol::ThreadSettingsOverrides {
            cwd,
        ...
        if turn_has_input {
            ...
            firefam_memories_write::start_memories_startup_task(
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            firefam_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            firefam_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<FirefamThread>)>, JSONRPCErrorError> {
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<FirefamThread>,
