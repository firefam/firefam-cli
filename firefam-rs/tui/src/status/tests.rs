...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .firefam_home(temp_home.path().to_path_buf())
...
async fn status_model_provider_uses_bedrock_runtime_base_url_and_gates_usage_link() {
    ...
    assert!(
        !rendered.contains("https://chatgpt.com/firefam/settings/usage"),
        "expected /status to hide ChatGPT usage link for Bedrock, got: {rendered}"
    );

    config.model_provider_id = "firefamai-proxy".to_string();
    config.model_provider = ModelProviderInfo {
        name: "FirefamAI Proxy".to_string(),
        base_url: Some("https://firefamai-proxy.example/v1".to_string()),
    ...
    assert!(
        rendered.contains("https://chatgpt.com/firefam/settings/usage"),
        "expected /status to show ChatGPT usage link for FirefamAI-auth proxy, got: {rendered}"
    );
}
