...
use firefam_state::BackfillState;
use firefam_state::BackfillStats;
use firefam_state::BackfillStatus;
use firefam_state::DB_ERROR_METRIC;
use firefam_state::DB_METRIC_BACKFILL;
use firefam_state::DB_METRIC_BACKFILL_DURATION_MS;
use firefam_state::ExtractionOutcome;
use firefam_state::ThreadMetadataBuilder;
use firefam_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = firefam_otel::global();
    ...
    let sessions_root = firefam_home.join(SESSIONS_SUBDIR);
    let archived_root = firefam_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(firefam_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(firefam_home: &Path, path: &Path) -> String {
    path.strip_prefix(firefam_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
