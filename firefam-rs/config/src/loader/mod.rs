...
use firefam_file_system::ExecutorFileSystem;
...
/// - system    `/etc/firefam/requirements.toml` (Unix) or
///   `%ProgramData%\FirefamAI\Firefam\requirements.toml` (Windows)
///
...
///
/// See https://support.firefam.ai/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, firefam_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(firefam_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, firefam_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so firefam_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, firefam_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/firefam/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_firefam_system_dir() -> PathBuf {
    ...
    program_data.join("FirefamAI").join("Firefam")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_firefam_system_dir().join("requirements.toml");
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_firefam_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let firefam_home_abs = AbsolutePathBuf::from_absolute_path(firefam_home)?;
    let firefam_home_normalized = normalize_path(firefam_home_abs.as_path())
        .unwrap_or_else(|_| firefam_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
...
/// from a file like `/etc/firefam/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Firefam to function correctly.
