...
const FIREFAMAI_CURATED_TAB_ID: &str = "marketplace:firefamai-curated";
...
const APPS_HELP_ARTICLE_URL: &str =
    "https://support.firefam.ai/en/articles/11487775-apps-in-chatgpt";
...
impl ChatWidget {
    ...
    fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Firefam.".dim(),
        ));
    ...
    fn plugins_popup_params(
        ...
        let curated_installed = curated_entries
            ...
            .count();
        tabs.push(SelectionTab {
            id: FIREFAMAI_CURATED_TAB_ID.to_string(),
            label: "FirefamAI Curated".to_string(),
            header: plugins_header(
                "FirefamAI Curated marketplace.".to_string(),
                format!(
                    "Installed {curated_installed} of {curated_total} FirefamAI Curated plugins."
                ),
            ),
            items: self.plugin_selection_items(
                curated_entries,
                /*include_marketplace_names*/ false,
                "No FirefamAI Curated plugins available",
                "No FirefamAI Curated plugins available.",
            ),
        });
        ...
        let mut additional_marketplaces: Vec<&PluginMarketplaceEntry> = marketplaces
            .iter()
            .copied()
            .filter(|marketplace| marketplace.name != FIREFAMAI_CURATED_MARKETPLACE_NAME)
            .collect();
            ...
            let actions: Vec<SelectionAction> = if let Some(marketplace_path) = marketplace_path {
                vec![Box::new(move |tx| {
                    ...
                    tx.send(AppEvent::FetchPluginDetail {
                        cwd: cwd.clone(),
                        params: firefam_app_server_protocol::PluginReadParams {
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(firefam_app_server_protocol::HookEventName, usize)>::new();
