...
use core_test_support::test_firefam::TestFirefam;
...
use firefam_core::ModelClient;
use firefam_core::NewThread;
use firefam_core::Prompt;
use firefam_core::ResponseEvent;
use firefam_core::ThreadManager;
use firefam_core::resolve_installation_id;
use firefam_core::thread_store_from_config;
...
use firefam_protocol::config_types::ModelProviderAuthInfo;
...
use firefam_protocol::models::DEFAULT_IMAGE_DETAIL;
use firefam_protocol::models::FunctionCallOutputContentItem;
use firefam_protocol::models::FunctionCallOutputPayload;
use firefam_protocol::models::ImageDetail;
use firefam_protocol::models::LocalShellAction;
use firefam_protocol::models::LocalShellExecAction;
use firefam_protocol::models::LocalShellStatus;
use firefam_protocol::models::MessagePhase;
use firefam_protocol::models::ReasoningItemContent;
use firefam_protocol::models::ReasoningItemReasoningSummary;
use firefam_protocol::models::ResponseItem;
use firefam_protocol::models::WebSearchAction;
...
use firefam_protocol::protocol::SessionMeta;
...
/// Writes an `auth.json` into the provided `firefam_home` with the specified parameters.
/// Returns the fake JWT string written to `tokens.id_token`.
#[expect(clippy::unwrap_used)]
fn write_auth_json(
    firefam_home: &TempDir,
    firefamai_api_key: Option<&str>,
    ...
    let auth_json = json!({
        "OPENAI_API_KEY": firefamai_api_key,
        "tokens": tokens,
    ...
    std::fs::write(
        firefam_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match firefam_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path()) {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = firefam_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![firefam_protocol::models::ContentItem::InputText {
    ...
    let prior_system = firefam_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![firefam_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = firefam_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Firefam to resume from our file
    let firefam_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_firefam()
        .with_home(firefam_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, firefam_home, session_path.clone())
        .await
        .expect("resume conversation");
    let firefam = test.firefam.clone();
    ...
    firefam
        ...
        .await
        .unwrap();
    wait_for_event(&firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let firefam_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_firefam().with_model("gpt-5.4");
    let test = builder
        .resume(&server, firefam_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_firefam().with_auth(FirefamAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.firefam_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-firefam-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let firefam_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&firefam_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = firefam_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        firefam_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            FirefamAuth::from_api_key("unused-api-key"),
        )),
        thread_id.into(),
        thread_id,
    ...
    let mut stream = client_session
        .stream(
            ...
            &firefam_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* firefamai_base_url */ /*firefamai_base_url*/ None)["openai"]
            .clone();
    model_provider.base_url = Some(format!("{}/api/firefam", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_firefam()
        .with_auth(create_dummy_firefam_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/firefam/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match FirefamAuth::from_auth_storage(
        firefam_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => firefam_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No FirefamAuth found in firefam_home"),
        Err(e) => panic!("Failed to load FirefamAuth: {e}"),
    };
    let installation_id = resolve_installation_id(&config.firefam_home)
        .await
...
async fn skills_append_to_developer_message() {
    ...
    let firefam_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = firefam_home.path().join("skills/demo");
    ...
    let firefam_home_path = firefam_home.path().to_path_buf();
    let mut builder = test_firefam()
        .with_home(firefam_home.clone())
        .with_auth(FirefamAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
    ...
    let _firefam_home_guard = firefam_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let firefam_home_parent = TempDir::new().unwrap();
    let long_home_parent = firefam_home_parent
        .path()
        .join("firefam-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let firefam_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = firefam_home.path().join("skills");
    for index in 0..12 {
    ...
    let _firefam_home_guard = firefam_home;
    let _firefam_home_parent_guard = firefam_home_parent;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestFirefam { firefam, .. } = test_firefam()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestFirefam { firefam, .. } = test_firefam().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    let TestFirefam {
        firefam, config, ..
    } = test_firefam().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let provider = ModelProviderInfo {
        name: "azure".into(),
        base_url: Some(format!("{}/firefamai", server.uri())),
        env_key: None,
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
    ...
    let model_info =
        firefam_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = firefam_core::test_support::auth_manager_from_auth(
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/firefamai/responses");
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-firefam-primary-used-percent", "12.5")
        .insert_header("x-firefam-secondary-used-percent", "40.0")
        .insert_header("x-firefam-primary-window-minutes", "10")
        .insert_header("x-firefam-secondary-window-minutes", "60")
        .insert_header("x-firefam-primary-reset-at", "1704069000")
        .insert_header("x-firefam-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_firefam()
        .with_auth(FirefamAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    wait_for_event(&firefam, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-firefam-primary-used-percent", "100.0")
        .insert_header("x-firefam-secondary-used-percent", "87.5")
        .insert_header("x-firefam-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-firefam-primary-window-minutes", "15")
        .insert_header("x-firefam-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_firefam();
    let firefam_fixture = builder.build(&server).await?;
    let firefam = firefam_fixture.firefam.clone();
    ...
    let token_event = wait_for_event(&firefam, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&firefam, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&firefam, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&firefam, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = FirefamErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn azure_overrides_assign_properties_used_for_responses_url() {
    ...
    // Expect POST to /firefamai/responses with api-version query param
    Mock::given(method("POST"))
        .and(path("/firefamai/responses"))
...
fn create_dummy_firefam_auth() -> FirefamAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Firefam sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
