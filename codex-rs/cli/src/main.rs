...
struct LoginCommand {
    ...
    #[arg(
        ...
        help = "Read the access token from stdin (e.g. `printenv CODEX_ACCESS_TOKEN | firefam login --with-access-token`)"
    )]
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Firefam installation method. Please update manually: https://developers.openai.com/codex/cli/"
            );
        };
        run_update_action(action)
    }
...
mod tests {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let codex_home = find_codex_home().expect("firefam home");
        codex_app_server::app_server_control_socket_path(&codex_home)
