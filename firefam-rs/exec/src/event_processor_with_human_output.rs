...
use firefam_protocol::num_format::format_with_separators;
use firefam_protocol::protocol::SessionConfiguredEvent;
use firefam_utils_sandbox_summary::summarize_permission_profile;
use owo_colors::OwoColorize;
use owo_colors::Style;

use crate::event_processor::EventProcessor;
use crate::event_processor::FirefamStatus;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "firefam".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> FirefamStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        firefam_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        firefam_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => FirefamStatus::Running,
