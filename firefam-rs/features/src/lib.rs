...
pub enum Feature {
    ...
    ChildAgentsMd,
    /// Compress request bodies (zstd) when sending streaming requests to firefam-backend.
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://support.firefam.ai/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        id: Feature::FirefamGitCommit,
        key: "firefam_git_commit",
        stage: Stage::Removed,
        default_enabled: false,
    },
    FeatureSpec {
