...
use firefam_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    async fn new() -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(firefam_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<FirefamThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<FirefamThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<FirefamThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, FirefamErr::ThreadNotFound(id) if id == thread_id);
}
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread
                ...
                .session
                .input_queue
                .has_pending_input(&thread.firefam.session.active_turn)
                .await
            {
                break;
            }
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.firefam.session.new_default_turn().await;
    ...
    let history = child_thread.firefam.session.clone_history().await;
    ...
    assert_eq!(
        serde_json::to_value(child_thread.firefam.session.reference_context_item().await)
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    let queued_turn_context = parent_thread.firefam.session.new_default_turn().await;
    parent_thread
        .firefam
        .session
        .record_conversation_items(
    ...
    let triggered_turn_context = parent_thread.firefam.session.new_default_turn().await;
    parent_thread
        .firefam
        .session
        .record_conversation_items(
    ...
    parent_thread
        ...
        .await;
    let spawn_turn_context = parent_thread.firefam.session.new_default_turn().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let FirefamErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected FirefamErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let FirefamErr::AgentLimitReached { max_threads } = err else {
        panic!("expected FirefamErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.firefam.session.new_default_turn().await;
    tester_thread
        .firefam
        .session
        .send_event(
