...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "firefam@example.com"])?;
    run_git(root, &["config", "user.name", "Firefam Tests"])?;
...
fn marketplace_install_root(firefam_home: &Path) -> std::path::PathBuf {
    firefam_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(firefam_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&firefam_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(firefam_home.path())?;

    let mut mcp = McpProcess::new(firefam_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(firefam_home.path(), "debug")?;
    let tools_root = expected_installed_root(firefam_home.path(), "tools")?;
