...
use firefam_ansi_escape::ansi_escape_line;
...
use firefam_app_server_protocol::HooksListEntry;
...
use firefam_app_server_protocol::MergeStrategy;
...
use firefam_app_server_protocol::SkillErrorInfo;
...
use firefam_models_manager::model_presets::HIDE_GPT_5_1_FIREFAM_MAX_MIGRATION_PROMPT_CONFIG;
use firefam_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use firefam_protocol::permissions::FileSystemSandboxKind;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        firefam_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&firefam_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&firefam_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[firefam_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&firefam_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<firefam_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn session_summary(
    ...
    let resume_hint = resumable_thread.as_ref().and_then(|thread| {
        firefam_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
    });
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.firefam_error_info,
        Some(AppServerFirefamErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        let requires_firefamai_auth = bootstrap.requires_firefamai_auth;
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            firefam_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("firefam.status_line", /*inc*/ 1, &[]);
        }
...
See the Firefam keymap documentation for supported actions and examples."
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.firefam_home.clone();
        ...
        if requires_firefamai_auth && has_chatgpt_account {
