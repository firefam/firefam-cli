...
use firefam_core::config::AgentRoleConfig;
...
fn write_home_skill(firefam_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = firefam_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestFirefam) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_firefam::TestFirefamBuilder,
    ) -> core_test_support::test_firefam::TestFirefamBuilder,
) -> Result<(
    TestFirefam,
    String,
    ...
    let mut builder = configure_test(test_firefam().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.firefam_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
