...
use firefam_app_server_protocol::ForcedChatgptWorkspaceIds as ApiForcedChatgptWorkspaceIds;
use firefam_app_server_protocol::Tools;
use firefam_app_server_protocol::UserSavedConfig;
...
use firefam_protocol::config_types::WebSearchToolConfig;
...
use firefam_utils_path::normalize_for_path_comparison;
...
pub struct ConfigToml {
    ...
    /// Base URL override for the built-in `firefamai` model provider.
    pub firefamai_base_url: Option<String>,
    ...
    /// directories for `.firefam` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    pub allow_firefam_version_mismatch: Option<bool>,
...
pub use firefam_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use firefam_protocol::protocol::RealtimeVoice;
...
Built-in providers cannot be overridden. Rename your custom provider (for example, `firefamai-custom`).",
            conflicts.join(", ")
        ))
    }
}
