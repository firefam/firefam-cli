...
This repository uses Bazel to build the Rust workspace under `firefam-rs`.
...
- `rules_rs` imports third-party crates from `firefam-rs/Cargo.toml` and
  `firefam-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `firefam_rust_crate`, which wraps `rust_library`,
...
- Each crate in `firefam-rs/*/BUILD.bazel` typically uses `firefam_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `firefam_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `firefam_rust_crate` to configure it.
