...
impl AppServerClient {
    pub fn spawn(
        ...
        let mut child = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{firefam_bin}` app-server"))?;
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        let request = ClientRequest::Initialize {
            ...
            params: firefam_app_server_protocol::InitializeParams {
        ...
        let _parsed: firefam_app_server_protocol::InitializeResponse =
...
fn handle_server_request(
    ...
    let Ok(server_request) = firefam_app_server_protocol::ServerRequest::try_from(request) else {
        return Ok(());
    };

    match server_request {
        firefam_app_server_protocol::ServerRequest::CommandExecutionRequestApproval {
            request_id,
            ..
        } => {
            let response = firefam_app_server_protocol::CommandExecutionRequestApprovalResponse {
                decision: CommandExecutionApprovalDecision::Decline,
            };
            send_jsonrpc_response(stdin, request_id, response)
        }
        firefam_app_server_protocol::ServerRequest::FileChangeRequestApproval {
            request_id,
            ..
        } => {
            let response = firefam_app_server_protocol::FileChangeRequestApprovalResponse {
