...
use crate::test_firefam::TestFirefam;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = firefam_utils_cargo_bin::cargo_bin("firefam-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `firefam-execve-wrapper` binary");
        return Ok(None);
    };
