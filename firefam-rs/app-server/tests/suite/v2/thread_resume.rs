...
use firefam_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use firefam_protocol::protocol::SessionSource as RolloutSessionSource;
...
const FIREFAM_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Firefam, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_goal_get_rejects_unmaterialized_thread() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_without_managed_config(firefam_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(firefam_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, firefam_home.path()).await?;
...
fn set_thread_source_on_fake_rollout(
    ...
    let path = rollout_path(firefam_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in [
        "firefam_chatgpt_android_remote",
        "firefam_chatgpt_ios_remote",
    ] {
...
) -> Result<firefam_app_server_protocol::Thread> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let firefam_home = TempDir::new()?;
    create_config_toml(firefam_home.path(), &server.uri())?;
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(firefam_home, filename_ts, conversation_id);
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(firefam_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            firefam_state::ThreadGoalAccountingMode::ActiveOnly,
...
async fn thread_goal_clear_deletes_goal_and_notifies() -> Result<()> {
    ...
    .await??;
    let get: firefam_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(firefam_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
"#,
    ...
    let repo_path = firefam_home.path().join("repo");
    ...
    let rollout_path = rollout_path(firefam_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let firefam_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(firefam_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(firefam_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(firefam_home.path()).await?;
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(firefam_home.path(), "2025-01-01T00-00-00", &thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let firefam_home = tmp.path().join("firefam_home");
    std::fs::create_dir(&firefam_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&firefam_home, &server.uri())?;

    let mut primary = McpProcess::new(&firefam_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![firefam_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(firefam_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(firefam_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(firefam_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(firefam_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(firefam_home).await?;
    ...
    let mut second_mcp = McpProcess::new(firefam_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(FIREFAM_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn setup_rollout_fixture(firefam_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(firefam_home, server_uri)?;
