...
#[test]
fn try_new_rejects_relative_firefam_home() {
    let err = PluginStore::try_new(PathBuf::from("relative"))
        .expect_err("relative firefam home should fail");
...
fn install_with_version_uses_requested_cache_version() {
    ...
    write_plugin(tmp.path(), "sample-plugin", "sample-plugin");
    let plugin_id =
        PluginId::new("sample-plugin".to_string(), "firefamai-curated".to_string()).unwrap();
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/firefamai-curated/sample-plugin/{plugin_version}"
    ));
    assert_eq!(
        result,
        PluginInstallResult {
