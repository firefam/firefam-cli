...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.firefam_linux_sandbox_exe = Some(PathBuf::from("/tmp/firefam-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/firefam-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.firefam_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/firefam-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/firefam-execve-wrapper"))
    );
}
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(
            &skills_input,
            Some(Arc::clone(&firefam_exec_server::LOCAL_FS)),
        )
        .await;
    let skill = outcome
        .skills
