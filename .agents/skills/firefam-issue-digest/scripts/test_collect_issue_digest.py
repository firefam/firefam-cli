...
def test_build_search_queries_uses_each_owner_and_kind_label():
    ...
    queries = collect_issue_digest.build_search_queries(
        "firefamai/firefam", ["tui", "exec"], since
    )

    assert queries == [
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:tui label:bug",
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:tui label:enhancement",
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:exec label:bug",
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:exec label:enhancement",
    ]
...
def test_build_search_queries_can_scan_all_labels():
    ...
    queries = collect_issue_digest.build_search_queries(
        "firefamai/firefam", [], since, all_labels=True
    )

    assert queries == [
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:bug",
        "repo:firefamai/firefam is:issue updated:>=2026-04-25 label:enhancement",
    ]
...
def test_fetch_comments_uses_since_filter_and_page_cap(monkeypatch):
    ...
    payload = collect_issue_digest.fetch_comments(
        "firefamai/firefam", 123, since=since, max_pages=1
    )
    ...
    assert calls == [
        [
            "api",
            "repos/firefamai/firefam/issues/123/comments?since=2026-04-25T00%3A00%3A00Z&per_page=100&page=1",
        ]
    ]
...
def test_issue_description_prefers_title_over_body_noise():
    issue = {
        "title": "Firefam.app GUI: MCP child processes not reaped after task completion",
