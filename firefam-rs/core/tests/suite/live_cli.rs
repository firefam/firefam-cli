...
//! Optional smoke tests that hit the real FirefamAI /v1/responses endpoint. They are `#[ignore]` by
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::new(firefam_utils_cargo_bin::cargo_bin("firefam").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn firefam-rs");
