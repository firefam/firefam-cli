...
use firefam_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical Firefam tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub firefam_turn_id: FirefamTurnId,
...
/// Raw invocation payload for the canonical Firefam tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            firefam_turn_id: invocation.firefam_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == firefam_code_mode::PUBLIC_TOOL_NAME
}
