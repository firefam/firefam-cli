...
pub fn write_mock_responses_config_toml(
    ...
    let requires_line = match requires_firefamai_auth {
        Some(true) => "requires_firefamai_auth = true\n".to_string(),
        Some(false) | None => String::new(),
    };
    let provider_name = if matches!(requires_firefamai_auth, Some(true)) {
        "FirefamAI"
    } else {
...
"#
    ...
    let firefamai_base_url_line = if model_provider_id == "openai" {
        format!("firefamai_base_url = \"{server_uri}/v1\"\n")
    } else {
        String::new()
    };
