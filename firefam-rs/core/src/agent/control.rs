...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> FirefamThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.firefam.thread_config_snapshot().await;
    ...
    ) -> FirefamResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(FirefamErr::Fatal(
        ...
        let multi_agent_v2_usage_hint_texts_to_filter: Vec<String> =
            if let Some(parent_thread) = parent_thread.as_ref() {
                if parent_thread.enabled(Feature::MultiAgentV2) {
                    let parent_config = parent_thread.firefam.session.get_config().await;
                    [
                        parent_config
                            .multi_agent_v2
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let history = stored_thread
            .history
            .ok_or_else(|| FirefamErr::ThreadNotFound(thread_id))?
            .items;
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> FirefamResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: FirefamResult<String>,
    ) -> FirefamResult<String> {
        if matches!(result, Err(FirefamErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> FirefamResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.firefam.session.ensure_rollout_materialized().await;
            thread.firefam.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> FirefamResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> FirefamResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(FirefamErr::ThreadNotFound(_)) | Err(FirefamErr::InternalAgentDied) => {
                }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(FirefamErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> FirefamResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> FirefamResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> FirefamResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| FirefamErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_shell_snapshot_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        parent_thread.firefam.session.user_shell().shell_snapshot()
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        Some(Arc::clone(
            &parent_thread.firefam.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> FirefamResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> FirefamResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
