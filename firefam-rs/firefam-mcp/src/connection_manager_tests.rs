use super::*;
use crate::declared_firefamai_file_input_param_names;
use crate::elicitation::ElicitationRequestManager;
use crate::elicitation::elicitation_is_rejected_by_policy;
use crate::firefam_apps::FIREFAM_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use crate::firefam_apps::FirefamAppsToolsCacheContext;
use crate::firefam_apps::load_startup_cached_firefam_apps_tools_snapshot;
use crate::firefam_apps::read_cached_firefam_apps_tools;
use crate::firefam_apps::write_cached_firefam_apps_tools;
...
fn create_firefam_apps_tools_cache_context(
    firefam_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> FirefamAppsToolsCacheContext {
    FirefamAppsToolsCacheContext {
        firefam_home,
        user_key: FirefamAppsToolsCacheKey {
...
#[test]
fn declared_firefamai_file_fields_treat_names_literally() {
    ...
    assert_eq!(
        declared_firefamai_file_input_param_names(Some(meta)),
        vec![
            "file".to_string(),
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "upload").tool;
...
#[test]
fn firefam_apps_tools_cache_is_overwritten_by_last_write() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "two")];

    write_cached_firefam_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_firefam_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_firefam_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 = read_cached_firefam_apps_tools(&cache_context)
        .expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn firefam_apps_tools_cache_is_scoped_per_user() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "two")];

    write_cached_firefam_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_firefam_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_firefam_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_firefam_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn firefam_apps_tools_cache_filters_disallowed_connectors() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_firefam_apps_tools(&cache_context, &tools);
    let cached =
        read_cached_firefam_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn firefam_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": FIREFAM_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(FIREFAM_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_firefam_apps_tools(&cache_context).is_none());
}

#[test]
fn firefam_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_firefam_apps_tools_loads_from_disk_cache() {
    let firefam_home = tempdir().expect("tempdir");
    let cache_context = create_firefam_apps_tools_cache_context(
        firefam_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_firefam_apps_tools(&cache_context, &cached_tools);

    let startup_snapshot = load_startup_cached_firefam_apps_tools_snapshot(
        FIREFAM_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    );
    ...
    assert_eq!(startup_tools[0].server_name, FIREFAM_APPS_MCP_SERVER_NAME);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__firefam_apps__", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, FIREFAM_APPS_MCP_SERVER_NAME);
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let (manager, cancel_token) = McpConnectionManager::new(
        ...
        firefam_home.path().to_path_buf(),
        FirefamAppsToolsCacheKey {
            account_id: None,
            chatgpt_user_id: None,
            is_workspace_account: false,
        },
        /*host_owned_firefam_apps_enabled*/ false,
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = FIREFAM_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `firefam mcp login {server_name}`."
    );
