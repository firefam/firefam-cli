...
use firefam_app_server_protocol::Model;
use firefam_app_server_protocol::ModelListParams;
use firefam_app_server_protocol::ModelListResponse;
use firefam_app_server_protocol::ModelServiceTier;
use firefam_app_server_protocol::ModelUpgradeInfo;
use firefam_app_server_protocol::ReasoningEffortOption;
...
fn expected_visible_models() -> Vec<Model> {
    ...
    let mut presets = ModelPreset::filter_by_auth(
        firefam_core::test_support::all_model_presets().clone(),
        /*chatgpt_mode*/ false,
    );
...
firefamai_base_url = "{server_uri}/v1"
"#
        ),
    )?;
    write_chatgpt_auth(
