...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::FirefamOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::FirefamOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::FirefamOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::FirefamOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(firefam_config::types::KeybindingsSpec::One(
        firefam_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
        ...
        ServerNotification::ThreadGoalUpdated(
            firefam_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: firefam_app_server_protocol::ThreadGoal {
                    ...
                    status: firefam_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/firefam-test-rollout.jsonl");
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::FirefamOp(Op::OverrideTurnContext {
