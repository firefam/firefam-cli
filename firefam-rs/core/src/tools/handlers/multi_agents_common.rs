...
use firefam_protocol::protocol::CollabAgentRef;
use firefam_protocol::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: FirefamErr) -> FunctionCallError {
    match err {
        FirefamErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        FirefamErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: FirefamErr) -> FunctionCallError {
    match err {
        FirefamErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.firefam_linux_sandbox_exe = turn.firefam_linux_sandbox_exe.clone();
    #[allow(deprecated)]
...
fn find_spawn_agent_model_name(
    available_models: &[firefam_protocol::firefamai_models::ModelPreset],
    requested_model: &str,
