...
/// Remote endpoint used by the FirefamAI-compatible model manager.
///
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_firefam_backend);
...
/// FirefamAI-compatible model manager backed by bundled models, cache, and `/models`.
#[derive(Debug)]
...
impl OpenAiModelsManager {
    /// Construct an FirefamAI-compatible remote model manager.
    pub fn new(
        firefam_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Self {
        let cache_path = firefam_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_firefam_backend().await || self.endpoint_client.has_command_auth()
    }
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            firefam_otel::start_global_timer("firefam.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
