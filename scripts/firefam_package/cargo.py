"""Cargo builds for source-built Firefam package artifacts."""
...
FIREFAM_RS_ROOT = REPO_ROOT / "firefam-rs"
...
class SourceBuildOutputs:
    entrypoint_bin: Path
    bwrap_bin: Path | None
    firefam_command_runner_bin: Path | None
    firefam_windows_sandbox_setup_bin: Path | None
...
def build_source_binaries(
    ...
    *,
    ...
    if binaries:
        ...
        print("+", " ".join(cmd))
        subprocess.run(cmd, cwd=FIREFAM_RS_ROOT, check=True)
    ...
    outputs = SourceBuildOutputs(
        ...
        firefam_command_runner_bin=(
            output_dir / "firefam-command-runner.exe" if spec.is_windows else None
        ),
        firefam_windows_sandbox_setup_bin=(
            output_dir / "firefam-windows-sandbox-setup.exe" if spec.is_windows else None
        ),
    )
...
def cargo_target_dir() -> Path:
    ...
    if target_dir is None:
        return FIREFAM_RS_ROOT / "target"

    path = Path(target_dir)
    if path.is_absolute():
        return path

    return FIREFAM_RS_ROOT / path
...
def validate_source_outputs(outputs: SourceBuildOutputs) -> None:
    for path in [
        ...
        outputs.bwrap_bin,
        outputs.firefam_command_runner_bin,
        outputs.firefam_windows_sandbox_setup_bin,
    ]:
