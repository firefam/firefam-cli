...
pub(super) async fn test_config() -> Config {
    ...
    let firefam_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_firefam_home(firefam_home.clone(), Vec::new())
            .await
            .expect("config");
    config.firefam_home = firefam_home.abs();
    config.sqlite_home = firefam_home.clone();
    config.log_dir = firefam_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::FirefamOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Firefam to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> firefam_app_server_protocol::TokenUsageBreakdown {
    firefam_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    firefam_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: firefam_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    firefam_error_info: Option<FirefamErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = firefam_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: firefam_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            firefam_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("firefam-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        name: FIREFAMAI_CURATED_MARKETPLACE_NAME.to_string(),
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(firefam_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    firefam_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: firefam_app_server_protocol::HookEventName,
    status: firefam_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<firefam_app_server_protocol::HookOutputEntry>,
) -> firefam_app_server_protocol::HookRunSummary {
    firefam_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: firefam_app_server_protocol::HookHandlerType::Command,
        execution_mode: firefam_app_server_protocol::HookExecutionMode::Sync,
        scope: firefam_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: firefam_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            firefam_app_server_protocol::HookRunStatus::Completed
                | firefam_app_server_protocol::HookRunStatus::Failed
                | firefam_app_server_protocol::HookRunStatus::Blocked
                | firefam_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            firefam_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                firefam_app_server_protocol::HookOutputEntry {
                    kind: firefam_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                firefam_app_server_protocol::HookOutputEntry {
                    kind: firefam_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: firefam_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        firefam_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        firefam_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        firefam_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        firefam_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        firefam_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        firefam_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        firefam_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        firefam_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        firefam_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
