...
use crate::firefamai_models::ReasoningEffort;
...
/// Serialized with lowercase values to match the FirefamAI API.
#[derive(
    Hash,
    Debug,
    Serialize,
...
/// Validated plain profile-v2 name used to select `$FIREFAM_HOME/<name>.config.toml`.
...
mod tests {
    ...
    fn web_search_tool_config_merge_prefers_overlay_values() {
        let base = WebSearchToolConfig {
            ...
            allowed_domains: Some(vec!["firefam.ai".to_string()]),
