...
use firefam_config::McpServerTransportConfig;
...
use firefam_protocol::mcp::Resource;
use firefam_protocol::mcp::ResourceTemplate;
use firefam_protocol::mcp::Tool;
...
use crate::firefam_apps::firefam_apps_tools_cache_key;
use crate::runtime::McpRuntimeEnvironment;
use crate::server::EffectiveMcpServer;

pub const FIREFAM_APPS_MCP_SERVER_NAME: &str = "firefam_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const FIREFAM_CONNECTORS_TOKEN_ENV_VAR: &str = "FIREFAM_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `firefam_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `firefam-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Optional path to `firefam-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn with_firefam_apps_mcp(
    mut servers: HashMap<String, EffectiveMcpServer>,
    auth: Option<&FirefamAuth>,
    config: &McpConfig,
) -> HashMap<String, EffectiveMcpServer> {
    if host_owned_firefam_apps_enabled(config, auth) {
        servers.insert(
            FIREFAM_APPS_MCP_SERVER_NAME.to_string(),
            EffectiveMcpServer::configured(firefam_apps_mcp_server_config(config)),
        );
    } else {
        servers.remove(FIREFAM_APPS_MCP_SERVER_NAME);
    }
    servers
}

pub fn host_owned_firefam_apps_enabled(config: &McpConfig, auth: Option<&FirefamAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(FirefamAuth::uses_firefam_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    with_firefam_apps_mcp(servers, auth, config)
}
...
pub async fn read_mcp_resource(
    ...
    let host_owned_firefam_apps_enabled = host_owned_firefam_apps_enabled(config, auth);
    ...
    let (manager, cancel_token) = McpConnectionManager::new(
        ...
        firefam_apps_tools_cache_key(auth),
...
pub(crate) fn firefam_apps_mcp_url(config: &McpConfig) -> String {
    firefam_apps_mcp_url_for_base_url(
...
fn firefam_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(FIREFAM_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(FIREFAM_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(FIREFAM_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_firefam_apps_base_url(base_url: &str) -> String {
...
fn firefam_apps_mcp_url_for_base_url(
    base_url: &str,
    apps_mcp_path_override: Option<&str>,
) -> String {
    let base_url = normalize_firefam_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/firefam") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/firefam"), "apps")
    };
...
fn firefam_apps_mcp_server_config(config: &McpConfig) -> McpServerConfig {
    let url = firefam_apps_mcp_url(config);
    let http_headers = config.apps_mcp_product_sku.as_ref().map(|product_sku| {
        HashMap::from([("X-FirefamAI-Product-Sku".to_string(), product_sku.clone())])
    });

    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: firefam_apps_mcp_bearer_token_env_var(),
            http_headers,
