...
use firefam_rollout::ARCHIVED_SESSIONS_SUBDIR;
use firefam_rollout::append_rollout_item_to_path;
use firefam_rollout::append_thread_name;
...
pub(super) async fn update_thread_metadata(
    ...
    firefam_rollout::state_db::reconcile_rollout(
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    firefam_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(firefam_git_utils::GitSha::new),
        branch: branch.clone(),
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.firefam_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.firefam_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = firefam_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd = firefam_utils_path::normalize_for_path_comparison(workspace.as_path())
