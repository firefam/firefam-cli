...
fn test_deserialize_chat_wire_api_shows_helpful_error() {
    let provider_toml = r#"
name = "FirefamAI using Chat Completions"
...
#[test]
fn test_supports_remote_compaction_for_firefamai() {
    let provider = ModelProviderInfo::create_firefamai_provider(/*base_url*/ None);
...
fn test_supports_remote_compaction_for_azure_name() {
    let provider = ModelProviderInfo {
        name: "Azure".into(),
        base_url: Some("https://example.com/firefamai".into()),
        env_key: Some("AZURE_OPENAI_API_KEY".into()),
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
...
#[test]
fn test_supports_remote_compaction_for_non_firefamai_non_azure_provider() {
    let provider = ModelProviderInfo {
        name: "Example".into(),
...
fn test_built_in_model_providers_include_amazon_bedrock() {
    let providers = built_in_model_providers(/*firefamai_base_url*/ None);
...
fn test_merge_configured_model_providers_adds_custom_provider() {
    ...
    let mut expected = built_in_model_providers(/*firefamai_base_url*/ None);
    expected.insert("custom".to_string(), custom_provider);

    assert_eq!(
        merge_configured_model_providers(
            built_in_model_providers(/*firefamai_base_url*/ None),
...
fn test_merge_configured_model_providers_allows_amazon_bedrock_default_fields() {
    ...
    assert_eq!(
        ...
        Ok(built_in_model_providers(/*firefamai_base_url*/ None))
    );
}
...
fn test_validate_provider_aws_rejects_conflicting_auth() {
    ...
    assert_eq!(
        provider.validate(),
        Err("provider aws cannot be combined with env_key, requires_firefamai_auth".to_string())
    );
}
