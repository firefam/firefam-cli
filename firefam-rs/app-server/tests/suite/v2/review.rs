...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    create_config_toml_with_approval_policy(firefam_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": firefam_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
fn create_config_toml(firefam_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(firefam_home, server_uri, "never")
}
