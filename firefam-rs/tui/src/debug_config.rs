...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_firefam_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_firefam_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use firefam_config::FilesystemConstraints;
    use firefam_config::HookEventsToml;
    use firefam_config::HookHandlerConfig;
    use firefam_config::ManagedHooksRequirementsToml;
    use firefam_config::MatcherGroup;
    ...
    use firefam_config::NetworkUnixSocketPermissionToml;
    use firefam_config::NetworkUnixSocketPermissionsToml;
    use firefam_config::RequirementSource;
    use firefam_config::ResidencyRequirement;
    use firefam_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\firefam\\config.toml")
        } else {
            absolute_path("/etc/firefam/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.firefam")
        } else {
            absolute_path("/repo/.firefam")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_firefam_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\FirefamAI\\Firefam\\requirements.toml")
        } else {
            absolute_path("/etc/firefam/requirements.toml")
        };
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/blocked.sock=none, /tmp/firefam.sock=allow} (source: cloud requirements)"
        ));
    }
