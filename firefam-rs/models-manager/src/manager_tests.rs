...
use firefam_login::AuthCredentialsStoreMode;
use firefam_login::AuthManager;
use firefam_login::ExternalAuth;
use firefam_login::ExternalAuthRefreshContext;
use firefam_login::ExternalAuthTokens;
use firefam_login::FirefamAuth;
use firefam_login::TokenData;
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    async fn uses_firefam_backend(&self) -> bool {
        self.uses_firefam_backend
    }
...
fn firefamai_manager_for_tests(
    firefam_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
) -> OpenAiModelsManager {
    firefamai_manager_for_tests_with_auth(
        firefam_home,
        endpoint_client,
...
fn firefamai_manager_for_tests_with_auth(
    firefam_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
    auth_manager: Option<Arc<AuthManager>>,
) -> OpenAiModelsManager {
    OpenAiModelsManager::new(firefam_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(firefam_home: &Path) -> FirefamAuth {
    let auth_dot_json = firefam_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        firefamai_api_key: None,
        tokens: Some(TokenData {
            id_token: firefam_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(firefam_home).expect("firefam home should be created");
    std::fs::write(
...
async fn get_model_info_tracks_fallback_usage() {
    let firefam_home = tempdir().expect("temp dir");
    let config = ModelsManagerConfig::default();
    let manager = firefamai_manager_for_tests(
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "firefamai-firefam/gpt-image".to_string();
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager = firefamai_manager_for_tests(firefam_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        firefamai_manager_for_tests(firefam_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        firefamai_manager_for_tests(firefam_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = firefamai_manager_for_tests(firefam_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_keeps_merging_for_api_auth() {
    ...
    let manager = firefamai_manager_for_tests_with_auth(
        firefam_home.path().to_path_buf(),
        endpoint.clone(),
        Some(AuthManager::from_auth_for_testing(
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager =
        firefamai_manager_for_tests(firefam_home.path().to_path_buf(), endpoint.clone());
    ...
    async fn uses_firefam_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => auth_manager
                ...
                .await
                .as_ref()
                .is_some_and(FirefamAuth::uses_firefam_backend),
            None => false,
        }
    }
...
async fn refresh_available_models_skips_network_when_external_api_key_overrides_chatgpt_auth() {
    ...
    let firefam_home = tempdir().expect("temp dir");
    let auth_manager =
        AuthManager::from_auth_for_testing(FirefamAuth::create_dummy_chatgpt_auth_for_testing());
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(firefam_home.path()).await;
    let manager = firefamai_manager_for_tests_with_auth(
        firefam_home.path().to_path_buf(),
        endpoint.clone(),
