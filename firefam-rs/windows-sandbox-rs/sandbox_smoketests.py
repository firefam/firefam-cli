...
# Run a suite of smoke tests against the Windows sandbox via the Firefam CLI
...
def _resolve_firefam_cmd() -> List[str]:
    """Resolve the Firefam CLI to invoke `firefam sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Firefam.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "firefam.exe",
        ws_root / "target" / "release" / "firefam.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "firefam.exe",
            cargo_base / "release" / "firefam.exe",
        ])
    ...
    if shutil.which("firefam"):
        return ["firefam"]

    raise FileNotFoundError(
        "Firefam CLI not found. Build it first, e.g.\n"
        "  cargo build -p firefam-cli --bin firefam --release\n"
        "or for debug:\n"
        "  cargo build -p firefam-cli --bin firefam\n"
    )

FIREFAM_CMD = _resolve_firefam_cmd()
print(FIREFAM_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to firefam CLI overrides.
    ...
    argv = [*FIREFAM_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"FIREFAM_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\firefam_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.firefam artifacts) denied
    firefam_home = Path(os.environ["USERPROFILE"]) / ".firefam"
    cap_sid_target = firefam_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .firefam\\policy.json"], WS_ROOT)
    add("WS: .firefam cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .firefam policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > firefam_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://firefam-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
