...
use firefam_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use firefam_mcp_server::ExecApprovalElicitRequestParams;
use firefam_mcp_server::ExecApprovalResponse;
use firefam_mcp_server::FirefamToolCallParam;
use firefam_mcp_server::PatchApprovalElicitRequestParams;
use firefam_mcp_server::PatchApprovalResponse;
use firefam_protocol::protocol::FileChange;
use firefam_protocol::protocol::ReviewDecision;
use firefam_shell_command::parse_command;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "firefam" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let firefam_request_id = mcp_process
        .send_firefam_tool_call(FirefamToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            firefam_request_id.to_string(),
            params.firefam_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `firefam` tool call completes and that the file was created.
    let firefam_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(firefam_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(firefam_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    firefam_mcp_tool_call_id: String,
    firefam_event_id: String,
    ...
    let firefam_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        firefam_elicitation: "exec-approval".to_string(),
        firefam_mcp_tool_call_id,
        firefam_event_id,
        firefam_command: command,
        firefam_cwd: workdir.to_path_buf(),
        firefam_call_id: "call1234".to_string(),
        firefam_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "firefam" tool request that will trigger the apply_patch command
    let firefam_request_id = mcp_process
        .send_firefam_tool_call(FirefamToolCallParam {
    ...
    // Verify the original `firefam` tool call completes
    let firefam_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(firefam_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_firefam_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = firefam_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn firefam_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `firefam mcp` with a specific config.toml.
    let firefam_home = TempDir::new()?;
    create_config_toml(firefam_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(firefam_home.path()).await?;
    ...
    .await??;
    assert_eq!(firefam_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(firefam_response.id, RequestId::Number(firefam_request_id));
    assert_eq!(
        firefam_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": firefam_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("firefam tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        ...
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        firefam_elicitation: "patch-approval".to_string(),
        firefam_mcp_tool_call_id,
        firefam_event_id,
        firefam_reason: reason,
        firefam_grant_root: grant_root,
        firefam_changes: changes,
        firefam_call_id: "call1234".to_string(),
    })?;
