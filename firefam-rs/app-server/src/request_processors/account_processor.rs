...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "FIREFAM_APP_SERVER_LOGIN_ISSUER";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth.as_ref().map(FirefamAuth::api_auth_mode),
            plan_type: auth.as_ref().and_then(FirefamAuth::account_plan_type),
        }
    }
    ...
    async fn login_v2(
        ...
        match params {
            ...
            LoginAccountParams::Chatgpt {
                firefam_streamlined_login,
            } => {
                self.login_chatgpt_v2(request_id, firefam_streamlined_login)
                    .await;
            }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self.login_chatgpt_response(firefam_streamlined_login).await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self.login_chatgpt_common(firefam_streamlined_login).await?;
    ...
    async fn login_chatgpt_device_code_response(
        ...
        let opts = self
            .login_chatgpt_common(/*firefam_streamlined_login*/ false)
            .await?;
    ...
    async fn get_auth_status_response(
        ...
        // If a custom provider is configured with `requires_firefamai_auth == false`,
        // then no auth step is required; otherwise, default to requiring auth.
        let requires_firefamai_auth = self.config.model_provider.requires_firefamai_auth;

        let response = if !requires_firefamai_auth {
            ...
            match auth {
                Some(auth) => {
                    ...
                    let (reported_auth_method, token_opt) =
                        if matches!(auth, FirefamAuth::AgentIdentity(_))
    ...
    async fn get_account_response(
        ...
        Ok(GetAccountResponse {
            account,
            requires_firefamai_auth: account_state.requires_firefamai_auth,
        })
    }
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "firefam account authentication required to notify workspace owner",
            ));
        };
    ...
    > {
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "firefam account authentication required to read rate limits",
            ));
        };
        ...
        let snapshots = client
            ...
            .await
            .map_err(|err| internal_error(format!("failed to fetch firefam rate limits: {err}")))?;
        if snapshots.is_empty() {
            return Err(internal_error(
                "failed to fetch firefam rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<String, CoreRateLimitSnapshot> = snapshots
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "firefam".to_string());
                (limit_id, snapshot)
            })
            .collect();
