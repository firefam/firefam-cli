...
FIREFAM_HOME_DIR="${AGENTS_HOME:-$HOME/.firefam}"
...
download_file() {
  ...
  echo "curl or wget is required to install Firefam." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/firefam/firefam-cli/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/firefam/firefam-cli/releases/tags/rust-v%s\n' "$resolved_version"
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Firefam download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Firefam archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Firefam." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/firefam/firefam-cli/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Firefam release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> Firefam installer >>>"
  end_marker="# <<< Firefam installer <<<"
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/firefam" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && firefam"
      step "Future terminals: open a new terminal and run: firefam"
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed Firefam at $existing_path"
  warn "Multiple managed Firefam installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="npm uninstall -g firefam/firefam-cli"
      ;;
    bun)
      uninstall_cmd="bun remove -g firefam/firefam-cli"
      ;;
    *)
      uninstall_cmd="npm uninstall -g firefam/firefam-cli"
      ;;
  ...
  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Firefam now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Firefam. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Firefam installed. PATH order will determine which firefam runs."
  fi
}
...
update_visible_command() {
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.firefam.$$"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/firefam" "$tmp_link"
}
...
asset="firefam-npm-$npm_tag-$resolved_version.tgz"
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Firefam CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Firefam CLI"
else
  step "Installing Firefam CLI"
fi
