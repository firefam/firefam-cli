...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Firefam git baseline\n\nCo-authored-by: Firefam <noreply@firefam.ai>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = firefam_signature();
...
fn firefam_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: "Firefam".into(),
        email: "noreply@firefam.ai".into(),
        time: gix::date::Time {
