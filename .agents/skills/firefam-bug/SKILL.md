---
name: firefam-bug
description: Diagnose GitHub bug reports in firefamai/firefam. Use when given a GitHub issue URL from firefamai/firefam and asked to decide next steps such as verifying against the repo, requesting more info, or explaining why it is not a bug; follow any additional user-provided instructions.
---
...
Diagnose a Firefam GitHub bug report and decide the next action: verify against sources, request more info, or explain why it is not a bug.

## Workflow

1. Confirm the input

- Require a GitHub issue URL that points to `github.com/firefamai/firefam/issues/…`.
...
- Prefer the GitHub API over HTML pages because the HTML is noisy:
  - Issue: `https://api.github.com/repos/firefamai/firefam/issues/<number>`
  - Comments: `https://api.github.com/repos/firefamai/firefam/issues/<number>/comments`
...
- **Verify with sources** when the report is specific and likely reproducible. Inspect relevant Firefam files (or mention the files to inspect if access is unavailable).
