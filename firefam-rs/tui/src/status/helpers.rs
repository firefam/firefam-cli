...
mod tests {
    ...
    async fn test_config(firefam_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let firefam_home = TempDir::new().expect("temp firefam home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = firefam_home.path().join(DEFAULT_AGENTS_MD_FILENAME);
        let config = test_config(&firefam_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let firefam_home = TempDir::new().expect("temp firefam home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = firefam_home.path().join(LOCAL_AGENTS_MD_FILENAME);
        let config = test_config(&firefam_home, &cwd).await;
