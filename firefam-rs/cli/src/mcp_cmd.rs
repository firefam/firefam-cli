...
use firefam_rmcp_client::delete_oauth_tokens;
use firefam_rmcp_client::perform_oauth_login;
use firefam_utils_cli::CliConfigOverrides;
use firefam_utils_cli::format_env_display;
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "firefam mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: firefam_config::types::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    ConfigEditsBuilder::new(&firefam_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", firefam_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `firefam mcp login {name}` to login."
        ),
    }
...
async fn run_remove(config_overrides: &CliConfigOverrides, remove_args: RemoveArgs) -> Result<()> {
    ...
    if removed {
        ConfigEditsBuilder::new(&firefam_home)
            ...
            .await
            .with_context(|| {
                format!("failed to write MCP servers to {}", firefam_home.display())
            })?;
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `firefam mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: firefam mcp remove {}", get_args.name);
