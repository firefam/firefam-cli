...
use firefam_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut firefam_core::config::Config) {
...
pub(crate) async fn mount_analytics_capture(
    ...
    Mock::given(method("POST"))
        .and(path("/firefam/analytics-events/events"))
    ...
    let body = timeout(read_timeout, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/firefam/analytics-events/events"
            }) {
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/firefam/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "firefam_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("firefam_thread_initialized event should be present"))
}
