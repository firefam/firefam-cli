use super::*;
use firefam_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use firefam_protocol::items::AgentMessageContent;
use firefam_protocol::items::AgentMessageItem;
use firefam_protocol::items::FileChangeItem;
use firefam_protocol::items::ImageViewItem;
use firefam_protocol::items::McpToolCallItem;
use firefam_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use firefam_protocol::items::ReasoningItem;
use firefam_protocol::items::TurnItem;
use firefam_protocol::items::UserMessageItem;
use firefam_protocol::items::WebSearchItem;
use firefam_protocol::mcp::CallToolResult;
use firefam_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use firefam_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use firefam_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use firefam_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use firefam_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use firefam_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use firefam_protocol::protocol::AgentStatus as CoreAgentStatus;
use firefam_protocol::protocol::AskForApproval as CoreAskForApproval;
use firefam_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use firefam_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use firefam_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use firefam_protocol::user_input::UserInput as CoreUserInput;
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        firefam_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        firefam_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        firefam_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.firefam/skills/skill-creator/SKILL.md"),
            },
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(firefam_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_marketplace_entry_serializes_remote_only_path_as_null() {
    assert_eq!(
        serde_json::to_value(PluginMarketplaceEntry {
            name: "firefamai-curated-remote".to_string(),
            path: None,
            interface: None,
            plugins: Vec::new(),
        })
...
fn plugin_read_params_serialization_uses_install_source_fields() {
    ...
    assert_eq!(
        serde_json::from_value::<PluginReadParams>(json!({
            "remoteMarketplaceName": "firefamai-curated-remote",
            "pluginName": "gmail",
        }))
        .unwrap(),
        PluginReadParams {
            marketplace_path: None,
            remote_marketplace_name: Some("firefamai-curated-remote".to_string()),
            plugin_name: "gmail".to_string(),
        },
    );
}
...
fn plugin_skill_read_params_serialization_uses_remote_plugin_id() {
    assert_eq!(
        serde_json::to_value(PluginSkillReadParams {
            remote_marketplace_name: "firefamai-curated-remote".to_string(),
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@firefam-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "firefam-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@firefam-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "firefam-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
fn plugin_share_list_response_serializes_share_items() {
    assert_eq!(
        serde_json::to_value(PluginShareListResponse {
            data: vec![PluginShareListItem {
                plugin: PluginSummary {
                    id: "gmail@firefamai-curated-remote".to_string(),
        ...
        json!({
            "data": [{
                "plugin": {
                    "id": "gmail@firefamai-curated-remote",
                    "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
                    "localVersion": null,
                    "name": "gmail",
                    "shareContext": null,
...
fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
    assert_eq!(
        serde_json::to_value(PluginUninstallParams {
            plugin_id: "gmail@firefamai-curated".to_string(),
        })
        .unwrap(),
        json!({
            "pluginId": "gmail@firefamai-curated",
        }),
    );
...
#[test]
fn firefam_error_info_serializes_http_status_code_in_camel_case() {
    let value = FirefamErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn firefam_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(FirefamErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn firefam_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = FirefamErrorInfo::ActiveTurnNotSteerable {
