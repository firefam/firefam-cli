...
class AsyncFirefam:
    """Async mirror of :class:`Firefam`.

    Prefer ``async with AsyncFirefam()`` so initialization and shutdown are
    ...
    async def __aenter__(self) -> "AsyncFirefam":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncFirefam is not initialized yet. Prefer `async with AsyncFirefam()`; "
    ...
    # BEGIN GENERATED: AsyncFirefam.flat_methods
    ...
    # END GENERATED: AsyncFirefam.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        await self._firefam._ensure_initialized()
        return await self._firefam._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._firefam._ensure_initialized()
        return await self._firefam._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._firefam._ensure_initialized()
        return await self._firefam._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        await self._firefam._ensure_initialized()
        return await self._firefam._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        await self._firefam._ensure_initialized()
        return await self._firefam._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._firefam._ensure_initialized()
        self._firefam._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._firefam._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._firefam._client.unregister_turn_notifications(self.id)
