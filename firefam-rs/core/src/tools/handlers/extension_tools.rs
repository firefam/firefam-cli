...
pub(crate) struct ExtensionToolAdapter(Arc<dyn firefam_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn firefam_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl firefam_extension_api::ToolExecutor<firefam_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> firefam_tools::ToolName {
            firefam_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> Option<firefam_tools::ToolSpec> {
            Some(firefam_tools::ToolSpec::Function(
                firefam_tools::ResponsesApiTool {
                    name: "extension_echo".to_string(),
                    description: "Echoes arguments.".to_string(),
                    strict: true,
                    parameters: firefam_tools::parse_tool_input_schema(&json!({
                        "type": "object",
                        "properties": {
                            "message": { "type": "string" },
                        },
        ...
        async fn handle(
            &self,
            _call: firefam_tools::ToolCall,
        ) -> Result<Box<dyn firefam_tools::ToolOutput>, firefam_tools::FunctionCallError> {
            Ok(Box::new(firefam_tools::JsonToolOutput::new(
                json!({ "ok": true }),
            )))
        }
    }
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: firefam_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = firefam_tools::JsonToolOutput::new(json!({ "ok": true }));
