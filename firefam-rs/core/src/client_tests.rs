...
use super::X_FIREFAM_INSTALLATION_ID_HEADER;
use super::X_FIREFAM_PARENT_THREAD_ID_HEADER;
use super::X_FIREFAM_TURN_METADATA_HEADER;
use super::X_FIREFAM_WINDOW_ID_HEADER;
use super::X_FIREFAMAI_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use firefam_api::ApiError;
use firefam_api::ResponseEvent;
use firefam_app_server_protocol::AuthMode;
use firefam_login::AuthManager;
use firefam_login::FirefamAuth;
use firefam_model_provider::BearerAuthProvider;
use firefam_model_provider_info::CHATGPT_FIREFAM_BASE_URL;
use firefam_model_provider_info::ModelProviderInfo;
use firefam_model_provider_info::WireApi;
use firefam_model_provider_info::create_oss_provider_with_base_url;
...
use firefam_rollout_trace::ExecutionStatus;
use firefam_rollout_trace::InferenceTraceAttempt;
use firefam_rollout_trace::InferenceTraceContext;
use firefam_rollout_trace::RawTraceEventPayload;
use firefam_rollout_trace::RolloutTrace;
use firefam_rollout_trace::TraceWriter;
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::FirefamTurnStarted {
        firefam_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn build_subagent_headers_sets_other_subagent_label() {
    ...
    let value = headers
        .get(X_FIREFAMAI_SUBAGENT_HEADER)
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    assert_eq!(
        ...
        std::collections::HashMap::from([
            ...
            (
                X_FIREFAMAI_SUBAGENT_HEADER.to_string(),
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_firefamai_provider(Some(
                CHATGPT_FIREFAM_BASE_URL.to_string(),
            )),
        )
    } else {
        (
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_firefam_responses() {
...
#[tokio::test]
async fn non_chatgpt_firefam_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
