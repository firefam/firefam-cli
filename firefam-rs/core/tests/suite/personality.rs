...
use firefam_protocol::firefamai_models::ModelInstructionsVariables;
use firefam_protocol::firefamai_models::ModelMessages;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let firefam_home = TempDir::new().expect("create temp dir");
    ...
    let model_info = firefam_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info =
        firefam_core::test_support::construct_model_info_offline("gpt-5.3-codex", &config);
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "firefam-remote-default-personality";
    ...
    let mut builder = test_firefam()
        .with_auth(firefam_login::FirefamAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "firefam-remote-personality";
