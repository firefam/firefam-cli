...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(firefam_home)?.contains_current_source(source_path)
}
...
pub fn record_imported_session(
    ...
    let mut ledger = load_import_ledger(firefam_home)?;
    ...
    save_import_ledger(firefam_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(firefam_home);
...
fn import_ledger_path(firefam_home: &Path) -> PathBuf {
    firefam_home.join(SESSION_IMPORT_LEDGER_FILE)
}
