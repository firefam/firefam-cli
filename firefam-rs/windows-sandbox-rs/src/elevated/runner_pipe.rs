...
pub fn find_runner_exe(firefam_home: &Path, log_dir: Option<&Path>) -> PathBuf {
    resolve_helper_for_launch(HelperExecutable::CommandRunner, firefam_home, log_dir)
}
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\firefam-runner-{nonce:x}");
