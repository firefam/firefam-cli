//! Implements the `firefam doctor` diagnostic report.
//!
...
use anyhow::Context;
use clap::Parser;
use firefam_api::ApiError;
use firefam_api::ResponsesWebsocketClient;
use firefam_api::is_azure_responses_provider;
...
use firefam_install_context::FirefamPackageLayout;
use firefam_install_context::InstallContext;
use firefam_install_context::InstallMethod;
use firefam_install_context::StandalonePlatform;
use firefam_login::AuthDotJson;
use firefam_login::AuthManager;
use firefam_login::CODEX_API_KEY_ENV_VAR;
use firefam_login::FIREFAM_ACCESS_TOKEN_ENV_VAR;
use firefam_login::FirefamAuth;
use firefam_login::OPENAI_API_KEY_ENV_VAR;
use firefam_login::default_client::build_reqwest_client;
use firefam_login::default_client::default_headers;
use firefam_login::load_auth_dot_json;
use firefam_model_provider::create_model_provider;
use firefam_protocol::protocol::AskForApproval;
use firefam_terminal_detection::Multiplexer;
use firefam_terminal_detection::TerminalInfo;
use firefam_terminal_detection::TerminalName;
use firefam_terminal_detection::terminal_info;
...
const FIREFAMAI_BETA_HEADER: &str = "OpenAI-Beta";
...
/// Options for building a local Firefam diagnostic report.
///
...
///
/// This is the CLI entry point for firefam doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (config_check, network_check, terminal_check, state_check, reachability_check) = tokio::join!(
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun firefam doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        firefam_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
fn config_overrides_from_interactive(
    ...
    ConfigOverrides {
        ...
        model_provider: interactive
            ...
            .flatten(),
        firefam_self_exe: arg0_paths.firefam_self_exe.clone(),
        firefam_linux_sandbox_exe: arg0_paths.firefam_linux_sandbox_exe.clone(),
...
/// JSON support report emitted by `firefam doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        firefam_version: report.firefam_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("FIREFAM_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "FIREFAM_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = firefam_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH firefam entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH firefam #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary = "npm install -g firefam/firefam-cli would update a different install"
                    .to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Firefam so the JS shim provides FIREFAM_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("FIREFAM_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("FIREFAM_MANAGED_BY_NPM").is_none()
        && env::var_os("FIREFAM_MANAGED_BY_BUN").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&FirefamPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("FIREFAM_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("firefam");
...
fn firefam_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["firefam"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "firefam"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("FIREFAM_HOME: {}", config.firefam_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.firefam_home.join(firefam_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.firefam_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    if let Some(check) = provider_specific_auth_check(
        config.model_provider.requires_firefamai_auth,
    ...
    match load_auth_dot_json(&config.firefam_home, config.cli_auth_credentials_store_mode) {
        Ok(Some(auth)) => {
            details.push(format!("stored auth mode: {}", stored_auth_mode(&auth)));
            details.push(format!(
                "stored API key: {}",
                auth.firefamai_api_key.is_some()
            ));
            ...
            if status == CheckStatus::Fail {
                check = check
                    .remediation("Run firefam login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Firefam credentials were found",
        )
        .details(details)
        .remediation("Run firefam login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run firefam login again."),
    }
}
...
fn provider_specific_auth_check(
    ...
    details.push(format!(
        "model provider requires FirefamAI auth: {requires_firefamai_auth}"
    ));
    if requires_firefamai_auth {
        return None;
    }
    ...
    match provider_env_key {
        ...
        None => Some(
            DoctorCheck::new(
                ...
                "auth",
                CheckStatus::Ok,
                "FirefamAI auth is not required for the active model provider",
            )
            .details(details),
        ),
    }
...
fn stored_auth_mode(auth: &firefam_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        firefam_app_server_protocol::AuthMode::ApiKey => "api_key",
        firefam_app_server_protocol::AuthMode::Chatgpt => "chatgpt",
        firefam_app_server_protocol::AuthMode::ChatgptAuthTokens => "chatgpt_auth_tokens",
        firefam_app_server_protocol::AuthMode::AgentIdentity => "agent_identity",
    }
}

fn stored_auth_mode_value(auth: &AuthDotJson) -> firefam_app_server_protocol::AuthMode {
    if let Some(mode) = auth.auth_mode {
        return mode;
    }
    if auth.firefamai_api_key.is_some() {
        firefam_app_server_protocol::AuthMode::ApiKey
    } else {
        firefam_app_server_protocol::AuthMode::Chatgpt
    }
}
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        ...
        firefam_app_server_protocol::AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
        ...
        firefam_app_server_protocol::AuthMode::AgentIdentity => {
            if auth
                .agent_identity
                .as_deref()
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["FIREFAM_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "firefam-linux-sandbox helper",
        arg0_paths.firefam_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.firefam_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "FIREFAM_HOME", &config.firefam_home);
    ...
    for db in firefam_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.firefam_home);
    ...
    if status == CheckStatus::Fail {
        check = check.remediation(
            "Back up FIREFAM_HOME, then remove or repair the affected SQLite database.",
        );
    }
    check
}
...
async fn sqlite_integrity_detail(
    ...
    match firefam_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, firefam_home: &Path) {
    let active = collect_rollout_stats(&firefam_home.join("sessions"));
    let archived = collect_rollout_stats(&firefam_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &FirefamAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match firefam_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "FIREFAM_HOME was resolved without config",
        )
        .detail(format!("FIREFAM_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "FIREFAM_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_reachability_plan(config: &Config) -> ReachabilityPlan {
    let stored_auth =
        load_auth_dot_json(&config.firefam_home, config.cli_auth_credentials_store_mode)
            .ok()
            .flatten();
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(FIREFAM_ACCESS_TOKEN_ENV_VAR) {
        return ProviderAuthReachabilityMode::Chatgpt;
    }
    match stored_auth.map(stored_auth_mode_value) {
        Some(firefam_app_server_protocol::AuthMode::ApiKey) => ProviderAuthReachabilityMode::ApiKey,
        Some(
            firefam_app_server_protocol::AuthMode::Chatgpt
            | firefam_app_server_protocol::AuthMode::ChatgptAuthTokens
            | firefam_app_server_protocol::AuthMode::AgentIdentity,
...
fn provider_reachability_plan_from_parts(
    ...
    let endpoint_label = if provider_id == "openai" {
    ...
    let endpoints = match mode {
        ProviderAuthReachabilityMode::ApiKey => vec![ReachabilityEndpoint {
            label: format!("{endpoint_label} API"),
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/firefam");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("firefam"),
            }
        );
    }
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            firefam_self_exe: Some(PathBuf::from("/bin/firefam")),
            firefam_linux_sandbox_exe: Some(PathBuf::from("/bin/firefam-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/firefam-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.firefam_self_exe, arg0_paths.firefam_self_exe);
        assert_eq!(
            overrides.firefam_linux_sandbox_exe,
            arg0_paths.firefam_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            firefam_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "mcp.config",
                    "mcp",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "FIREFAM_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("FIREFAM_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    #[test]
    fn provider_specific_auth_allows_non_firefamai_provider_without_env_key() {
        ...
        .expect("non-FirefamAI provider should produce a provider-specific check");
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Firefam."),
            Vec::new(),
            |_| false,
        )
        .expect("non-FirefamAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Firefam.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_api_key() {
        let auth = AuthDotJson {
            auth_mode: Some(firefam_app_server_protocol::AuthMode::ApiKey),
            firefamai_api_key: None,
            tokens: None,
            last_refresh: None,
            agent_identity: None,
        };
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        let api_key_auth = AuthDotJson {
            auth_mode: Some(firefam_app_server_protocol::AuthMode::ApiKey),
            firefamai_api_key: Some("sk-test".to_string()),
            tokens: None,
            last_refresh: None,
            agent_identity: None,
        };
    ...
    #[test]
    fn provider_reachability_adds_models_route_probe_for_firefamai_compatible_base_urls() {
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-firefam-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-firefam-doctor-mcp\" is not resolvable",
            )
        }));
    }
