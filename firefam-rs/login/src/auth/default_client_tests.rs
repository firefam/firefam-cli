...
#[test]
fn test_get_firefam_user_agent() {
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("firefam-tui"), true);
    assert_eq!(is_first_party_originator("firefam_vscode"), true);
    assert_eq!(is_first_party_originator("Firefam Something Else"), true);
    assert_eq!(is_first_party_originator("firefam_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("firefam_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("firefam_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("firefam_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    let expected_ua = get_firefam_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "firefam_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "firefam_cli_rs/0.0.0 (bad_suffix)"
    );
}
