...
use firefam_config::permissions_toml::overlay_network_domain_permissions;
...
use firefam_network_proxy::NetworkProxyConstraintError;
...
use firefam_network_proxy::validate_policy_against_constraints;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    ...
    let (exec_policy, warning) = match load_exec_policy(&config_layer_stack).await {
        ...
        Err(err @ ExecPolicyError::ParsePolicy { .. }) => {
            (firefam_execpolicy::Policy::empty(), Some(err))
        }
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_firefam_folder } => {
                    Some(dot_firefam_folder.join(CONFIG_TOML_FILE))
                }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            firefam_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            firefam_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: firefam_network_proxy::NetworkDomainPermission,
) {
    config
        .network
