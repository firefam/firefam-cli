...
use firefam_app_server_client::AppServerClient;
use firefam_app_server_client::AppServerEvent;
use firefam_app_server_client::AppServerRequestHandle;
use firefam_app_server_client::TypedRequestError;
...
use firefam_app_server_protocol::ConfigBatchWriteParams;
use firefam_app_server_protocol::ConfigWriteResponse;
use firefam_app_server_protocol::ExternalAgentConfigDetectParams;
use firefam_app_server_protocol::ExternalAgentConfigDetectResponse;
use firefam_app_server_protocol::ExternalAgentConfigImportParams;
use firefam_app_server_protocol::ExternalAgentConfigImportResponse;
use firefam_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use firefam_app_server_protocol::ThreadMemoryMode;
...
use firefam_app_server_protocol::ThreadRealtimeAudioChunk;
...
use firefam_protocol::approvals::GuardianAssessmentEvent;
use firefam_protocol::firefamai_models::ModelAvailabilityNux;
use firefam_protocol::firefamai_models::ModelPreset;
use firefam_protocol::firefamai_models::ModelServiceTier;
use firefam_protocol::firefamai_models::ModelUpgrade;
use firefam_protocol::firefamai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<firefam_protocol::account::PlanType>,
    /// Whether the configured model provider needs FirefamAI-style auth. Combined
    /// with `has_chatgpt_account` to decide if a startup rate-limit prefetch
    /// should be fired.
    pub(crate) requires_firefamai_auth: bool,
...
impl AppServerSession {
    ...
    pub(crate) async fn bootstrap(&mut self, config: &Config) -> Result<AppServerBootstrap> {
        ...
        let (
            ...
            Some(Account::Chatgpt { email, plan_type }) => {
                let feedback_audience = if email.ends_with("@firefam.ai") {
        ...
        Ok(AppServerBootstrap {
            ...
            status_account_display,
            plan_type,
            requires_firefamai_auth: account.requires_firefamai_auth,
            default_model,
            feedback_audience,
            has_chatgpt_account,
            available_models,
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: firefam_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<firefam_protocol::firefamai_models::ReasoningEffort>,
        summary: Option<firefam_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<firefam_protocol::config_types::CollaborationMode>,
        personality: Option<firefam_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<firefam_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<firefam_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<firefam_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(firefam_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(firefam_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(firefam_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(firefam_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<firefam_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &firefam_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<firefam_protocol::firefamai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        firefam_message_history::HistoryConfig::new(config.firefam_home.clone(), &config.history);
    let (log_id, entry_count) = firefam_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    use firefam_protocol::config_types::Verbosity;
    use firefam_protocol::config_types::WebSearchMode;
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .firefam_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(firefam_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("firefam"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("firefam".to_string(), rate_limit_snapshot("firefam")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(firefam_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: firefam_app_server_protocol::Thread {
                ...
                source: firefam_app_server_protocol::SessionSource::Cli,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        firefam_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![firefam_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        firefam_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: firefam_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: firefam_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .firefam_home(temp_dir.path().join("embedded-firefam-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &firefam_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let history_config = firefam_message_history::HistoryConfig::new(
            config.firefam_home.clone(),
            &config.history,
        );

        firefam_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        firefam_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            firefam_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(firefam_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(firefam_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
