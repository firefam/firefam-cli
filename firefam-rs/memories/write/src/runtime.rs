...
use firefam_core::content_items_to_text;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<FirefamThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<firefam_otel::Timer> {
...
impl MemoryStartupContext {
    ...
    pub(crate) async fn stage_one_request_context(
        ...
        let turn_metadata_header =
            firefam_core::build_turn_metadata_header(&config.cwd, /*sandbox*/ None).await;
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let firefam_protocol::models::ResponseItem::Message { content, .. } =
                            item
