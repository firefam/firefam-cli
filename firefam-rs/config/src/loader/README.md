# `firefam-config` loader

This module is the canonical place to **load and describe Firefam configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `firefam_config::loader`:

- `load_config_layers_state(fs, firefam_home, cwd_opt, cli_overrides, options, cloud_requirements, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
