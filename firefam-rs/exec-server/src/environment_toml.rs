...
pub(crate) fn environment_provider_from_firefam_home(
    firefam_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = firefam_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "firefam exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "FIREFAM_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("firefam".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = firefam_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "firefam exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
FIREFAM_LOG = "debug"
"#,
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_firefam_home_uses_present_environments_file() {
        let firefam_home = tempdir().expect("tempdir");
        std::fs::write(
            firefam_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider = environment_provider_from_firefam_home(firefam_home.path())
    ...
    #[tokio::test]
    async fn environment_provider_from_firefam_home_falls_back_when_file_is_missing() {
        let firefam_home = tempdir().expect("tempdir");
