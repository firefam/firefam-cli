...
use crate::firefam_apps::FirefamAppsToolsCacheKey;
...
use firefam_protocol::protocol::McpStartupCompleteEvent;
use firefam_protocol::protocol::McpStartupFailure;
use firefam_protocol::protocol::McpStartupStatus;
use firefam_protocol::protocol::McpStartupUpdateEvent;
...
pub struct McpConnectionManager {
    ...
    host_owned_firefam_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub fn is_host_owned_firefam_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_firefam_apps_enabled && server_name == FIREFAM_APPS_MCP_SERVER_NAME
    }
    ...
    pub async fn new(
        ...
        firefam_apps_tools_cache_key: FirefamAppsToolsCacheKey,
        host_owned_firefam_apps_enabled: bool,
        ...
        let firefam_apps_auth_provider = auth
            .filter(|auth| auth.uses_firefam_backend())
            .map(firefam_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let firefam_apps_tools_cache_context = if server_name == FIREFAM_APPS_MCP_SERVER_NAME {
                Some(FirefamAppsToolsCacheContext {
                    firefam_home: firefam_home.clone(),
                    user_key: firefam_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == FIREFAM_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    firefam_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    /// Force-refresh firefam apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_firefam_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(FIREFAM_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{FIREFAM_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{FIREFAM_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_firefam_apps_tools_if_needed(
            FIREFAM_APPS_MCP_SERVER_NAME,
            managed_client.firefam_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
