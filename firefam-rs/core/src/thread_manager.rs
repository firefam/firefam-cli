...
use crate::firefam_thread::FirefamThread;
use crate::mcp::McpManager;
use crate::rollout::truncation;
use crate::session::Firefam;
use crate::session::FirefamSpawnArgs;
use crate::session::FirefamSpawnOk;
...
use firefam_model_provider_info::FIREFAMAI_PROVIDER_ID;
...
use firefam_state::DirectionalThreadSpawnEdgeStatus;
use firefam_thread_store::InMemoryThreadStore;
use firefam_thread_store::LocalThreadStore;
use firefam_thread_store::LocalThreadStoreConfig;
use firefam_thread_store::ReadThreadByRolloutPathParams;
...
use firefam_thread_store::UpdateThreadMetadataParams;
...
struct TempFirefamHomeGuard {
    path: PathBuf,
}

impl Drop for TempFirefamHomeGuard {
    fn drop(&mut self) {
...
pub struct ThreadManager {
    ...
    _test_firefam_home_guard: Option<TempFirefamHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<firefam_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<FirefamThread>>>>,
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let firefam_home = config.firefam_home.clone();
        ...
        Self {
            ...
            _test_firefam_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided FirefamAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let firefam_home = std::env::temp_dir().join(format!(
            "firefam-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&firefam_home)
            .unwrap_or_else(|err| panic!("temp firefam home dir create failed: {err}"));
        ...
        manager._test_firefam_home_guard = Some(TempFirefamHomeGuard { path: firefam_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided FirefamAuth and firefam home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_firefam_home = match AbsolutePathBuf::from_absolute_path_checked(&firefam_home) {
            Ok(firefam_home) => firefam_home,
            Err(err) => panic!("test firefam_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: firefam_home.clone(),
                default_model_provider_id: FIREFAMAI_PROVIDER_ID.to_string(),
            },
            state_db.clone(),
        ));
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(firefam_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_manager,
                plugins_manager,
                mcp_manager,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> FirefamResult<Arc<FirefamThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `FirefamThread`/`LiveThread`, so metadata changes stay ordered
    ...
    ) -> FirefamResult<StoredThread> {
        if let Ok(thread) = self.get_thread(thread_id).await {
            if thread.config_snapshot().await.ephemeral {
                return Err(FirefamErr::InvalidRequest(format!(
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    FirefamErr::ThreadNotFound(thread_id)
                }
        ...
        if let Some(state_db_ctx) = thread.state_db() {
            for status in [
                ...
                for descendant_id in state_db_ctx
                    ...
                    .await
                    .map_err(|err| {
                        FirefamErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                    })?
                {
    ...
    pub async fn start_thread(&self, config: Config) -> FirefamResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<firefam_protocol::dynamic_tools::DynamicToolSpec>,
        persist_extended_history: bool,
    ) -> FirefamResult<NewThread> {
    ...
    /// as `Arc<FirefamThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<FirefamThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> FirefamResult<Arc<FirefamThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(FirefamErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => FirefamErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> FirefamResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<FirefamThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let FirefamSpawnOk {
            firefam, thread_id, ..
        } = Firefam::spawn(FirefamSpawnArgs {
            config,
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(firefam, thread_id, tracked_session_source)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = firefam.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(FirefamErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(FirefamThread::new(
                    firefam,
        ...
        if let Err(err) = firefam.shutdown_and_wait().await {
            warn!("failed to shut down duplicate thread {thread_id}: {err}");
        }
        Err(FirefamErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> firefam_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return firefam_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            .await
            .ok()
            .map(|thread| thread.firefam.session.services.rollout_thread_trace.clone())
            .unwrap_or_else(firefam_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> FirefamErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => FirefamErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => FirefamErr::InvalidRequest(message),
        err => FirefamErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> FirefamErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => FirefamErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => FirefamErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => FirefamErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => FirefamErr::Fatal(format!(
