...
use firefam_execpolicy::MatchOptions;
...
use firefam_protocol::protocol::NetworkPolicyRuleAction;
...
use firefam_shell_command::bash::parse_shell_lc_plain_commands;
use firefam_shell_command::bash::parse_shell_lc_single_command_prefix;
use firefam_shell_escalation::EscalateServer;
use firefam_shell_escalation::EscalationDecision;
use firefam_shell_escalation::EscalationExecution;
use firefam_shell_escalation::EscalationPermissions;
use firefam_shell_escalation::EscalationPolicy;
use firefam_shell_escalation::EscalationSession;
use firefam_shell_escalation::ExecParams;
use firefam_shell_escalation::ExecResult;
use firefam_shell_escalation::PreparedExec;
use firefam_shell_escalation::ResolvedPermissionProfile;
use firefam_shell_escalation::ShellCommandExecutor;
use firefam_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Firefam(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        firefam_linux_sandbox_exe: ctx.turn.firefam_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    firefam_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<firefam_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["FIREFAM_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            firefam_linux_sandbox_exe: self.firefam_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Firefam(FirefamErr::Sandbox(
            SandboxErr::Timeout {
