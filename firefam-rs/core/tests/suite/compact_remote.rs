...
use core_test_support::test_firefam::TestFirefamBuilder;
use core_test_support::test_firefam::TestFirefamHarness;
...
use firefam_protocol::protocol::RealtimeConversationRealtimeEvent;
use firefam_protocol::protocol::RealtimeEvent;
use firefam_protocol::protocol::RealtimeOutputModality;
...
fn remote_realtime_test_firefam_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestFirefamBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_firefam()
        .with_auth(FirefamAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(firefam: &firefam_core::FirefamThread) -> Result<()> {
    firefam
    ...
    wait_for_event_match(firefam, |msg| match msg {
...
async fn close_realtime_conversation(firefam: &firefam_core::FirefamThread) -> Result<()> {
    firefam.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(firefam, |msg| match msg {
...
async fn wait_for_turn_complete(firefam: &firefam_core::FirefamThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestFirefamHarness::with_builder(
        test_firefam().with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let firefam = harness.test().firefam.clone();
    ...
    firefam
        ...
        .await?;
    wait_for_turn_complete(&firefam).await;

    firefam.submit(Op::Compact).await?;
    wait_for_turn_complete(&firefam).await;
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_firefam().with_auth(auth);
    ...
    let harness = TestFirefamHarness::with_builder(builder).await?;
    let firefam = harness.test().firefam.clone();
    let image_url =
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        FirefamAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-firefam-beta-features")
            .as_deref()
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![
        DynamicToolSpec {
            namespace: Some("firefam_app".to_string()),
    ...
    let new_thread = test
        ...
        .await?;
    test.firefam = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "firefam_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "firefam_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&firefam, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestFirefamHarness::with_builder(
        test_firefam()
            .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_firefam = baseline_harness.test().firefam.clone();
    ...
    baseline_firefam
        ...
        .await?;
    wait_for_event(&baseline_firefam, |event| {
    ...
    baseline_firefam.submit(Op::Compact).await?;
    wait_for_event(&baseline_firefam, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestFirefamHarness::with_builder(
        test_firefam()
            .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_firefam = override_harness.test().firefam.clone();
    ...
    override_firefam
        ...
        .await?;
    wait_for_event(&override_firefam, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_firefam.submit(Op::Compact).await?;
    wait_for_event(&override_firefam, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    firefam.submit(Op::Shutdown).await?;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.firefam.submit(Op::Compact).await?;
    wait_for_event(&initial.firefam, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    initial.firefam.submit(Op::Shutdown).await?;
    wait_for_event(&initial.firefam, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.firefam, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.firefam
        ...
        .await?;
    wait_for_event(&test.firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.firefam.submit(Op::Compact).await?;
    wait_for_event(&test.firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_firefam_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.firefam.as_ref()).await?;
    ...
    close_realtime_conversation(test.firefam.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_firefam_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_firefam_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.firefam.as_ref()).await?;
    ...
    close_realtime_conversation(initial.firefam.as_ref()).await?;
