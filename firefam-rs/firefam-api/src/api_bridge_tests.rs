...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, FirefamErr::ServerOverloaded));
}
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let FirefamErr::CyberPolicy { message } = err else {
        panic!("expected FirefamErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let FirefamErr::InvalidRequest(message) = err else {
        panic!("expected FirefamErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("firefam_other"),
    );
    headers.insert(
        "x-firefam-other-limit-name",
        http::HeaderValue::from_static("firefam_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let FirefamErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected FirefamErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let FirefamErr::UnexpectedStatus(err) = err else {
        panic!("expected FirefamErr::UnexpectedStatus, got {err:?}");
    };
