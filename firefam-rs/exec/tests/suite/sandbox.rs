#![cfg(unix)]
use firefam_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use firefam_core::exec::build_exec_request;
...
async fn spawn_command_under_sandbox(
    ...
    use firefam_core::spawn_command_under_linux_sandbox;

    let firefam_linux_sandbox_exe = core_test_support::find_firefam_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_firefam_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    .await
    .expect("should spawn command creating .firefam");

    let status = child
        .wait()
        .await
        .expect("should wait for .firefam command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .firefam creation: {status:?}"
    );
    let dot_firefam_metadata = tokio::fs::symlink_metadata(&dot_firefam).await;
    if let Ok(metadata) = dot_firefam_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_firefam.display()
        );
    } else if let Err(err) = &dot_firefam_metadata {
        assert_eq!(
            err.kind(),
