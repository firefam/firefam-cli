...
async fn global_doc_invalid_utf8_warns_and_uses_lossy_text() {
    ...
    let firefam_home_abs = firefam_home.abs();
    let path = firefam_home_abs.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::write(&path, b"global\xFF doc").unwrap();

    let mut warnings = Vec::new();
    let loaded = AgentsMdManager::load_global_instructions(
        LOCAL_FS.as_ref(),
        Some(&firefam_home_abs),
        &mut warnings,
    )
    .await
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".firefam-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.firefam_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.firefam_home).unwrap();
...
fn create_skill(firefam_home: PathBuf, name: &str, description: &str) {
    let skill_dir = firefam_home.join(format!("skills/{name}"));
