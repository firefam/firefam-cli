...
use firefam_extension_api::ResponsesApiTool;
use firefam_extension_api::ToolCall as ExtensionToolCall;
use firefam_extension_api::ToolExecutor;
...
impl firefam_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl ToolExecutor<ExtensionToolCall> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> Option<ToolSpec> {
        Some(ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: firefam_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    ) -> Result<Box<dyn firefam_tools::ToolOutput>, firefam_tools::FunctionCallError> {
        let arguments: serde_json::Value =
            serde_json::from_str(call.function_arguments()?).expect("test arguments should parse");
        Ok(Box::new(firefam_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
            "ok": true,
        }))))
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__firefam_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__firefam_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&router.model_visible_specs(), "firefam_app"),
