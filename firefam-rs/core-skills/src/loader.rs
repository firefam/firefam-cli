...
use firefam_config::default_project_root_markers;
use firefam_config::merge_toml_values;
use firefam_config::project_root_markers_from_config;
...
use firefam_utils_plugins::plugin_namespace_for_skill_path;
...
const SKILLS_METADATA_FILENAME: &str = "firefamai.yaml";
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$FIREFAM_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$FIREFAM_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/firefam/` on Unix, so treat
                // `/etc/firefam/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
...
async fn discover_skills_under_root(
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Firefam itself.
