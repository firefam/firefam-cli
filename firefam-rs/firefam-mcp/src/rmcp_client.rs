...
use crate::firefam_apps::CachedFirefamAppsToolsLoad;
use crate::firefam_apps::FirefamAppsToolsCacheContext;
use crate::firefam_apps::filter_disallowed_firefam_apps_tools;
use crate::firefam_apps::load_cached_firefam_apps_tools;
use crate::firefam_apps::load_startup_cached_firefam_apps_tools_snapshot;
use crate::firefam_apps::normalize_firefam_apps_callable_name;
use crate::firefam_apps::normalize_firefam_apps_callable_namespace;
use crate::firefam_apps::normalize_firefam_apps_tool_title;
use crate::firefam_apps::write_cached_firefam_apps_tools_if_needed;
...
use firefam_exec_server::HttpClient;
use firefam_exec_server::ReqwestHttpClient;
use firefam_protocol::protocol::Event;
use firefam_rmcp_client::ExecutorStdioServerLauncher;
use firefam_rmcp_client::LocalStdioServerLauncher;
use firefam_rmcp_client::RmcpClient;
use firefam_rmcp_client::StdioServerLauncher;
...
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "firefam/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "firefam.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "firefam.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) firefam_apps_tools_cache_context: Option<FirefamAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.firefam_apps_tools_cache_context.as_ref()
            && let CachedFirefamAppsToolsLoad::Hit(tools) =
                load_cached_firefam_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.firefam_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        firefam_apps_tools_cache_context: Option<FirefamAppsToolsCacheContext>,
        ...
        let startup_snapshot = load_startup_cached_firefam_apps_tools_snapshot(
            &server_name,
            firefam_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        firefam_apps_tools_cache_context,
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_firefam_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_firefam_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title = normalize_firefam_apps_tool_title(
                    server_name,
                    connector_name.as_deref(),
                    title,
                );
    ...
    if server_name == FIREFAM_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_firefam_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "firefam-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Firefam".into()),
            description: None,
            icons: None,
            website_url: None,
        },
    ...
    write_cached_firefam_apps_tools_if_needed(
        &server_name,
        firefam_apps_tools_cache_context.as_ref(),
        &tools,
    );
...
mod tests {
    ...
    #[test]
    fn firefam_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
