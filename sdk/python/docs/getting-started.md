...
Existing Firefam auth state is reused automatically. To authenticate from the SDK,
...
- `Firefam()` started and initialized `firefam app-server`.
...
with Firefam() as firefam:
    thread = firefam.thread_resume(THREAD_ID)
