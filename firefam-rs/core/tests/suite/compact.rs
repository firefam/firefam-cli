...
use core_test_support::test_firefam::turn_permission_fields;
use core_test_support::wait_for_event;
use core_test_support::wait_for_event_match;
use firefam_core::compact::SUMMARIZATION_PROMPT;
use firefam_core::compact::SUMMARY_PREFIX;
...
fn non_firefamai_model_provider(server: &MockServer) -> ModelProviderInfo {
    let mut provider =
        built_in_model_providers(/* firefamai_base_url */ /*firefamai_base_url*/ None)["openai"]
            .clone();
    provider.name = "FirefamAI (test)".into();
...
async fn assert_compaction_uses_turn_lifecycle_id(
    firefam: &std::sync::Arc<firefam_core::FirefamThread>,
) {
    ...
    while turn_completed_id.is_none() {
        let event = firefam.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Firefam.
    let model_provider = non_firefamai_model_provider(&server);
    ...
    firefam.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&firefam, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Firefam to flush rollout entries before inspecting the file.
    firefam.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&firefam, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    firefam.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&firefam, |ev| match ev {
    ...
    wait_for_event(&firefam, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs =
        read_hook_inputs(&test.firefam_home_path().join("pre_compact_block_log.jsonl"));
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let firefam = builder.build(&server).await.unwrap().firefam;
    ...
    let first = wait_for_event_match(&firefam, |ev| match ev {
    ...
    let last = wait_for_event_match(&firefam, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    firefam
        ...
        .await
        .unwrap();
    wait_for_event(&firefam, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    {
        let event = firefam.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    let non_firefamai_provider_name = non_firefamai_model_provider(&server).name;
    let firefam = test_firefam()
        .with_config(move |config| {
            config.model_provider.name = non_firefamai_provider_name;
        })
        .build(&server)
        .await
        .expect("build firefam")
        .firefam;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        firefam_protocol::models::ResponseItem::Message {
            id: None,
        ...
        firefam_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_firefam().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.firefam, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let model_provider = non_firefamai_model_provider(&server);
    let mut builder = test_firefam()
        .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
            set_test_compact_prompt(config);
        });
    let test = builder.build(&server).await.expect("build test firefam");
    ...
    test.firefam
        ...
        .await
        .expect("submit first user turn");
    wait_for_event(&test.firefam, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.firefam).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_firefamai_model_provider(&server);
    let mut initial_builder = test_firefam()
        .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test firefam");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.firefam, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_firefamai_model_provider(&server);
    let mut resumed_builder = test_firefam()
        .with_auth(FirefamAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.firefam).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let mut model_provider = non_firefamai_model_provider(&server);
    ...
    let reconnect_message = wait_for_event_match(&firefam, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&firefam, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let firefam = builder.build(&server).await.unwrap();

    firefam.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    firefam.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_pre_turn_compaction_context_window_exceeded() {
    ...
    let error_message = wait_for_event_match(&firefam, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    firefam.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&firefam, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
