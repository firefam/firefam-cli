...
const FIXED_CWD: &str = "/tmp/firefam_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> FirefamAuth {
        match self {
            AuthCase::ChatGpt => FirefamAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => FirefamAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&firefam, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().firefam_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestFirefamHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    let mut builder = test_firefam().with_auth(settings.auth.build());
    if hooks {
        builder = builder.with_pre_build_hook(write_manual_compact_hooks);
    }
    TestFirefamHarness::with_builder(builder.with_config(move |config| {
        config.cwd = firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestFirefamHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    firefam: &firefam_core::FirefamThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<FIREFAM_HOME>");
            search_start = start_index + "<FIREFAM_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <FIREFAM_HOME>/skills/.system/imagegen/SKILL.md and \
         <FIREFAM_HOME>/skills/custom/SKILL.md"
    );
}
