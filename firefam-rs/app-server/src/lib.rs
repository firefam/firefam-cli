...
use firefam_app_server_protocol::TextPosition as AppTextPosition;
use firefam_app_server_protocol::TextRange as AppTextRange;
use firefam_config::ConfigLoadError;
use firefam_config::TextRange as CoreTextRange;
use firefam_core::ExecPolicyError;
...
use firefam_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "firefam-app-server";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_firefam_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_firefam_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let environment_manager = if loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_firefam_home(firefam_home.clone(), Some(local_runtime_paths)).await
    }
    .map(Arc::new)
        ...
        .await
    {
        Ok(config) => {
            ...
            let auth_manager =
                AuthManager::shared_from_config(&config, /*enable_firefam_api_key_env*/ false)
                    .await;
    ...
    let otel = firefam_core::otel_init::build_provider(
        &config,
    ...
    firefam_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    firefam_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&firefam_home)?;
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match firefam_core::personality_migration::maybe_migrate_personality(
                    &config.firefam_home,
                    &config_toml,
                    state_db.clone(),
                )
                .await
                {
                    Ok(firefam_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        firefam_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | firefam_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | firefam_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
    ...
    let feedback = FirefamFeedback::new();
