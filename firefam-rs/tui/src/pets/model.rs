...
impl Pet {
    ...
    pub(super) fn load_with_firefam_home(value: &str, firefam_home: Option<&Path>) -> Result<Self> {
        ...
        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, firefam_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, firefam_home);
        }

        load_custom_pet(value, firefam_home)
    }
...
fn load_custom_pet(value: &str, firefam_home: Option<&Path>) -> Result<Pet> {
    ...
    let avatar_dir = firefam_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_firefam_home(dir.path().to_str().unwrap(), /*firefam_home*/ None).unwrap()
    }

    fn load_pet_error_from_dir(dir: &tempfile::TempDir) -> anyhow::Error {
        Pet::load_with_firefam_home(dir.path().to_str().unwrap(), /*firefam_home*/ None)
            .unwrap_err()
    }

    #[test]
    fn load_builtin_pet_uses_app_catalog_storage() {
        let firefam_home = tempfile::tempdir().unwrap();
        super::super::asset_pack::write_test_pack(firefam_home.path());

        let pet =
            Pet::load_with_firefam_home("dewey", /*firefam_home*/ Some(firefam_home.path()))
                .unwrap();
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_firefam_home(
    ...
    #[test]
    fn custom_pet_selector_loads_firefam_home_pet_manifest() {
        let dir = write_minimal_pet();
        let firefam_home = tempfile::tempdir().unwrap();
        let pet_dir = firefam_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let firefam_home = tempfile::tempdir().unwrap();
        let avatar_dir = firefam_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let firefam_home = tempfile::tempdir().unwrap();
        let pet_dir = firefam_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_firefam_home(
