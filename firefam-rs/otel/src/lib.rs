...
pub use firefam_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on firefam-core.
...
impl From<firefam_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: firefam_app_server_protocol::AuthMode) -> Self {
        match mode {
            firefam_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            firefam_app_server_protocol::AuthMode::Chatgpt
            | firefam_app_server_protocol::AuthMode::ChatgptAuthTokens
            | firefam_app_server_protocol::AuthMode::AgentIdentity => Self::Chatgpt,
        }
    }
}
