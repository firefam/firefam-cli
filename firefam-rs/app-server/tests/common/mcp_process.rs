...
use firefam_app_server_protocol::ClientNotification;
...
use firefam_app_server_protocol::ConfigReadParams;
use firefam_app_server_protocol::ConfigValueWriteParams;
use firefam_app_server_protocol::ExperimentalFeatureListParams;
use firefam_app_server_protocol::FeedbackUploadParams;
use firefam_app_server_protocol::FsCopyParams;
use firefam_app_server_protocol::FsCreateDirectoryParams;
use firefam_app_server_protocol::FsGetMetadataParams;
use firefam_app_server_protocol::FsReadDirectoryParams;
use firefam_app_server_protocol::FsReadFileParams;
use firefam_app_server_protocol::FsRemoveParams;
use firefam_app_server_protocol::FsUnwatchParams;
use firefam_app_server_protocol::FsWatchParams;
use firefam_app_server_protocol::FsWriteFileParams;
...
use firefam_app_server_protocol::JSONRPCError;
use firefam_app_server_protocol::JSONRPCErrorError;
use firefam_app_server_protocol::JSONRPCMessage;
use firefam_app_server_protocol::JSONRPCNotification;
use firefam_app_server_protocol::JSONRPCRequest;
use firefam_app_server_protocol::JSONRPCResponse;
...
use firefam_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use firefam_app_server_protocol::ProcessKillParams;
use firefam_app_server_protocol::ProcessResizePtyParams;
use firefam_app_server_protocol::ProcessSpawnParams;
use firefam_app_server_protocol::ProcessWriteStdinParams;
...
use firefam_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use firefam_login::default_client::FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "firefam-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "FIREFAM_APP_SERVER_DISABLE_MANAGED_CONFIG";

impl McpProcess {
    pub async fn new(firefam_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(firefam_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(firefam_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(firefam_home, &[(DISABLE_MANAGED_CONFIG_ENV_VAR, Some("1"))]).await
    }
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(firefam_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(firefam_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(firefam_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(firefam_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(firefam_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(firefam_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = firefam_utils_cargo_bin::cargo_bin("firefam-app-server")
            .context("should find binary for firefam-app-server")?;
        ...
        cmd.env(
            ...
            firefam_home.join("managed_config.toml"),
        );
        cmd.env_remove(FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("firefam-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: firefam_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `firefam-app-server` child process.
        //
