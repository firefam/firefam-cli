...
use firefam_app_server_protocol::SkillsListEntry;
use firefam_app_server_protocol::SkillsListResponse;
use firefam_core_skills::model::SkillDependencies;
use firefam_core_skills::model::SkillInterface;
use firefam_core_skills::model::SkillMetadata;
use firefam_core_skills::model::SkillToolDependency;
...
use firefam_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
...
pub(crate) fn find_app_mentions(
    ...
    for app in apps.iter().filter(|app| app.is_enabled) {
        let slug = firefam_connectors::metadata::connector_mention_slug(app);
