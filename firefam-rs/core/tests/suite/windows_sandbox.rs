...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("firefam-resources");
    std::fs::create_dir_all(&resources_dir)?;
    for helper_name in ["firefam-windows-sandbox-setup", "firefam-command-runner"] {
        let helper = firefam_utils_cargo_bin::cargo_bin(helper_name)?;
