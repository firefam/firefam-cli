...
use crate::protocol_event::firefam_turn_trace_event;
...
pub const FIREFAM_ROLLOUT_TRACE_ROOT_ENV: &str = "FIREFAM_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `FIREFAM_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Firefam
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(FIREFAM_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Firefam turn lifecycle events from protocol lifecycle events.
    pub fn record_firefam_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            firefam_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Firefam did while executing that boundary.
    pub fn record_tool_call_event(
        &self,
        firefam_turn_id: impl Into<FirefamTurnId>,
        event: &EventMsg,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        ...
        context.append_with_context_best_effort(firefam_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_firefam_turn_id: impl Into<FirefamTurnId>,
    ...
    pub fn record_firefam_turn_started(&self, firefam_turn_id: impl Into<FirefamTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let firefam_turn_id = firefam_turn_id.into();
        context.append_with_context_best_effort(
            firefam_turn_id.clone(),
            RawTraceEventPayload::FirefamTurnStarted {
                firefam_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(firefam_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Firefam turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        firefam_turn_id: FirefamTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            firefam_turn_id: Some(firefam_turn_id),
        };
