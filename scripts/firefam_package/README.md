# Firefam package builder

This package contains the implementation behind `scripts/build_firefam_package.py`.
...
The builder creates a canonical Firefam package directory:

```text
.
├── firefam-package.json
├── bin
│   └── <entrypoint>[.exe]
├── firefam-resources
│   ├── bwrap                             # Linux only
│   ├── firefam-command-runner.exe          # Windows only
│   └── firefam-windows-sandbox-setup.exe   # Windows only
└── firefam-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Firefam release
...
`firefam` and `firefam-app-server`. The `version` field in `firefam-package.json` is
read from `[workspace.package].version` in `firefam-rs/Cargo.toml`.
...
- Linux targets: `bwrap`
- Windows targets: `firefam-command-runner` and `firefam-windows-sandbox-setup`
...
DotSlash manifest at `firefam-cli/bin/rg`. Downloaded archives are cached under
`$TMPDIR/firefam-package/<target>-rg` and are reused only after the recorded size
