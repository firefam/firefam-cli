...
//! request payload that Firefam would send to the model and assert that the
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<FirefamThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.firefam)
}

async fn user_turn(conversation: &Arc<FirefamThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<FirefamThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<FirefamThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<FirefamThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = firefam_core::test_support::auth_manager_from_auth(
        firefam_login::FirefamAuth::from_api_key("dummy"),
    );
