...
use firefam_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    create_config_toml(firefam_home.path())?;
    ...
    let thread_id = seed_stage1_output(&state_db, firefam_home.path()).await?;
...
async fn seed_stage1_output(state_db: &Arc<StateRuntime>, firefam_home: &Path) -> Result<ThreadId> {
    let now = Utc::now();
    let thread_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let worker_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        firefam_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
