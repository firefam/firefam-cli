//! Asynchronous worker that executes a **Firefam** tool-call inside a spawned
...
use firefam_core::config::Config as FirefamConfig;
...
pub async fn run_firefam_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: FirefamConfig,
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!(
                    "Failed to start Firefam session: {e}"
                ))],
                is_error: Some(true),
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_firefam_uuid.lock().await.remove(&id);
        return;
    }

    run_firefam_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_firefam_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_firefam_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<FirefamThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(firefam_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("Firefam runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
