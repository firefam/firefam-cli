...
async fn steer_rejection_queues_review_follow_up_before_existing_queued_messages() {
    ...
    handle_error(
        ...
        Some(FirefamErrorInfo::ActiveTurnNotSteerable {
...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::FirefamOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::FirefamOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::FirefamOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        firefam_app_server_protocol::ServerNotification::TurnStarted(
            firefam_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: firefam_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: firefam_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: firefam_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        firefam_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            firefam_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: Some("turn-1".to_string()),
                goal: firefam_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "Run until the token budget is limited".to_string(),
                    status: firefam_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        firefam_app_server_protocol::ServerNotification::TurnCompleted(
            firefam_app_server_protocol::TurnCompletedNotification {
                ...
                turn: firefam_app_server_protocol::Turn {
                    ...
                    items_view: firefam_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: firefam_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
