...
use firefam_app_server_protocol::ConfigEdit;
use firefam_app_server_protocol::HookEventName;
use firefam_app_server_protocol::HookHandlerType;
use firefam_app_server_protocol::HookMetadata;
use firefam_app_server_protocol::HookSource;
use firefam_app_server_protocol::HookTrustStatus;
...
fn command_hook_hash(
    ...
    let Ok(value) = firefam_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    firefam_config::version_for_toml(&value)
}
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(firefam_home.path())?;
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let firefam_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(firefam_home.path(), "{ not-json")?;
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    .await??;
    let _: firefam_app_server_protocol::ConfigWriteResponse = to_response(response)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let firefam_home = TempDir::new()?;
    let hook_script_path = firefam_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = firefam_home
        .path()
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![firefam_home.path().to_path_buf()],
        })
        .await?;
    let response: JSONRPCResponse = timeout(
        DEFAULT_TIMEOUT,
