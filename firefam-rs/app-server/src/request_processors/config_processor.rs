...
use firefam_app_server_protocol::ComputerUseRequirements;
use firefam_app_server_protocol::ConfigBatchWriteParams;
use firefam_app_server_protocol::ConfigReadParams;
use firefam_app_server_protocol::ConfigReadResponse;
use firefam_app_server_protocol::ConfigRequirements;
use firefam_app_server_protocol::ConfigRequirementsReadResponse;
use firefam_app_server_protocol::ConfigValueWriteParams;
use firefam_app_server_protocol::ConfigWriteErrorCode;
use firefam_app_server_protocol::ConfigWriteResponse;
use firefam_app_server_protocol::ConfiguredHookHandler;
use firefam_app_server_protocol::ConfiguredHookMatcherGroup;
use firefam_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use firefam_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use firefam_app_server_protocol::JSONRPCErrorError;
use firefam_app_server_protocol::ManagedHooksRequirements;
use firefam_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use firefam_app_server_protocol::NetworkDomainPermission;
use firefam_app_server_protocol::NetworkRequirements;
use firefam_app_server_protocol::NetworkUnixSocketPermission;
...
use firefam_config::HookEventsToml;
use firefam_config::HookHandlerConfig as CoreHookHandlerConfig;
use firefam_config::ManagedHooksRequirementsToml;
use firefam_config::MatcherGroup as CoreMatcherGroup;
use firefam_config::ResidencyRequirement as CoreResidencyRequirement;
use firefam_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use firefam_core::ThreadManager;
use firefam_features::canonical_feature_for_key;
use firefam_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    async fn refresh_apps_list_after_experimental_feature_enablement_set(&self) {
        ...
        if !config.features.apps_enabled_for_auth(
            auth.as_ref()
                .is_some_and(firefam_login::FirefamAuth::uses_firefam_backend),
        ) {
            return;
        }
    ...
    ) -> Result<firefam_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = firefam_core_plugins::toggles::collect_plugin_enabled_candidates(
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata = firefam_core_plugins::loader::installed_plugin_telemetry_metadata(
                self.config_manager.firefam_home(),
                &plugin_id,
            )
            .await;
            if enabled {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(firefam_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(firefam_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
fn map_computer_use_requirements_to_api(
    computer_use: firefam_config::ComputerUseRequirementsToml,
...
) -> firefam_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => firefam_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: firefam_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(firefam_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(firefam_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(firefam_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: firefam_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        firefam_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        firefam_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: firefam_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        firefam_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        firefam_config::NetworkUnixSocketPermissionToml::None => NetworkUnixSocketPermission::None,
    }
}
...
mod tests {
    ...
    use firefam_config::ComputerUseRequirementsToml;
    use firefam_config::ConfigRequirementsToml;
