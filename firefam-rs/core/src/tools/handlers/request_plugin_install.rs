...
use firefam_tools::DiscoverableToolAction;
use firefam_tools::DiscoverableToolType;
use firefam_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use firefam_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use firefam_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use firefam_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use firefam_tools::RequestPluginInstallArgs;
use firefam_tools::RequestPluginInstallResult;
use firefam_tools::ToolName;
use firefam_tools::ToolSpec;
use firefam_tools::all_requested_connectors_picked_up;
use firefam_tools::build_request_plugin_install_elicitation_request;
use firefam_tools::filter_request_plugin_install_discoverable_tools_for_client;
use firefam_tools::verified_connector_install_completed;
...
impl ToolExecutor<ToolInvocation> for RequestPluginInstallHandler {
    ...
    async fn handle(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("firefam-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in firefam-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.firefam_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    firefam_home: &firefam_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&firefam_login::FirefamAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_firefam_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh firefam apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
