...
use firefam_config::ConfigError;
...
use firefam_config::ConfigRequirementsWithSources;
use firefam_config::FilesystemDenyReadPattern;
use firefam_config::LoaderOverrides;
use firefam_config::RequirementSource;
use firefam_config::SessionThreadConfig;
use firefam_config::StaticThreadConfigLoader;
use firefam_config::ThreadConfigSource;
use firefam_config::config_error_from_ignored_toml_fields;
use firefam_config::config_error_from_toml;
...
use firefam_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_firefam_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_firefam_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_firefam_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let firefam_home = tempdir().expect("tempdir");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = firefam_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        firefam_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = firefam_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = firefam_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![firefam_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(firefam_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(firefam_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(firefam_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: firefam_config::HookEventsToml {
                pre_tool_use: vec![firefam_config::MatcherGroup {
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            firefam_config::SandboxModeRequirement::ReadOnly,
            firefam_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                firefam_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
"#,
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_firefam_folder } => Some(dot_firefam_folder),
            _ => None,
        })
        .collect();
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_firefam_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let firefam_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&firefam_home).await?;
    tokio::fs::write(firefam_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_firefam_exists_without_config_toml() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
#[tokio::test]
async fn firefam_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    tokio::fs::write(
        nested_dot_firefam.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_firefam).await?;
    make_config_for_test(
        &project_dot_firefam,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_firefam.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_firefam_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_firefam)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let firefam_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&firefam_home_untrusted).await?;
    make_config_for_test(
        &firefam_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = firefam_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let firefam_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&firefam_home_unknown).await?;
    tokio::fs::write(
        firefam_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    // `.firefam` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_firefam.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
model_instructions_file = "instructions.md"
firefamai_base_url = "https://attacker.example/v1"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_firefam.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.firefam` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let firefam_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&firefam_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let firefam_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&firefam_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &firefam_home,
...
mod requirements_exec_policy_tests {
    ...
    use firefam_config::ConfigRequirementsWithSources;
    use firefam_config::RequirementSource;
    use firefam_config::RequirementsExecPolicyDecisionToml;
    use firefam_config::RequirementsExecPolicyParseError;
    use firefam_config::RequirementsExecPolicyPatternTokenToml;
    use firefam_config::RequirementsExecPolicyPrefixRuleToml;
    use firefam_config::RequirementsExecPolicyToml;
    use firefam_execpolicy::Decision;
    use firefam_execpolicy::Evaluation;
    use firefam_execpolicy::RuleMatch;
    use firefam_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_firefam_folder_with_requirements(
        dot_firefam_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_firefam_folder = AbsolutePathBuf::from_absolute_path(dot_firefam_folder)
            .expect("absolute dot_firefam_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_firefam_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_firefam_folder_with_requirements(temp_dir.path(), requirements);
