...
pub(super) fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.firefam_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match firefam_app_server::app_server_control_socket_path(&config.firefam_home)
    {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run firefam app-server daemon version for more details.");
    }
    check
}
