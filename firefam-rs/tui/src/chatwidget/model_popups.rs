...
impl ChatWidget {
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        let base_url = self.custom_firefamai_base_url()?;
        let warning = format!(
            "Warning: FirefamAI base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
        Some(Line::from(warning.red()))
    }

    fn custom_firefamai_base_url(&self) -> Option<String> {
        if !self.config.model_provider.is_firefamai() {
            return None;
        }
        ...
        if normalized == DEFAULT_FIREFAMAI_BASE_URL {
            return None;
        }
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("firefam-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "firefam-auto-fast" => 0,
            "firefam-auto-balanced" => 1,
            "firefam-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running firefam -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
