...
use anyhow::Context;
use clap::Args;
use firefam_app_server::AppServerRuntimeOptions;
use firefam_app_server::AppServerTransport;
use firefam_app_server::AppServerWebsocketAuthSettings;
use firefam_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use firefam_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use firefam_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use firefam_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use firefam_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use firefam_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = firefam_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = firefam_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
...
async fn run_foreground_remote_control(
    ...
    let mut app_server_task = tokio::spawn(firefam_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    firefam_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_firefam_path, managed_firefam_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_firefam_path.display()),
        format!(
            "  version: {}",
            managed_firefam_version.unwrap_or("unknown")
        ),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_firefam_path,
            output.managed_firefam_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_firefam_path: PathBuf::from("/opt/firefam/bin/firefam"),
                managed_firefam_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/firefam/bin/firefam".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedFirefamPath": "/opt/firefam/bin/firefam",
                    "managedFirefamVersion": "1.0.0",
