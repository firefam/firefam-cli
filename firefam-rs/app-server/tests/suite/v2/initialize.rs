...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_firefam_home = AbsolutePathBuf::try_from(firefam_home.path().canonicalize()?)?;
    create_config_toml(firefam_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(firefam_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "firefam_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        firefam_home: response_firefam_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("firefam_vscode/"));
    assert_eq!(response_firefam_home, expected_firefam_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "firefam_app_server_daemon".to_string(),
            title: Some("Firefam App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("firefam_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_firefam_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let firefam_home = TempDir::new()?;
    create_config_toml(firefam_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "firefam-backend".to_string(),
            title: Some("Firefam Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("firefam_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("firefam_originator_via_env_var/"));
    assert_eq!(response_firefam_home, expected_firefam_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        firefam_home.path(),
        &[("FIREFAM_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let firefam_home = TempDir::new()?;
    let notify_file = firefam_home.path().join("notify.json");
    let notify_capture = cargo_bin("firefam-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(firefam_home, server_uri, approval_policy, "")
}
