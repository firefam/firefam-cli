...
use firefam_config::HooksFile;
use firefam_config::types::McpServerConfig;
use firefam_config::types::PluginConfig;
use firefam_config::types::PluginMcpServerConfig;
use firefam_core_skills::SkillMetadata;
use firefam_core_skills::config_rules::SkillConfigRules;
use firefam_core_skills::config_rules::resolve_disabled_skill_paths;
use firefam_core_skills::config_rules::skill_config_rules_from_stack;
use firefam_core_skills::loader::SkillRoot;
use firefam_core_skills::loader::load_skills_from_roots;
use firefam_exec_server::LOCAL_FS;
use firefam_plugin::AppConnectorId;
use firefam_plugin::LoadedPlugin;
...
use firefam_plugin::PluginLoadOutcome;
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(firefam_home.to_path_buf()).map_err(|err| err.to_string())?;
    ...
    for plugin in curated_marketplace.plugins {
        ...
        if plugin_sources.contains_key(&plugin_name) {
            warn!(
                plugin = plugin_name,
                marketplace = FIREFAMAI_CURATED_MARKETPLACE_NAME,
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_firefam_home(
            firefam_home,
    ...
    for marketplace in marketplace_outcome.marketplaces {
        if marketplace.name == FIREFAMAI_CURATED_MARKETPLACE_NAME {
            continue;
        }
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(firefam_home, &source).map_err(|err| {
...
fn configured_plugins_from_firefam_home(
    firefam_home: &Path,
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| plugin_id.marketplace_name == FIREFAMAI_CURATED_MARKETPLACE_NAME)
    .collect::<Vec<_>>();
...
pub fn configured_curated_plugin_ids_from_firefam_home(firefam_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_firefam_home(
        firefam_home,
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(firefam_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = firefam_home.join("plugins/.marketplace-plugin-source-staging");
