...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let firefam_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = firefam_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&firefam_home)
        .await
        .unwrap();
