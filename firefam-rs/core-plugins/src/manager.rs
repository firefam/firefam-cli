...
use crate::FIREFAMAI_CURATED_MARKETPLACE_NAME;
use crate::installed_marketplaces::installed_marketplace_roots_from_layer_stack;
use crate::loader::configured_curated_plugin_ids_from_firefam_home;
...
use crate::startup_sync::sync_firefamai_plugins_repo;
...
use firefam_config::PluginConfigEdit;
use firefam_config::apply_user_plugin_config_edits;
use firefam_config::clear_user_plugin;
use firefam_config::set_user_plugin_enabled;
use firefam_config::types::PluginConfig;
use firefam_config::version_for_toml;
use firefam_core_skills::SkillMetadata;
use firefam_hooks::plugin_hook_declarations;
...
use firefam_plugin::PluginIdError;
use firefam_plugin::prompt_safe_plugin_description;
...
use firefam_utils_plugins::PluginSkillRoot;
...
impl PluginsManager {
    pub fn new(firefam_home: PathBuf) -> Self {
        Self::new_with_restriction_product(firefam_home, Some(Product::Firefam))
    }
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == FIREFAMAI_CURATED_MARKETPLACE_NAME {
                let curated_plugin_version = read_curated_plugins_sha(self.firefam_home.as_path())
                    .ok_or_else(|| {
        ...
        let firefam_home = self.firefam_home.clone();
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(firefam_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = if self.store.active_plugin_root(&plugin_id).is_some() {
            Some(installed_plugin_telemetry_metadata(self.firefam_home.as_path(), &plugin_id).await)
        } else {
            None
        };
        let store = self.store.clone();
        ...
        clear_user_plugin(&self.firefam_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.firefam_home.as_path());
        ...
        let config_result = if config_edits.is_empty() {
            ...
            apply_user_plugin_config_edits(&self.firefam_home, config_edits).await
        };
        self.clear_cache();
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path =
            if matches!(plugin.source, MarketplacePluginSource::Git { .. }) && plugin.installed {
                ...
                let materialized = tokio::task::spawn_blocking(move || {
                    materialize_marketplace_plugin_source(firefam_home.as_path(), &source)
                })
                .await
                .map_err(|err| {
                    MarketplaceError::InvalidPlugin(format!(
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest.paths,
            self.restriction_product,
            &firefam_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
        )
        .await;
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_firefamai_plugins_repo(firefam_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_firefam_home(firefam_home.as_path());
    ...
    fn run_non_curated_plugin_cache_refresh_loop(self: Arc<Self>) {
        loop {
            ...
            let refresh_result = match request.mode {
                NonCuratedCacheRefreshMode::IfVersionChanged => {
                    refresh_non_curated_plugin_cache(self.firefam_home.as_path(), &request.roots)
                }
    ...
    fn marketplace_roots(
        ...
        let curated_repo_root = curated_plugins_repo_path(self.firefam_home.as_path());
