...
```rust
fn create_firefamai_url(base_url: Option<String>, retry_count: usize) -> String {
...
targeted package runs such as `just argument-comment-lint -p firefam-core`.
...
```bash
./tools/argument-comment-lint/run.py -p firefam-core
```

Run the lint against `firefam-rs` from the repo root:

```bash
just argument-comment-lint
bazel build --config=argument-comment-lint -- \
  $(./tools/argument-comment-lint/list-bazel-targets.sh)
./tools/argument-comment-lint/run-prebuilt-linter.py -p firefam-core
just argument-comment-lint -p firefam-core
```

If no package selection is provided, `just argument-comment-lint` now defaults
to the Bazel aspect path over `//firefam-rs/...`. The Python wrappers remain the
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.py -p firefam-core -- --all-targets
```
