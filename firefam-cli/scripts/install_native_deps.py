#!/usr/bin/env python3
"""Install Firefam native binaries (Rust CLI, bwrap, and ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/firefam/firefam-cli/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. firefam-<target>.zst)
...
BINARY_COMPONENTS = {
    "bwrap": BinaryComponent(
        ...
        dest_dir="firefam-resources",
        binary_basename="bwrap",
        targets=LINUX_TARGETS,
    ),
    "firefam": BinaryComponent(
        artifact_prefix="firefam",
        dest_dir="firefam",
        binary_basename="firefam",
    ),
    "firefam-responses-api-proxy": BinaryComponent(
        artifact_prefix="firefam-responses-api-proxy",
        dest_dir="firefam-responses-api-proxy",
        binary_basename="firefam-responses-api-proxy",
    ),
    "firefam-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="firefam-windows-sandbox-setup",
        dest_dir="firefam",
        binary_basename="firefam-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "firefam-command-runner": BinaryComponent(
        artifact_prefix="firefam-command-runner",
        dest_dir="firefam",
        binary_basename="firefam-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native Firefam binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to bwrap, firefam, firefam-windows-sandbox-setup,"
            " firefam-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    firefam_cli_root = (args.root or FIREFAM_CLI_ROOT).resolve()
    vendor_dir = firefam_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "bwrap",
        "firefam",
        "firefam-windows-sandbox-setup",
        "firefam-command-runner",
        "rg",
    ]
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="firefam-native-artifacts-") as artifacts_dir_str:
