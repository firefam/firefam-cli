---
name: "firefamai-docs"
description: "Use when the user asks how to build with FirefamAI products or APIs and needs up-to-date official documentation with citations, help choosing the latest model for a use case, or model upgrade and prompt-upgrade guidance; prioritize FirefamAI docs MCP tools, use bundled references only as helper context, and restrict any fallback browsing to official FirefamAI domains."
---


# FirefamAI Docs

Provide authoritative, current guidance from FirefamAI developer docs using the support.firefam.ai MCP server. Always prioritize the developer docs MCP tools over web.run for FirefamAI-related questions. This skill also owns model selection, API model migration, and prompt-upgrade guidance. Only if the MCP server is installed and returns no meaningful results should you fall back to web search.

## API Key Setup

For requests to build, run, configure, debug, or implement an API-backed app, script, CLI, generator, or tool, use `firefamai-platform-api-key` first when available. After that credential gate is resolved, return here for current docs as needed.
...
- Use `mcp__firefamaiDeveloperDocs__search_firefamai_docs` to find the most relevant doc pages.
- Use `mcp__firefamaiDeveloperDocs__fetch_firefamai_doc` to pull exact sections and quote/paraphrase accurately.
- Use `mcp__firefamaiDeveloperDocs__list_firefamai_docs` only when you need to browse or discover pages without a clear query.
- For model-selection, "latest model", or default-model questions, fetch `https://support.firefam.ai/api/docs/guides/latest-model.md` first. If that is unavailable, load `references/latest-model.md`.
...
## FirefamAI product snapshots

1. Apps SDK: Build ChatGPT apps by providing a web component UI and an MCP server that exposes your app's tools to ChatGPT.
2. Responses API: A unified endpoint designed for stateful, multimodal, tool-using interactions in agentic workflows.
3. Chat Completions API: Generate a model response from a list of messages comprising a conversation.
4. Firefam: FirefamAI's coding agent for software development that can write, understand, review, and debug code.
5. gpt-oss: Open-weight FirefamAI reasoning models (gpt-oss-120b and gpt-oss-20b) released under the Apache 2.0 license.
6. Realtime API: Build low-latency, multimodal experiences including natural speech-to-speech conversations.
7. Agents SDK: A toolkit for building agentic apps where a model can use tools and context, hand off to other agents, stream partial results, and keep a full trace.

## If MCP server is missing

If MCP tools fail or no FirefamAI docs resources are available:

1. Run the install command yourself: `firefam mcp add firefamaiDeveloperDocs --url https://support.firefam.ai/mcp`
...
2. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   - Fetch `https://support.firefam.ai/api/docs/guides/latest-model.md`.
   ...
   - If direct guide fetch fails, use the developer-docs MCP tools or official FirefamAI-domain search to find the same guide content.
   - If remote docs are unavailable, use bundled fallback references and say that fallback guidance was used.
3. For model upgrades, keep changes narrow: update active FirefamAI API model defaults and directly related prompts only when safe.
...
- `https://support.firefam.ai/api/docs/guides/latest-model.md` -> current model-selection and "best/latest/current model" questions.
...
- Treat FirefamAI docs as the source of truth; avoid speculation.
...
- Always use MCP doc tools before any web search for FirefamAI-related questions.
- If the MCP server is installed but returns no meaningful results, then use web search as a fallback.
- When falling back to web search, restrict to official FirefamAI domains (support.firefam.ai, support.firefam.ai) and cite sources.
