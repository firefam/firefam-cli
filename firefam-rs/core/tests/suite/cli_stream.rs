...
fn repo_root() -> std::path::PathBuf {
    ...
    firefam_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = firefam_utils_cargo_bin::cargo_bin("firefam").unwrap();
    ...
    //     10,
    //     None,
    //     firefam_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
/// Ensures `firefamai_base_url` config override routes built-in firefamai provider requests.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_supports_firefamai_base_url_config_override() {
    skip_if_no_network!();
    ...
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("firefamai_base_url=\"{}/v1\"", server.uri()))
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("AGENTS_HOME", home.path())
...
/// received by a mock FirefamAI Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Firefam to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `firefam exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the firefam CLI and invoke `exec`, which is what records a session.
    let bin = firefam_utils_cargo_bin::cargo_bin("firefam").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.timeout(Duration::from_secs(30));
    cmd.arg("exec")
    ...
    let bin2 = firefam_utils_cargo_bin::cargo_bin("firefam").unwrap();
    ...
    assert!(output2.status.success(), "resume firefam-cli run failed");
