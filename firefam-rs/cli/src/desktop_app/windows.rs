...
const FIREFAM_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const FIREFAM_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";

pub async fn run_windows_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_id) = find_firefam_app_id().await? {
        eprintln!("Opening Firefam Desktop...");
        open_installed_firefam_app(&app_id).await?;
        eprintln!(
            "In Firefam Desktop, open workspace {workspace}.",
            workspace = display_workspace_path(&workspace)
        );
        return Ok(());
    }

    eprintln!("Firefam Desktop not found; opening Windows installer...");
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(FIREFAM_WINDOWS_INSTALLER_URL);
    if open_url(download_url).await.is_err() && download_url_override.is_none() {
        open_url(FIREFAM_MICROSOFT_STORE_WEB_URL).await?;
    }
    eprintln!(
        "After installing Firefam Desktop, open workspace {workspace}.",
        workspace = display_workspace_path(&workspace)
    );
    Ok(())
}

async fn find_firefam_app_id() -> anyhow::Result<Option<String>> {
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg("Get-StartApps -Name 'Firefam' | Select-Object -First 1 -ExpandProperty AppID")
        .output()
        .await
...
async fn open_installed_firefam_app(app_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn display_workspace_path_removes_windows_extended_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\C:\Users\fcoury\code\firefam")),
            r"C:\Users\fcoury\code\firefam"
        );
    }
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\firefam")),
            r"\\server\share\firefam"
        );
    }
