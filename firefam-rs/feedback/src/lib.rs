...
/// Filename used for the redacted `firefam doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "firefam-doctor-report.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_recovery_followup_success = tracing::field::debug(&snapshot.auth_recovery_followup_success),
        auth_recovery_followup_status = tracing::field::debug(&snapshot.auth_recovery_followup_status),
        auth_env_firefamai_api_key_present = tracing::field::debug(auth_env.firefamai_api_key_env_present),
        auth_env_firefam_api_key_present = tracing::field::debug(auth_env.firefam_api_key_env_present),
        auth_env_firefam_api_key_enabled = tracing::field::debug(auth_env.firefam_api_key_env_enabled),
...
#[derive(Clone)]
pub struct FirefamFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for FirefamFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl FirefamFeedback {
    pub fn new() -> Self {
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `firefam-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("firefam-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("firefam-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = FirefamFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("firefam-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = FirefamFeedback::new()
        ...
        let attachments_without_diagnostics = FirefamFeedback::new()
            .snapshot(/*session_id*/ None)
