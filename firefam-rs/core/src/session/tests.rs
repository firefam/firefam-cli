...
use firefam_config::ConfigLayerStackOrdering;
use firefam_config::LoaderOverrides;
use firefam_config::NetworkConstraints;
use firefam_config::NetworkDomainPermissionToml;
use firefam_config::NetworkDomainPermissionsToml;
use firefam_config::RequirementSource;
use firefam_config::Sourced;
...
use firefam_models_manager::model_info;
...
use firefam_protocol::SessionId;
use firefam_protocol::ThreadId;
use firefam_protocol::account::PlanType as AccountPlanType;
use firefam_protocol::config_types::ServiceTier;
use firefam_protocol::config_types::TrustLevel;
use firefam_protocol::exec_output::ExecToolCallOutput;
use firefam_protocol::models::ActivePermissionProfile;
use firefam_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use firefam_protocol::models::FileSystemPermissions;
use firefam_protocol::models::FunctionCallOutputBody;
use firefam_protocol::models::FunctionCallOutputPayload;
use firefam_protocol::models::PermissionProfile;
use firefam_protocol::models::SandboxEnforcement;
use firefam_protocol::permissions::FileSystemAccessMode;
use firefam_protocol::permissions::FileSystemPath;
use firefam_protocol::permissions::FileSystemSandboxEntry;
use firefam_protocol::permissions::FileSystemSandboxPolicy;
use firefam_protocol::permissions::FileSystemSpecialPath;
use firefam_protocol::protocol::NonSteerableTurnKind;
use firefam_protocol::protocol::SandboxPolicy;
use firefam_protocol::protocol::TurnEnvironmentSelection;
use firefam_protocol::request_permissions::PermissionGrantScope;
use firefam_protocol::request_permissions::RequestPermissionProfile;
use tracing::Span;
...
use firefam_app_server_protocol::McpElicitationSchema;
use firefam_config::config_toml::ConfigToml;
use firefam_config::config_toml::ProjectConfig;
use firefam_execpolicy::Decision;
use firefam_execpolicy::NetworkRuleProtocol;
use firefam_execpolicy::Policy;
use firefam_network_proxy::NetworkProxyConfig;
use firefam_otel::MetricsClient;
use firefam_otel::MetricsConfig;
use firefam_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use firefam_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use firefam_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use firefam_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use firefam_otel::TelemetryAuthMode;
...
use firefam_protocol::models::BaseInstructions;
...
use firefam_protocol::protocol::CompactedItem;
use firefam_protocol::protocol::ConversationAudioParams;
use firefam_protocol::protocol::CreditsSnapshot;
use firefam_protocol::protocol::GranularApprovalConfig;
use firefam_protocol::protocol::InitialHistory;
use firefam_protocol::protocol::InterAgentCommunication;
use firefam_protocol::protocol::NetworkApprovalProtocol;
use firefam_protocol::protocol::RateLimitSnapshot;
use firefam_protocol::protocol::RateLimitWindow;
use firefam_protocol::protocol::RealtimeAudioFrame;
use firefam_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use firefam_protocol::protocol::RealtimeVoice;
...
use firefam_protocol::protocol::ThreadGoalStatus;
use firefam_protocol::protocol::ThreadRolledBackEvent;
use firefam_protocol::protocol::ThreadSettingsOverrides;
use firefam_protocol::protocol::TokenCountEvent;
use firefam_protocol::protocol::TokenUsage;
use firefam_protocol::protocol::TokenUsageInfo;
use firefam_protocol::protocol::TurnAbortedEvent;
use firefam_protocol::protocol::TurnCompleteEvent;
use firefam_protocol::protocol::TurnStartedEvent;
use firefam_protocol::protocol::UserMessageEvent;
use firefam_protocol::protocol::W3cTraceContext;
use firefam_protocol::request_user_input::RequestUserInputAnswer;
use firefam_protocol::request_user_input::RequestUserInputResponse;
use firefam_rmcp_client::ElicitationAction;
...
use firefam_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "firefam-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "firefam_apps".to_string(),
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        ModelProviderInfo::create_firefamai_provider(/* base_url */ /*base_url*/ None),
        firefam_protocol::protocol::SessionSource::Exec,
...
fn write_project_hooks(dot_firefam: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_firefam)?;
    std::fs::write(
        dot_firefam.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        firefam_home.join(firefam_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<firefam_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&firefam_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: firefam_hooks::StartHookTarget::SessionStart {
                source: firefam_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn firefam_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { firefam_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> firefam_sandboxing::SandboxablePreference {
            firefam_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(firefam_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: firefam_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let config_toml_path = firefam_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = firefam_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let firefam_home = session.firefam_home().await;
    std::fs::create_dir_all(&firefam_home).expect("create firefam home");
    let base_config_path = firefam_home.join(CONFIG_TOML_FILE);
    let profile_config_path = firefam_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .firefam_home(firefam_home.to_path_buf())
    ...
    assert_eq!(
        config
            ...
            .map(firefam_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    ...
    let config_toml_path = firefam_home.join(CONFIG_TOML_FILE);
    let user_config: firefam_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = firefam_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = firefam_hooks::list_hooks(firefam_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..firefam_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        firefam_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: firefam_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: firefam_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: firefam_config::MatcherGroup {
                matcher: None,
                hooks: vec![firefam_config::HookHandlerConfig::Command {
        ...
        let identity = firefam_config::TomlValue::try_from(identity)?;
        firefam_config::version_for_toml(&identity)
    };
...
disabled_tools = [
  ...
  { type = "plugin", id = "slack@firefamai-curated" },
]
"#,
    )
    .expect("write user config");
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl firefam_extension_api::TokenUsageContributor for TokenUsageRecorder {
        async fn on_token_usage(
            &self,
            session_store: &firefam_extension_api::ExtensionData,
            thread_store: &firefam_extension_api::ExtensionData,
            turn_store: &firefam_extension_api::ExtensionData,
            token_usage: &TokenUsageInfo,
        ) {
            self.records
                .lock()
    ...
    let mut builder =
        firefam_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl firefam_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: firefam_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_firefam().with_config(move |config| {
        config
            .permissions
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let firefam_home = tempfile::TempDir::new()?;
    ...
    let config = ConfigBuilder::default()
        .firefam_home(firefam_home.path().to_path_buf())
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_firefam().with_config(|config| {
        config.permissions.approval_policy =
            firefam_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        firefam_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "firefam_core__firefam_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: firefam_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            firefam_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            firefam_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            firefam_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.firefam_error_info,
        Some(FirefamErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let firefam_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(firefam_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        firefam_home: config.firefam_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(firefam_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("firefam_other".to_string()),
        limit_name: Some("firefam_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(firefam_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("firefam".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.firefam_error_info == Some(FirefamErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(firefam_home: &Path) -> Config {
...
fn turn_environments_for_tests(
    environment: &Arc<firefam_exec_server::Environment>,
    cwd: &firefam_utils_absolute_path::AbsolutePathBuf,
) -> crate::environment_selection::ResolvedTurnEnvironments {
    crate::environment_selection::ResolvedTurnEnvironments {
        turn_environments: vec![TurnEnvironment {
            environment_id: firefam_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = firefam_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        firefam_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let firefam_home = parent_config.firefam_home.clone();
    let skill_dir = firefam_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&firefam_exec_server::LOCAL_FS));
    ...
    let role_path = firefam_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let firefam_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(firefam_home.path()).await;
    config
        .features
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(FirefamAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.firefam_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.firefam_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(firefam_extension_api::ExtensionRegistryBuilder::new().build()),
        AgentControl::default(),
        Arc::new(firefam_exec_server::EnvironmentManager::default_for_tests()),
        /*analytics_events_client*/ None,
        Arc::new(firefam_thread_store::LocalThreadStore::new(
            firefam_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        firefam_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        firefam_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: firefam_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(firefam_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: firefam_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: firefam_extension_api::ExtensionData::new(thread_id.to_string()),
        agent_control,
        network_proxy: None,
        network_approval: Arc::clone(&network_approval),
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(firefam_thread_store::LocalThreadStore::new(
            firefam_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
        ...
        environment_manager: Arc::new(firefam_exec_server::EnvironmentManager::default_for_tests()),
    };
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    ...
    ConfigBuilder::default()
        .firefam_home(config.firefam_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(firefam_thread_store::LocalThreadStore::new(
            firefam_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                firefam_state::StateRuntime::init(
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            firefam_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(firefam_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = firefam_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(firefam_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    firefam_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("firefam-core-tests");
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            final_output_json_schema: None,
            responsesapi_client_metadata: None,
            thread_settings: firefam_protocol::protocol::ThreadSettingsOverrides {
                cwd: Some(config.cwd.to_path_buf()),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(firefam_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(firefam_protocol::config_types::CollaborationMode {
                    mode: firefam_protocol::config_types::ModeKind::Default,
                    settings: firefam_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        firefam_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, FirefamErr::InvalidRequest(_)));
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        firefam_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        firefam_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(firefam_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn firefam_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        firefam_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_emits_thread_stop_lifecycle() {
    ...
    #[async_trait::async_trait]
    impl firefam_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        async fn on_thread_stop(&self, input: firefam_extension_api::ThreadStopInput<'_>) {
            assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    #[async_trait::async_trait]
    impl firefam_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        async fn on_turn_abort(&self, input: firefam_extension_api::TurnAbortInput<'_>) {
            assert_eq!(
                self.expected_thread_id.to_string(),
                input.thread_store.level_id()
            );
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let firefam = Arc::clone(&firefam);
        tokio::spawn(async move { firefam.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let firefam = Arc::clone(&firefam);
        tokio::spawn(async move { firefam.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_firefam)
        .await;
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(firefam_home).await;
...
pub(crate) async fn make_session_and_context_with_dynamic_tools_and_rx(
    ...
    make_session_and_context_with_auth_and_config_and_rx(
        FirefamAuth::from_api_key("Test API Key"),
        dynamic_tools,
        |_config| {},
    )
    .await
...
async fn make_goal_session_and_context_with_rx() -> (
    ...
    (session, turn_context, rx, firefam_home)
}
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = firefam_state::ThreadMetadataBuilder::new(
...
impl firefam_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute<'a>(
        &'a self,
        _session_store: &'a firefam_extension_api::ExtensionData,
        thread_store: &'a firefam_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<firefam_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    firefam_extension_api::PromptFragment::developer_policy(
...
-> Arc<firefam_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = firefam_extension_api::ExtensionRegistryBuilder::new();
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        &turn_context.config.firefam_home,
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(firefam_extension_api::ExtensionData::new(
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = firefam_protocol::user_input::TextElement::new(
        firefam_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn interrupt_accounts_active_goal_without_pausing() -> anyhow::Result<()> {
    let (sess, tc, _rx, _firefam_home) = make_goal_session_and_context_with_rx().await;
    ...
    assert_eq!(
        firefam_protocol::protocol::ThreadGoalStatus::Active,
        goal.status
    );
...
async fn active_goal_continuation_runs_again_after_no_tool_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.firefam.next_event().await?;
...
async fn pending_request_user_input_does_not_spawn_extra_goal_continuation() -> anyhow::Result<()> {
    ...
    let request_user_input_event = wait_for_event_match(&test.firefam, |event| match event {
    ...
    assert!(
        timeout(Duration::from_millis(200), test.firefam.next_event())
            .await
            .is_err(),
...
async fn goal_test_state_db(sess: &Session) -> anyhow::Result<crate::StateDbHandle> {
    ...
    firefam_state::StateRuntime::init(config.sqlite_home.clone(), config.model_provider_id.clone())
        .await
}
...
async fn create_thread_goal_fills_empty_thread_preview() -> anyhow::Result<()> {
    ...
    let page = state_db
        .list_threads(
            /*page_size*/ 10,
            firefam_state::ThreadFilterOptions {
                archived_only: false,
                allowed_sources: &[],
                model_providers: None,
                cwd_filters: None,
                anchor: None,
                sort_key: firefam_state::SortKey::UpdatedAt,
                sort_direction: firefam_state::SortDirection::Desc,
                search_term: None,
            },
        )
        .await?;
...
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    let (sess, tc, rx, _firefam_home) = make_goal_session_and_context_with_rx().await;
    sess.set_thread_goal(
        tc.as_ref(),
        SetGoalRequest {
            objective: Some("Keep improving the benchmark".to_string()),
    ...
    assert_eq!(firefam_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn usage_limit_runtime_stops_active_goal_and_prevents_idle_continuation() -> anyhow::Result<()>
{
    ...
    assert_eq!(firefam_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_goal_mutation_accounts_active_turn_before_status_change() -> anyhow::Result<()> {
    ...
    let updated_goal = state_db
        ...
        .update_thread_goal(
            ...
            firefam_state::ThreadGoalUpdate {
                objective: None,
                status: Some(firefam_state::ThreadGoalStatus::Complete),
                token_budget: None,
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(firefam_state::ThreadGoalStatus::Complete, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_objective_change_steers_active_turn() -> anyhow::Result<()> {
    ...
    let old_goal = state_db
        ...
        .replace_thread_goal(
            ...
            firefam_state::ThreadGoalStatus::Active,
...
async fn external_active_goal_set_marks_current_turn_for_accounting() -> anyhow::Result<()> {
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(firefam_state::ThreadGoalStatus::Active, goal.status);
    assert_eq!(25, goal.tokens_used);
...
async fn completed_goal_accounts_current_turn_tokens_before_tool_response() -> anyhow::Result<()> {
    ...
    let state_db = firefam_state::StateRuntime::init(
    ...
    assert_eq!(
        firefam_state::ThreadGoalStatus::Complete,
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(firefam_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn create_goal_tool_rejects_existing_goal() {
    let (session, turn_context, _rx, _firefam_home) = make_goal_session_and_context_with_rx().await;
    let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::new()));
    ...
    handler
        .handle(ToolInvocation {
            ...
            tool_name: firefam_tools::ToolName::plain("create_goal"),
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let response = update_handler
        .handle(ToolInvocation {
            ...
            tool_name: firefam_tools::ToolName::plain("update_goal"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "status": "paused",
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use firefam_protocol::protocol::AskForApproval;
    use firefam_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: firefam_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: firefam_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    write_project_hooks(&root_dot_firefam)?;
    write_project_hooks(&nested_dot_firefam)?;
    write_project_trust_config(&firefam_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .firefam_home(firefam_home)
    ...
    let expected_source_path = firefam_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_firefam.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let firefam_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&firefam_home)?;
        write_project_trust_config(&firefam_home, &trust_entries).await?;
